...
pro `dokument_typ`. Open-Tracking kommt nicht.

## Consequences

- Migration: `vertraege.status` (Backfill aus Datei/`is_draft`), `mailvorlage.dokument_typ`,
  Unique-Standard pro Typ, Vertrag-Standardvorlage.
- System schreibt Entwurf/Erstellt/Gesendet/Unterschrieben; intern nur Verzögert/Abgelehnt plus Zurück.
