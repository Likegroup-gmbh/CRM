// PdfBrand.js
// LikeGroup-Auftritt fuer alle PDFs (Vertrag + Briefing): Logo, Anschrift, Footer.
// Eine Quelle — Caller zeichnen nur noch.

export const LIKEGROUP = {
  name: 'LikeGroup GmbH',
  street: 'Jakob-Latscha-Str. 3',
  zip: '60314',
  city: 'Frankfurt am Main',
  country: { de: 'Deutschland', en: 'Germany' },
};

export const LIKEGROUP_FOOTER_DE =
  'LikeGroup GmbH | Jakob-Latscha-Str. 3 | 60314 Frankfurt am Main | Deutschland';

export function likeGroupFooterLine(lang = 'de') {
  if (lang === 'en') {
    return 'LikeGroup GmbH | Jakob-Latscha-Str. 3 | 60314 Frankfurt am Main | Germany';
  }
  return LIKEGROUP_FOOTER_DE;
}

export const LIKEGROUP_LETTERHEAD = [
  LIKEGROUP.name,
  LIKEGROUP.street,
  `${LIKEGROUP.zip} ${LIKEGROUP.city}`,
];

export const PDF_BRAND = {
  footerY: 285,
  pageX: 196,
  logoLeft: { x: 14, y: 10, w: 22.75, h: 12.6 },
...
/** PNG-Data-URL oder null (Tests / fehlendes Canvas). */
export function loadLikeGroupLogoPng() {
  if (logoPromise) return logoPromise;
  logoPromise = (async () => {
    try {
      // jsdom hat kein Canvas — OffscreenCanvas fehlt dort, im Browser nicht.
      if (typeof OffscreenCanvas === 'undefined') return null;
      if (typeof Image === 'undefined') return null;
      const ctx = document.createElement('canvas').getContext?.('2d');
      if (!ctx) return null;
      return await svgToPngDataUrl(LIKEGROUP_LOGO_SVG, 120, 66);
    } catch (err) {
      console.warn('LikeGroup-Logo wird übersprungen:', err);
      return null;
    }
  })();
  return logoPromise;
...
export function drawLikeGroupFooter(doc, { page, lang = 'de', y = PDF_BRAND.footerY } = {}) {
  ...
  doc.text(likeGroupFooterLine(lang), 14, y);
  const pageLabel = lang === 'en' ? 'Page' : 'Seite';
  doc.text(`${pageLabel} ${page}`, PDF_BRAND.pageX, y, { align: 'right' });
  doc.setTextColor(0);
