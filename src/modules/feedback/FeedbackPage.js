...
import { FeedbackNotifications } from './FeedbackNotifications.js';
...
export const feedbackPage = {
  ...
  async addComment(feedbackId, text) {
    ...
    // Benachrichtigung an den Ticket-Ersteller senden
    const feedback = this.feedbacks.find(f => f.id === feedbackId);
    if (feedback?.created_by) {
      const commenterName = window.currentUser?.name || 'Jemand';
      FeedbackNotifications.createCommentNotification(
        feedback.created_by,
        feedbackId,
        text,
        commenterName
  ...
  async archiveFeedback(feedbackId) {
    ...
    // Feedback-Notifications aktualisieren (Ticket-Count)
    window.feedbackNotifications?.refresh();
  ...
  async onDrop(e) {
    ...
    // Feedback-Notifications aktualisieren (Badge-Zahl)
    window.feedbackNotifications?.refresh();
  },
