    ...
    document.getElementById('kickoff-inline-save')?.addEventListener('click', async () => {
      ...
      try {
        ...
        const insertData = {
          ...
          unternehmen_id: detail.data?.id || detail.unternehmenId || null,
          marke_id: null,
        ...
        if (detail.loadData) {
          await detail.loadData(detail.data?.id || detail.unternehmenId);
          if (detail.render) { detail.render(); detail.bindEvents(); }
        }
      } catch (err) {
