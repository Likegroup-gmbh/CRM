...
export class MarkeList extends BasePaginatedList {
   ...
   */
  getEmptyState() {
    const canEdit = window.isAdmin?.() || window.currentUser?.permissions?.marke?.can_edit;
    return {
      icon: 'tag',
      title: 'Keine Marken vorhanden',
      text: canEdit
        ? 'Legen Sie Ihre erste Marke an, um loszulegen.'
        : 'Es sind noch keine Marken für Sie freigegeben.',
      actionsHtml: canEdit ? '<button id="btn-marke-new" class="mdc-btn">Neue Marke anlegen</button>' : ''
    };
  }
  ...
  async loadPageData(page, limit, filters) {
    try {
      ...
      if (filtersWithoutBranche.name) {
        ...
        const orParts = [`markenname.ilike.%${search}%`, `webseite.ilike.%${search}%`];
  ...
  renderSingleRow(marke) {
    ...
    return `
      <tr data-id="${marke.id}">
        ...
        <td class="col-webseite table-cell-center">${marke.webseite ? `<a href="${safeExternalUrl(marke.webseite)}" target="_blank" rel="noopener noreferrer" class="external-link-btn" title="${marke.webseite}">${icon('external-link')}</a>` : '-'}</td>
    ...
    return `
      <div class="table-filter-wrapper">
        ...
        <div class="table-actions">
          ...
          ${canEdit ? '<button id="btn-marke-new" class="mdc-btn">Neue Marke anlegen</button>' : ''}
        </div>
      </div>
