// PersonaFolderRenderer.js
// Grid-Rendering für Personas: Unternehmen → Marken → Items.
...
import { NUR_UNTERNEHMEN_LABEL } from './PersonaFolders.js';

function canCreatePersona() {
  return !window.isKunde?.() && (window.isAdmin?.() || window.currentUser?.permissions?.persona?.can_edit);
}

export function personaCreateButtonHtml() {
  return canCreatePersona()
    ? '<button id="btn-persona-new" class="mdc-btn">Persona anlegen</button>'
    : '';
}
...
function folderToolbar({ backId, listViewMode }) {
  return `
    <div class="table-filter-wrapper">
      ...
      <div class="table-actions">${personaCreateButtonHtml()}</div>
    </div>
  `;
}
...
function emptyFoldersHtml({ title, text }) {
  return `<div class="grid-span-all">${renderEmptyState({
    ...
    title,
    text,
    actionsHtml: personaCreateButtonHtml()
  })}</div>`;
}
...
export function updateBrandsGrid(list) {
  ...
  if (!list.brandFolders.length) {
    grid.innerHTML = emptyFoldersHtml({
      title: 'Keine Marken mit Personas',
      text: 'Für dieses Unternehmen gibt es noch keine Personas.'
    });
    return;
  }
...
export function renderItemsView(list) {
  return `
    <div class="list-container">
      ...
      <div class="pagination-container" id="pagination-persona-items"></div>
    </div>
  `;
}
...
export function updateItemsTable(list) {
  ...
  if (!list.currentItems.length) {
    const html = resolveEmptyState({
      ...
      states: {
        default: {
          icon: 'users',
          title: 'Keine Personas für diese Marke vorhanden',
          text: emptyCreateText(),
          actionsHtml: personaCreateButtonHtml()
        }
      }
  ...
  tbody.innerHTML = pageItems.map((persona) => list.renderSingleRow(persona, { checkbox: false })).join('');
