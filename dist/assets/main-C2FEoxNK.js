...
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
    ...
    `}handleAction(e,t,n,a){switch(console.log(`🎯 ACTIONSDROPDOWN: Action ${e} für ${n} ${t}`),e){case"view":window.navigateTo(`/${n}/${t}`);break;case"edit":window.navigateTo(`/${n}/${t}/edit`);break;case"delete":this.confirmDelete(t,n);break;case"notiz":this.openNotizModal(t,n);break;case"rating":this.openRatingModal(t,n);break;case"marken":window.navigateTo(`/unternehmen/${t}/marken`);break;case"auftraege":window.navigateTo(`/unternehmen/${t}/auftraege`);break;case"kampagnen":window.navigateTo(`/auftrag/${t}/kampagnen`);break;case"quickview":this.openKooperationQuickView(t);break;case"assign-staff":if(window.currentUser?.rolle!=="admin"){alert("Nur Admins dürfen Mitarbeiter zuordnen.");break}this.openAssignStaffModal(t);break;case"rechnung":this.openRechnungModal(t,n);break;case"invoice-view":window.navigateTo(`/rechnung/${t}`);break;case"invoice-edit":window.navigateTo(`/rechnung/${t}/edit`);break;case"add_to_campaign":this.openAddToCampaignModal(t);break;case"favorite":{const r=document.querySelector("[data-kampagne-id]")?.dataset?.kampagneId;this.addToFavorites(t,r);break}case"add_to_list":{this.openAddToListModal(t);break}case"unassign-kampagne":{const r=a?.dataset?.mitarbeiterId||window.location.pathname.split("/").pop();if(!r){alert("Mitarbeiter-ID nicht gefunden");break}const i=t;if(!i||!confirm("Zuweisung dieser Kampagne vom Mitarbeiter entfernen?"))break;(async()=>{try{const{error:s}=await window.supabase.from("kampagne_mitarbeiter").delete().eq("mitarbeiter_id",r).eq("kampagne_id",i);if(s)throw s;const o=a.closest("tr");o&&o.remove();const d=document.querySelector('.tab-button[data-tab="kampagnen"] .tab-count');if(d){const c=parseInt(d.textContent||"1",10);d.textContent=String(Math.max(0,c-1))}alert("Zuweisung entfernt")}catch(s){console.error("❌ Zuweisung entfernen fehlgeschlagen",s),alert("Entfernen fehlgeschlagen")}})();break}case"video-view":window.navigateTo(`/video/${t}`);break;case"video-edit":window.navigateTo(`/video/${t}`);break;case"video-delete":this.confirmDelete(t,"kooperation_videos");break;default:console.warn(`⚠️ Unbekannte Action: ${e}`)}}async openKooperationQuickView(e){try{const t=document.createElement("div");t.className="drawer-overlay";const n=document.createElement("div");n.setAttribute("role","dialog"),n.className="drawer-panel";const a=document.createElement("div");a.className="drawer-header";const r=document.createElement("div"),i=document.createElement("h1");i.textContent="Kooperation · Schnellansicht";const s=document.createElement("p");s.style.margin="0",s.style.color="#6b7280",s.textContent="Videos & Kommentare",r.appendChild(i),r.appendChild(s);const o=document.createElement("div"),d=document.createElement("button");d.className="drawer-close",d.id="kvq-close",d.textContent="Schließen",o.appendChild(d),a.appendChild(r),a.appendChild(o);const c=document.createElement("div");c.className="drawer-body";const l=document.createElement("div");l.className="detail-section";const u=document.createElement("h2");u.textContent="Videos";const m=document.createElement("div");m.id="kvq-table",m.textContent="Lade...",l.appendChild(u),l.appendChild(m),c.replaceChildren(l),n.appendChild(a),n.appendChild(c),document.body.appendChild(t),document.body.appendChild(n);const h=()=>{try{t.remove(),n.remove()}catch{}};t.addEventListener("click",h),a.querySelector("#kvq-close")?.addEventListener("click",h),document.addEventListener("keydown",function y(L){L.key==="Escape"&&(h(),document.removeEventListener("keydown",y))}),requestAnimationFrame(()=>{n.classList.add("show")});const{data:g}=await window.supabase.from("kooperation_videos").select("id, position, content_art, titel, asset_url, status, created_at").eq("kooperation_id",e).order("position",{ascending:!0}),b=g||[];let v={};if(b.length){const y=b.map(D=>D.id),{data:L}=await window.supabase.from("kooperation_video_comment").select("id, video_id, runde, text, author_name, created_at").in("video_id",y).order("created_at",{ascending:!0});(L||[]).forEach(D=>{const C=D.video_id;v[C]||(v[C]={r1:[],r2:[]});const I=D.runde===2||D.runde==="2"?"r2":"r1";v[C][I].push(D)})}const w=y=>window.validatorSystem?.sanitizeHtml?.(y)??y,f=y=>y?new Date(y).toLocaleDateString("de-DE"):"-",_=y=>!y||!y.length?"-":y.map(L=>{const D=w(L.text||""),C=w(L.author_name||"-"),I=f(L.created_at);return`<div class="fb-line"><span class="fb-meta">${C} • ${I}</span><div class="fb-text">${D}</div></div>`}).join(""),E=b.map(y=>{const L=v[y.id]||{r1:[],r2:[]},D=y.asset_url?`<a class="kvq-link-btn" href="${y.asset_url}" target="_blank" rel="noopener">${this.getHeroIcon("view")}<span>Öffnen</span></a>`:"-";return`
          <tr>
            <td>${y.position||"-"}</td>
            <td>${w(y.content_art||"-")}</td>
            <td>
              <div class="kvq-cell">
                <span class="kvq-title-text">${w(y.titel||"-")}</span>
                ${D}
              </div>
            </td>
            <td class="feedback-cell">${_(L.r1)}</td>
            <td class="feedback-cell">${_(L.r2)}</td>
            <td><span class="status-badge status-${(y.status||"produktion").toLowerCase()}">${y.status==="abgeschlossen"?"Abgeschlossen":"Produktion"}</span></td>
          </tr>`}).join(""),$=b.length?`
        ...
        </div>`:'<p class="empty-state">Keine Videos vorhanden.</p>',A=c.querySelector("#kvq-table");if(A.innerHTML="",b.length){const y=document.createElement("div");y.className="data-table-container",y.innerHTML=$,A.appendChild(y)}else{const y=document.createElement("p");y.className="empty-state",y.textContent="Keine Videos vorhanden.",A.appendChild(y)}this.normalizeIcons(c)}catch(t){console.error("❌ Quickview öffnen fehlgeschlagen",t),alert("Schnellansicht konnte nicht geöffnet werden.")}}async openAssignStaffModal(e){const t=document.createElement("div");t.className="modal overlay-modal",t.innerHTML=`
      ...
      </div>`,document.body.appendChild(t);const n=t.querySelector("#staff-search"),a=t.querySelector("#staff-dropdown");let r=null;const i=async l=>{try{let u=[];try{const{data:g}=await window.supabase.from("kampagne_mitarbeiter").select("mitarbeiter_id").eq("kampagne_id",e);u=(g||[]).map(b=>b.mitarbeiter_id)}catch{}let m=window.supabase.from("benutzer").select("id, name, rolle, mitarbeiter_klasse:mitarbeiter_klasse_id(name)").in("rolle",["mitarbeiter","Mitarbeiter","admin"]).order("name");l&&(m=m.ilike("name",`%${l}%`)),u.length>0&&(m=m.not("id","in",`(${u.join(",")})`));const{data:h}=await m;return h||[]}catch(u){return console.warn("⚠️ Mitarbeiter-Suche fehlgeschlagen",u),[]}},s=l=>{a.innerHTML=l.length?l.map(u=>`<div class="dropdown-item" data-id="${u.id}">${u.name}${u.mitarbeiter_klasse?.name?` <span class="muted">(${u.mitarbeiter_klasse.name})</span>`:""}${u.rolle?` <span class="muted">[${u.rolle}]</span>`:""}</div>`).join(""):'<div class="dropdown-item no-results">Keine Mitarbeiter gefunden</div>'};s(await i("")),a.classList.add("show"),n.focus(),(()=>{(!a.style.position||a.style.position==="absolute")&&(a.style.position="relative"),a.style.display="block"})(),n.addEventListener("focus",()=>a.classList.add("show")),n.addEventListener("blur",()=>setTimeout(()=>a.classList.remove("show"),150));let d;n.addEventListener("input",()=>{clearTimeout(d),d=setTimeout(async()=>s(await i(n.value.trim())),200)}),a.addEventListener("click",l=>{const u=l.target.closest(".dropdown-item");!u||u.classList.contains("no-results")||(r=u.dataset.id,n.value=u.textContent.trim(),t.querySelector("#assign-staff-confirm").disabled=!1,a.classList.remove("show"))});const c=()=>t.remove();t.querySelector("#assign-staff-close").onclick=c,t.querySelector("#assign-staff-cancel").onclick=c,t.querySelector("#assign-staff-confirm").onclick=async()=>{if(r)try{await window.supabase.from("kampagne_mitarbeiter").insert({kampagne_id:e,mitarbeiter_id:r});try{const{data:l}=await window.supabase.from("kampagne").select("id, kampagnenname").eq("id",e).single(),u=l?.kampagnenname||e;await window.notificationSystem?.pushNotification(r,{type:"assign",entity:"kampagne",entityId:e,title:"Neue Kampagnen-Zuweisung",message:`Du wurdest der Kampagne "${u}" zugeordnet.`}),window.dispatchEvent(new Event("notificationsRefresh"))}catch{}c(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:"kampagne",action:"staff-assigned",id:e}})),alert("Mitarbeiter zugeordnet.")}catch(l){console.error("❌ Fehler beim Zuordnen",l),alert("Zuordnung fehlgeschlagen.")}}}async addToFavorites(e,t){try{if(!t){const n=window.location.pathname.match(/\/kampagne\/([0-9a-fA-F-]{36})/);t=n?n[1]:null}if(!t){alert("Kampagne konnte nicht ermittelt werden.");return}await window.supabase.from("kampagne_creator_favoriten").insert({kampagne_id:t,creator_id:e}),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:"kampagne",action:"favorite-added",id:t}})),alert("Zu Favoriten hinzugefügt.")}catch(n){console.error("❌ Fehler beim Hinzufügen zu Favoriten",n),alert("Hinzufügen zu Favoriten fehlgeschlagen.")}}async openAddToCampaignModal(e){let t=[],n=[],a=null;try{const[u,m]=await Promise.all([window.supabase.from("kampagne_creator").select("kampagne_id").eq("creator_id",e),window.supabase.from("kampagne_creator_sourcing").select("kampagne_id").eq("creator_id",e)]),h=(u?.data||[]).map(w=>w.kampagne_id).filter(Boolean),g=(m?.data||[]).map(w=>w.kampagne_id).filter(Boolean);if(n=Array.from(new Set([...h,...g])),window.currentUser?.rolle!=="admin"){const{data:w}=await window.supabase.from("kampagne_mitarbeiter").select("kampagne_id").eq("mitarbeiter_id",window.currentUser?.id);a=(w||[]).map(f=>f.kampagne_id).filter(Boolean)}let b=!0,v=window.supabase.from("kampagne").select("id, kampagnenname, status").order("created_at",{ascending:!1});if(Array.isArray(a)&&(a.length===0?(b=!1,t=[]):v=v.in("id",a)),n.length>0&&(v=v.not("id","in",`(${n.join(",")})`)),b){const{data:w}=await v;t=w||[]}}catch{}const r=document.createElement("div");r.className="modal overlay-modal",r.innerHTML=`
      <div class="modal-dialog">
        <div class="modal-header">
      ...
      </div>`,document.body.appendChild(r);const i=r.querySelector("#campaign-search"),s=r.querySelector("#campaign-dropdown");let o=null;const d=(u="")=>{const m=u.toLowerCase(),h=t.filter(g=>(g.kampagnenname||"").toLowerCase().includes(m));s.innerHTML=h.length?h.map(g=>`<div class="dropdown-item" data-id="${g.id}">${g.kampagnenname}</div>`).join(""):'<div class="dropdown-item no-results">Keine Kampagne gefunden</div>'};d(""),i.addEventListener("focus",()=>{s.classList.add("show")}),i.addEventListener("blur",()=>{setTimeout(()=>s.classList.remove("show"),150)});let c;i.addEventListener("input",()=>{const u=i.value.trim();clearTimeout(c),c=setTimeout(async()=>{try{let m=window.supabase.from("kampagne").select("id, kampagnenname, status").order("created_at",{ascending:!1});if(u.length>0&&(m=m.ilike("kampagnenname",`%${u}%`)),Array.isArray(a)){if(a.length===0){t=[],d(u);return}m=m.in("id",a)}n.length>0&&(m=m.not("id","in",`(${n.join(",")})`));const{data:h}=await m;t=h||[],d(u)}catch(m){console.warn("⚠️ Kampagnen-Suche fehlgeschlagen",m)}},200)}),s.addEventListener("click",u=>{const m=u.target.closest(".dropdown-item");!m||m.classList.contains("no-results")||(o=m.dataset.id,i.value=m.textContent,r.querySelector("#add-to-campaign-confirm").disabled=!1,s.classList.remove("show"))});const l=()=>r.remove();r.querySelector("#add-to-campaign-close").onclick=l,r.querySelector("#add-to-campaign-cancel").onclick=l,r.querySelector("#add-to-campaign-confirm").onclick=async()=>{if(o)try{await window.supabase.from("kampagne_creator_sourcing").insert({kampagne_id:o,creator_id:e});try{const[{data:u},{data:m}]=await Promise.all([window.supabase.from("kampagne_mitarbeiter").select("mitarbeiter_id").eq("kampagne_id",o),window.supabase.from("kampagne").select("kampagnenname").eq("id",o).single()]),h=m?.kampagnenname||o,g=(u||[]).map(b=>b.mitarbeiter_id).filter(Boolean);for(const b of g)await window.notificationSystem?.pushNotification(b,{type:"update",entity:"kampagne",entityId:o,title:"Sourcing-Update",message:`Neuer Creator wurde dem Sourcing von "${h}" hinzugefügt.`});g.length&&window.dispatchEvent(new Event("notificationsRefresh"))}catch{}l(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:"kampagne",action:"sourcing-added",id:o}})),alert("Creator wurde zum Sourcing der Kampagne hinzugefügt.")}catch(u){console.error("❌ Fehler beim Hinzufügen zur Kampagne",u),alert("Hinzufügen fehlgeschlagen.")}}}async openAddToListModal(e){let t=[],n=[];try{const{data:l}=await window.supabase.from("creator_list_member").select("list_id").eq("creator_id",e);n=(l||[]).map(m=>m.list_id).filter(Boolean);const{data:u}=await window.supabase.from("creator_list").select("id, name, created_at").order("created_at",{ascending:!1});t=(u||[]).filter(m=>!n.includes(m.id))}catch{}const a=document.createElement("div");a.className="modal overlay-modal",a.innerHTML=`
      <div class="modal-dialog">
        <div class="modal-header">
    ...
    `).join(""),z.normalizeIcons(document)}destroy(){}}const $e=new Ee;function Se(p,e={}){const{showActions:t=!1}=e,n=i=>i?new Date(i).toLocaleDateString("de-DE"):"-",a=i=>Array.isArray(i)&&i.length>0?i.join(", "):"-",r=(p||[]).map(i=>{const s=i.id,o=i.kampagnenname||"Unbekannt",d=i.unternehmen?.firmenname||"-",c=i.marke?.markenname||"-",l=a(i.art_der_kampagne),u=i.status||"-",m=n(i.start),h=n(i.deadline),g=i.creatoranzahl??"-",b=i.videoanzahl??"-";return`
      <tr data-id="${s||""}">
        <td>
  ...
  `}class Ae{constructor(){this.creatorId=null,this.creator=null,this.notizen=[],this.ratings=[],this.kampagnen=[],this.lists=[],this.kooperationen=[],this.rechnungen=[]}async init(e){if(console.log("🎯 CREATORDETAIL: Initialisiere Creator-Detailseite für ID:",e),this.creatorId=e,window.moduleRegistry?.currentModule!==this){console.log("⚠️ CREATORDETAIL: Nicht mehr das aktuelle Modul, breche ab");return}try{await this.loadCreatorData(),await this.render(),this.bindEvents(),console.log("✅ CREATORDETAIL: Initialisierung abgeschlossen")}catch(t){console.error("❌ CREATORDETAIL: Fehler bei der Initialisierung:",t),window.ErrorHandler.handle(t,"CreatorDetail.init")}}async loadCreatorData(){console.log("🔄 CREATORDETAIL: Lade Creator-Daten...");const{data:e,error:t}=await window.supabase.from("creator").select("*").eq("id",this.creatorId).single();if(t)throw new Error(`Fehler beim Laden der Creator-Daten: ${t.message}`);this.creator=e,console.log("✅ CREATORDETAIL: Creator-Basisdaten geladen:",e);try{const{data:s}=await window.supabase.from("creator_sprachen").select("sprache_id, sprachen:sprache_id(id, name)").eq("creator_id",this.creatorId);this.creator.sprachen=(s||[]).map(o=>o.sprachen).filter(Boolean)}catch{}try{const{data:s}=await window.supabase.from("creator_branchen").select("branche_id, branchen_creator:branche_id(id, name)").eq("creator_id",this.creatorId);this.creator.branchen=(s||[]).map(o=>o.branchen_creator).filter(Boolean)}catch{}try{const{data:s}=await window.supabase.from("creator_creator_type").select("creator_type_id, creator_type:creator_type_id(id, name)").eq("creator_id",this.creatorId);this.creator.creator_types=(s||[]).map(o=>o.creator_type).filter(Boolean)}catch{}this.notizen=await window.notizenSystem.loadNotizen("creator",this.creatorId),console.log("✅ CREATORDETAIL: Notizen geladen:",this.notizen.length),this.ratings=await window.bewertungsSystem.loadBewertungen("creator",this.creatorId),console.log("✅ CREATORDETAIL: Ratings geladen:",this.ratings.length);const{data:n,error:a}=await window.supabase.from("kampagne_creator").select(`
      ...
      `).eq("creator_id",this.creatorId).order("hinzugefuegt_am",{ascending:!1});a||(this.kampagnen=n||[]),console.log("✅ CREATORDETAIL: Kampagnen geladen:",this.kampagnen.length);try{const{data:s}=await window.supabase.from("kooperationen").select(`
          ...
          created_at
        `).eq("creator_id",this.creatorId).order("created_at",{ascending:!1});this.kooperationen=s||[]}catch(s){console.warn("⚠️ CREATORDETAIL: Kooperationen konnten nicht geladen werden",s),this.kooperationen=[]}console.log("✅ CREATORDETAIL: Kooperationen geladen:",this.kooperationen.length);try{const s=(this.kooperationen||[]).map(o=>o.id).filter(Boolean);if(s.length>0){const{data:o}=await window.supabase.from("rechnung").select("id, rechnung_nr, status, nettobetrag, bruttobetrag, gestellt_am, bezahlt_am, pdf_url, kooperation_id").in("kooperation_id",s).order("gestellt_am",{ascending:!1});this.rechnungen=o||[]}else this.rechnungen=[]}catch{this.rechnungen=[]}try{const s=(this.kooperationen||[]).filter(l=>l.kampagne).map(l=>({kampagne:{id:l.kampagne.id,kampagnenname:l.kampagne.kampagnenname,status:l.status||null,start:null,deadline:null,unternehmen:null,marke:null},hinzugefuegt_am:l.created_at||null,notiz:null})),o=[...this.kampagnen||[],...s],d=new Set;this.kampagnen=o.filter(l=>{const u=l?.kampagne?.id||l?.kampagne_id||l?.kampagne?.kampagnenname;return u?d.has(u)?!1:(d.add(u),!0):!0});const c=Array.from(new Set(this.kampagnen.map(l=>l?.kampagne?.id||l?.kampagne_id).filter(Boolean)));if(c.length>0){const{data:l}=await window.supabase.from("kampagne").select("id, unternehmen:unternehmen_id ( firmenname ), marke:marke_id ( markenname )").in("id",c),u=(l||[]).reduce((m,h)=>(m[h.id]=h,m),{});this.kampagnen=this.kampagnen.map(m=>{const h=m?.kampagne?.id||m?.kampagne_id,g=h?u[h]:null;return g&&(m.kampagne||(m.kampagne={id:h}),m.kampagne.unternehmen||(m.kampagne.unternehmen=g.unternehmen||null),m.kampagne.marke||(m.kampagne.marke=g.marke||null)),m})}}catch(s){console.warn("⚠️ CREATORDETAIL: Kampagnen-Merge aus Kooperationen fehlgeschlagen",s)}const{data:r,error:i}=await window.supabase.from("creator_list_member").select(`
        *,
        list:list_id (
          id,
          name,
    ...
    `;window.setContentSafely(window.content,e)}renderTagList(e){if(!e||e.length===0)return"-";if(Array.isArray(e))return`<div class="tags">${e.map(n=>{const a=typeof n=="object"&&(n.name||n.label)||n;return`<span class="tag">${String(a).trim()}</span>`}).join("")}</div>`;if(typeof e=="object"){const t=e.name||e.label;return t?`<div class="tags"><span class="tag">${t}</span></div>`:"-"}return`<div class="tags"><span class="tag">${String(e)}</span></div>`}renderNotizen(){return window.notizenSystem.renderNotizenContainer(this.notizen,"creator",this.creatorId)}renderRatings(){return window.bewertungsSystem.renderBewertungenContainer(this.ratings,"creator",this.creatorId)}renderKampagnen(){if(!this.kampagnen||this.kampagnen.length===0)return`
      ...
      `;const e=this.kampagnen.map(t=>{const n=t.kampagne||t;return{id:n.id,kampagnenname:n.kampagnenname,unternehmen:n.unternehmen||null,marke:n.marke||null,art_der_kampagne:n.art_der_kampagne,status:n.status,start:n.start,deadline:n.deadline,creatoranzahl:n.creatoranzahl,videoanzahl:n.videoanzahl}});return Se(e,{showActions:!1})}renderLists(){return this.lists.length===0?`
        <div class="empty-state">
    ...
    `}destroy(){}}const Le=new ze;class Fe{constructor(){this.selectedUnternehmen=new Set,this._boundEventListeners=new Set}async init(){if(window.setHeadline("Unternehmen Übersicht"),!(window.canViewPage&&window.canViewPage("unternehmen")||await window.checkUserPermission("unternehmen","can_view"))){window.content.innerHTML=`
        <div class="error-message">
          <p>Sie haben keine Berechtigung, Unternehmen anzuzeigen.</p>
        </div>
      `;return}this.bindEvents(),await this.loadAndRender()}async loadAndRender(){try{const e=await window.dataService.loadFilterData("unternehmen");await this.render(e);const t=k.getFilters("unternehmen");console.log("🔍 Lade Unternehmen mit Filter:",t);const n=await window.dataService.loadEntities("unternehmen",t);console.log("📊 Unternehmen geladen:",n?.length||0),this.updateTable(n)}catch(e){window.ErrorHandler.handle(e,"UnternehmenList.loadAndRender")}}async render(e){const t=window.currentUser?.permissions?.unternehmen?.can_edit||!1,n=k.getFilters("unternehmen");Object.entries(n).forEach(([i,s])=>{});let a=`<div class="filter-bar">
    ...
    `;window.setContentSafely(window.content,r),await this.initializeFilterBar()}async initializeFilterBar(){const e=document.getElementById("filter-container");e&&await k.renderFilterBar("unternehmen",e,t=>this.onFiltersApplied(t),()=>this.onFiltersReset())}onFiltersApplied(e){console.log("Filter angewendet:",e),k.applyFilters("unternehmen",e),this.loadAndRender()}onFiltersReset(){console.log("Filter zurückgesetzt"),k.resetFilters("unternehmen"),this.loadAndRender()}bindEvents(){this.boundFilterResetHandler=e=>{e.target.id==="btn-filter-reset"&&this.onFiltersReset()},document.addEventListener("click",this.boundFilterResetHandler),document.addEventListener("click",e=>{(e.target.id==="btn-unternehmen-new"||e.target.id==="btn-unternehmen-new-filter")&&(e.preventDefault(),window.navigateTo("/unternehmen/new"))}),document.addEventListener("click",e=>{if(e.target.classList.contains("table-link")&&e.target.dataset.table==="unternehmen"){e.preventDefault();const t=e.target.dataset.id;console.log("🎯 UNTERNEHMENLIST: Navigiere zu Unternehmen Details:",t),window.navigateTo(`/unternehmen/${t}`)}}),document.addEventListener("click",e=>{if(e.target.id==="btn-select-all"){e.preventDefault(),document.querySelectorAll(".unternehmen-check").forEach(a=>{a.checked=!0,a.dataset.id&&this.selectedUnternehmen.add(a.dataset.id)});const n=document.getElementById("select-all-unternehmen");n&&(n.indeterminate=!1,n.checked=!0),this.updateSelection()}}),document.addEventListener("click",e=>{e.target.id==="btn-deselect-all"&&(e.preventDefault(),this.deselectAll())}),window.addEventListener("entityUpdated",e=>{e.detail.entity==="unternehmen"&&this.loadAndRender()}),document.addEventListener("click",e=>{if(e.target.classList.contains("tag-x")){e.preventDefault(),e.stopPropagation();const n=e.target.closest(".filter-tag").dataset.key,a=k.getFilters("unternehmen");delete a[n],k.applyFilters("unternehmen",a),this.loadAndRender()}}),document.addEventListener("change",e=>{if(e.target.id==="select-all-unternehmen"){const t=document.querySelectorAll(".unternehmen-check"),n=e.target.checked;t.forEach(a=>{a.checked=n,n?this.selectedUnternehmen.add(a.dataset.id):this.selectedUnternehmen.delete(a.dataset.id)}),this.updateSelection(),console.log(`${n?"✅ Alle Unternehmen ausgewählt":"❌ Alle Unternehmen abgewählt"}: ${this.selectedUnternehmen.size}`)}}),document.addEventListener("change",e=>{e.target.classList.contains("unternehmen-check")&&(e.target.checked?this.selectedUnternehmen.add(e.target.dataset.id):this.selectedUnternehmen.delete(e.target.dataset.id),this.updateSelection(),this.updateSelectAllCheckbox())}),window.bulkActionSystem&&window.bulkActionSystem.registerList("unternehmen",this)}hasActiveFilters(){const e=k.getFilters("unternehmen");return Object.keys(e).length>0}updateSelection(){const e=this.selectedUnternehmen.size,t=document.getElementById("selected-count"),n=document.getElementById("btn-deselect-all"),a=document.getElementById("btn-delete-selected");t&&(t.textContent=`${e} ausgewählt`,t.style.display=e>0?"inline":"none"),n&&(n.style.display=e>0?"inline-block":"none"),a&&(a.style.display=e>0?"inline-block":"none")}async updateTable(e){const t=document.querySelector(".data-table tbody");if(!t)return;if(!e||e.length===0){const{renderEmptyState:i}=await x(async()=>{const{renderEmptyState:s}=await import("./core-C7Vz5Umf.js").then(o=>o.F);return{renderEmptyState:s}},[]);i(t);return}const n=e.map(i=>i.id).filter(Boolean),a=await this.loadAnsprechpartnerMap(n),r=e.map(i=>`
      <tr data-id="${i.id}">
        ...
        <td>${this.renderBrancheTags(i.branchen??i.branche)}</td>
        <td>${this.renderAnsprechpartnerList(a.get(i.id))}</td>
        <td>${window.validatorSystem.sanitizeHtml(i.telefonnummer||"")}</td>
    ...
    `).join("");t.innerHTML=r}renderBrancheTags(e){if(!e||Array.isArray(e)&&e.length===0)return"-";if(typeof e=="string"){const t=e.split(",").map(a=>a.trim()).filter(Boolean);return t.length===0?"-":`<div class="tags tags-compact">${t.map(a=>`<span class="tag tag--branche">${window.validatorSystem.sanitizeHtml(a)}</span>`).join("")}</div>`}if(Array.isArray(e))return`<div class="tags tags-compact">${e.map(n=>{const a=typeof n=="object"&&(n.name||n.label)||n;return`<span class="tag tag--branche">${window.validatorSystem.sanitizeHtml(String(a).trim())}</span>`}).join("")}</div>`;if(typeof e=="object"){const t=e.name||e.label;return t?`<div class="tags tags-compact"><span class="tag tag--branche">${window.validatorSystem.sanitizeHtml(t)}</span></div>`:"-"}return"-"}async loadAnsprechpartnerMap(e){const t=new Map;try{if(!window.supabase||!Array.isArray(e)||e.length===0)return t;const{data:n,error:a}=await window.supabase.from("ansprechpartner").select("id, vorname, nachname, unternehmen_id").in("unternehmen_id",e);if(a)return console.warn("⚠️ Konnte Ansprechpartner nicht laden:",a),t;(n||[]).forEach(r=>{const i=t.get(r.unternehmen_id)||[];i.push(r),t.set(r.unternehmen_id,i)})}catch(n){console.warn("⚠️ loadAnsprechpartnerMap Fehler:",n)}return t}renderAnsprechpartnerList(e){if(!e||e.length===0)return"-";const t=e.slice(0,3).map(a=>{const r=`${a.vorname||""} ${a.nachname||""}`.trim();return`<span class="tag">${window.validatorSystem.sanitizeHtml(r||"-")}</span>`}).join(""),n=e.length>3?`<span class="tag">+${e.length-3}</span>`:"";return`<div class="tags tags-compact">${t}${n}</div>`}destroy(){console.log("UnternehmenList: Cleaning up..."),this._boundEventListeners.forEach(({element:e,type:t,handler:n})=>{e.removeEventListener(t,n)}),this._boundEventListeners.clear(),this.boundFilterResetHandler&&(document.removeEventListener("click",this.boundFilterResetHandler),this.boundFilterResetHandler=null)}showCreateForm(){console.log("🎯 Zeige Unternehmen-Erstellungsformular"),window.setHeadline("Neues Unternehmen anlegen");const e=window.formSystem.renderFormOnly("unternehmen");window.content.innerHTML=`
      <div class="page-header">
        <div class="page-header-left">
...
`)}`)),alert(i),this.deselectAll(),await this.loadAndRender(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:"unternehmen",action:"bulk-deleted",count:n}}))}}const j=new Fe;class De{constructor(){this.formData={},this.selectedBranches=[],this.allBranches=[]}async init(){if(console.log("🎯 UNTERNEHMENCREATE: Initialisiere Unternehmen-Erstellungsseite mit FormSystem"),window.moduleRegistry?.currentModule!==this){console.log("⚠️ UNTERNEHMENCREATE: Nicht mehr das aktuelle Modul, breche ab");return}try{this.showCreateForm(),console.log("✅ UNTERNEHMENCREATE: Initialisierung abgeschlossen")}catch(e){console.error("❌ UNTERNEHMENCREATE: Fehler bei der Initialisierung:",e),window.ErrorHandler.handle(e,"UnternehmenCreate.init")}}async loadBranches(){console.log("🔄 UNTERNEHMENCREATE: Lade Branchen...");const{data:e,error:t}=await window.supabase.from("branchen").select("*").order("name");if(t)throw new Error(`Fehler beim Laden der Branchen: ${t.message}`);this.allBranches=e,console.log("✅ UNTERNEHMENCREATE: Branchen geladen:",e.length)}render(){window.setHeadline("Neues Unternehmen anlegen"),window.setContentSafely(`
    ...
    `;const n=document.querySelector(".form-page");n&&n.insertBefore(t,n.firstChild)}showErrorMessage(e){const t=document.createElement("div");t.className="alert alert-danger",t.textContent=e,t.style.cssText=`
      ...
      margin-bottom: 1rem;
      border: 1px solid #f5c6cb;
    `;const n=document.querySelector(".form-page");n&&n.insertBefore(t,n.firstChild)}}const G=new Me;class Be{constructor(){this.formData={}}async init(){console.log("🎯 MARKECREATE: Initialisiere Marke-Erstellung"),this.showCreateForm()}showCreateForm(){console.log("🎯 MARKECREATE: Zeige Marke-Erstellungsformular mit FormSystem"),window.setHeadline("Neue Marke anlegen");const e=window.formSystem.renderFormOnly("marke");window.content.innerHTML=`
      <div class="page-header">
        <div class="page-header-left">
    ...
    `,window.formSystem.bindFormEvents("marke",null);const t=document.getElementById("marke-form");t&&(t.onsubmit=async n=>{n.preventDefault(),await this.handleFormSubmit()})}async handleFormSubmit(){try{console.log("🎯 MARKECREATE: Verarbeite Formular-Submit");const e=document.querySelector('#marke-form button[type="submit"]');let t="Marke anlegen";e&&(t=e.innerHTML,e.innerHTML='<div class="loading-spinner"></div> Wird angelegt...',e.disabled=!0);const n=document.getElementById("marke-form"),a=new FormData(n),r={},i={},s=n.querySelectorAll('select[data-tag-based="true"]');console.log("🏷️ Tag-basierte Selects gefunden:",s.length),s.forEach(c=>{let l=n.querySelector(`select[name="${c.name}"][style*="display: none"]`);if(!l){const u=n.querySelectorAll(`select[name="${c.name}"]`);u.length>1&&(l=u[1])}if(l){const u=Array.from(l.selectedOptions).map(m=>m.value).filter(m=>m!=="");u.length>0&&(i[c.name]=u,console.log(`🏷️ Tag-basiertes Multi-Select ${c.name}:`,u))}});for(let[c,l]of a.entries())if(!i.hasOwnProperty(c))if(c.includes("[]")){const u=c.replace("[]","");i[u]||(i[u]=[]),i[u].push(l)}else i[c]?(Array.isArray(i[c])||(i[c]=[i[c]]),i[c].push(l)):i[c]=l;for(let[c,l]of Object.entries(i))r[c]=Array.isArray(l)?l:l.trim();console.log("📤 Finale Marke-Daten:",r);const o=window.validatorSystem.validateForm(r,{markenname:{type:"text",minLength:2,required:!0}});if(!o.isValid){this.showValidationErrors(o.errors);return}const d=await window.dataService.createEntity("marke",r);if(d.success)this.showSuccessMessage("Marke erfolgreich erstellt!"),setTimeout(()=>{window.navigateTo("/marke")},1500);else throw new Error(d.error||"Fehler beim Erstellen der Marke")}catch(e){console.error("❌ Fehler beim Erstellen der Marke:",e),this.showErrorMessage(e.message||"Ein unerwarteter Fehler ist aufgetreten")}finally{const e=document.querySelector('#marke-form button[type="submit"]');e&&(e.innerHTML=originalText,e.disabled=!1)}}showValidationErrors(e){document.querySelectorAll(".field-error").forEach(t=>t.remove());for(const[t,n]of Object.entries(e)){const a=document.querySelector(`[name="${t}"]`);if(a){const r=document.createElement("div");r.className="field-error",r.textContent=n,r.style.color="red",r.style.fontSize="12px",r.style.marginTop="4px",a.parentNode.appendChild(r)}}}showSuccessMessage(e){const t=document.createElement("div");t.className="toast success",t.textContent=e,t.style.cssText=`
      position: fixed;
      top: 20px;
      right: 20px;
      background: #10b981;
    ...
    `,document.body.appendChild(t),setTimeout(()=>t.style.opacity="1",100),setTimeout(()=>{t.style.opacity="0",setTimeout(()=>t.remove(),300)},3e3)}showErrorMessage(e){const t=document.createElement("div");t.className="toast error",t.textContent=e,t.style.cssText=`
      ...
      opacity: 0;
      transition: opacity 0.3s ease;
    `,document.body.appendChild(t),setTimeout(()=>t.style.opacity="1",100),setTimeout(()=>{t.style.opacity="0",setTimeout(()=>t.remove(),300)},3e3)}destroy(){console.log("🎯 MARKECREATE: Destroy")}}const K=new Be;class Ce{constructor(){this.selectedMarken=new Set,this._boundEventListeners=new Set}async init(){if(console.log("🎯 MARKELLIST: Initialisiere Marken-Liste"),window.moduleRegistry?.currentModule!==this){console.log("⚠️ MARKELLIST: Nicht mehr das aktuelle Modul, breche ab");return}window.setHeadline("Marken Übersicht");const e=window.canViewPage&&window.canViewPage("marke")||await window.checkUserPermission("marke","can_view");if(console.log("🔐 MARKELLIST: Berechtigung für marke.can_view:",e),!e){console.log("❌ MARKELLIST: Keine Berechtigung für Marken"),window.content.innerHTML=`
        <div class="error-message">
          <p>Sie haben keine Berechtigung, Marken anzuzeigen.</p>
        </div>
      `;return}console.log("✅ MARKELLIST: Berechtigung OK, lade Marken..."),this.bindEvents(),await this.loadAndRender(),console.log("✅ MARKELLIST: Initialisierung abgeschlossen")}async loadAndRender(){try{const e=await window.dataService.loadFilterData("marke");await this.render(e);const t=k.getFilters("marke");console.log("🔍 Lade Marken mit Filter:",t);const n=await window.dataService.loadEntities("marke",t);console.log("📊 Marken geladen:",n?.length||0),this.updateTable(n)}catch(e){window.ErrorHandler.handle(e,"MarkeList.loadAndRender")}}async render(e){const t=window.currentUser?.permissions?.marke?.can_edit||!1,n=k.getFilters("marke");Object.entries(n).forEach(([i,s])=>{});let a=`<div class="filter-bar">
      <div class="filter-left">
        <div id="filter-container"></div>
      </div>
      <div class="filter-right">
    ...
    `;window.setContentSafely(window.content,r),await this.initializeFilterBar()}async initializeFilterBar(){const e=document.getElementById("filter-container");e&&await k.renderFilterBar("marke",e,t=>this.onFiltersApplied(t),()=>this.onFiltersReset())}onFiltersApplied(e){console.log("Filter angewendet:",e),k.applyFilters("marke",e),this.loadAndRender()}onFiltersReset(){console.log("Filter zurückgesetzt"),k.resetFilters("marke"),this.loadAndRender()}bindEvents(){this.boundFilterResetHandler=e=>{e.target.id==="btn-filter-reset"&&this.onFiltersReset()},document.addEventListener("click",this.boundFilterResetHandler),document.addEventListener("click",e=>{(e.target.id==="btn-marke-new"||e.target.id==="btn-marke-new-filter")&&(e.preventDefault(),window.navigateTo("/marke/new"))}),document.addEventListener("click",e=>{if(e.target.classList.contains("table-link")&&e.target.dataset.table==="marke"){e.preventDefault();const t=e.target.dataset.id;console.log("🎯 MARKELIST: Navigiere zu Marken Details:",t),window.navigateTo(`/marke/${t}`)}}),document.addEventListener("click",e=>{if(e.target.id==="btn-select-all"){e.preventDefault(),document.querySelectorAll(".marke-check").forEach(a=>{a.checked=!0,a.dataset.id&&this.selectedMarken.add(a.dataset.id)});const n=document.getElementById("select-all-marken");n&&(n.indeterminate=!1,n.checked=!0),this.updateSelection()}}),document.addEventListener("click",e=>{if(e.target.id==="btn-deselect-all"){e.preventDefault(),document.querySelectorAll(".marke-check").forEach(a=>{a.checked=!1}),this.selectedMarken.clear();const n=document.getElementById("select-all-marken");n&&(n.indeterminate=!1,n.checked=!1),this.updateSelection()}}),window.addEventListener("entityUpdated",e=>{e.detail.entity==="marke"&&this.loadAndRender()}),document.addEventListener("click",e=>{if(e.target.classList.contains("tag-x")){e.preventDefault(),e.stopPropagation();const n=e.target.closest(".filter-tag").dataset.key,a=k.getFilters("marke");delete a[n],k.applyFilters("marke",a),this.loadAndRender()}}),document.addEventListener("change",e=>{e.target.id==="select-all-marken"&&(document.querySelectorAll(".marke-check").forEach(n=>{n.checked=e.target.checked,e.target.checked?this.selectedMarken.add(n.dataset.id):this.selectedMarken.delete(n.dataset.id)}),this.updateSelection())}),document.addEventListener("change",e=>{e.target.classList.contains("marke-check")&&(e.target.checked?this.selectedMarken.add(e.target.dataset.id):this.selectedMarken.delete(e.target.dataset.id),this.updateSelection())})}hasActiveFilters(){const e=k.getFilters("marke");return Object.keys(e).length>0}updateSelection(){const e=this.selectedMarken.size,t=document.getElementById("selected-count"),n=document.getElementById("btn-deselect-all"),a=document.getElementById("btn-delete-selected");t&&(t.textContent=`${e} ausgewählt`,t.style.display=e>0?"inline":"none"),n&&(n.style.display=e>0?"inline-block":"none"),a&&(a.style.display=e>0?"inline-block":"none")}async updateTable(e){const t=document.querySelector(".data-table tbody");if(!t)return;if(!e||e.length===0){const{renderEmptyState:a}=await x(async()=>{const{renderEmptyState:r}=await import("./core-C7Vz5Umf.js").then(i=>i.F);return{renderEmptyState:r}},[]);a(t);return}const n=e.map(a=>`
      <tr data-id="${a.id}">
        ...
        <td>${window.validatorSystem.sanitizeHtml(a.branche||"")}</td>
        ...
        <td>${this.renderZustaendigkeit(a.zustaendigkeit)}</td>
        <td>
    ...
    `).join("");t.innerHTML=n}renderAnsprechpartner(e){return!e||e.length===0?"-":Array.isArray(e)?e.map(t=>`${t.vorname} ${t.nachname}`).join(", "):`${e.vorname} ${e.nachname}`}renderZustaendigkeit(e){return!e||e.length===0?"-":Array.isArray(e)?e.map(t=>`${t.mitarbeiter?.name||"Unbekannt"}`).join(", "):e.mitarbeiter?.name||"Unbekannt"}destroy(){console.log("🗑️ MARKELLIST: Destroy aufgerufen"),this._boundEventListeners.forEach(({element:e,type:t,handler:n})=>{e.removeEventListener(t,n)}),this._boundEventListeners.clear(),this.boundFilterResetHandler&&(document.removeEventListener("click",this.boundFilterResetHandler),this.boundFilterResetHandler=null),window.setContentSafely(""),console.log("✅ MARKELLIST: Destroy abgeschlossen")}showCreateForm(){console.log("🎯 Zeige Marken-Erstellungsformular mit MarkeCreate"),K.showCreateForm()}}const W=new Ce;class Te{constructor(){this.markeId=null,this.marke=null,this.notizen=[],this.ratings=[],this.kampagnen=[],this.auftraege=[],this.ansprechpartner=[],this.rechnungen=[]}async init(e){console.log("🎯 MARKENDETAIL: Initialisiere Marken-Detailseite für ID:",e);try{this.markeId=e,await this.loadMarkeData(),this.render(),this.bindEvents(),console.log("✅ MARKENDETAIL: Initialisierung abgeschlossen")}catch(t){console.error("❌ MARKENDETAIL: Fehler bei der Initialisierung:",t),window.ErrorHandler.handle(t,"MarkeDetail.init")}}async loadMarkeData(){console.log("🔄 MARKENDETAIL: Lade Marken-Daten...");try{const{data:e,error:t}=await window.supabase.from("marke").select(`
          *,
          unternehmen:unternehmen_id(firmenname)
        `).eq("id",this.markeId).single();if(t)throw t;this.marke=e,console.log("✅ MARKENDETAIL: Marken-Basisdaten geladen:",this.marke),window.notizenSystem&&(this.notizen=await window.notizenSystem.loadNotizen("marke",this.markeId),console.log("✅ MARKENDETAIL: Notizen geladen:",this.notizen.length)),window.bewertungsSystem&&(this.ratings=await window.bewertungsSystem.loadBewertungen("marke",this.markeId),console.log("✅ MARKENDETAIL: Ratings geladen:",this.ratings.length));const{data:n,error:a}=await window.supabase.from("kampagne").select("*").eq("marke_id",this.markeId);a||(this.kampagnen=n||[],console.log("✅ MARKENDETAIL: Kampagnen geladen:",this.kampagnen.length));const{data:r,error:i}=await window.supabase.from("auftrag").select("*").eq("marke_id",this.markeId);i||(this.auftraege=r||[],console.log("✅ MARKENDETAIL: Aufträge geladen:",this.auftraege.length));try{const d=(this.auftraege||[]).map(c=>c.id).filter(Boolean);if(d.length>0){const{data:c}=await window.supabase.from("rechnung").select("id, rechnung_nr, status, nettobetrag, bruttobetrag, gestellt_am, zahlungsziel, bezahlt_am, pdf_url, auftrag_id").in("auftrag_id",d);this.rechnungen=c||[]}else this.rechnungen=[]}catch{this.rechnungen=[]}const{data:s,error:o}=await window.supabase.from("ansprechpartner_marke").select(`
          ansprechpartner_id,
          ansprechpartner:ansprechpartner(*)
        `).eq("marke_id",this.markeId);o||(this.ansprechpartner=s?.map(d=>d.ansprechpartner)||[],console.log("✅ MARKENDETAIL: Ansprechpartner geladen:",this.ansprechpartner.length))}catch(e){throw console.error("❌ MARKENDETAIL: Fehler beim Laden der Marken-Daten:",e),e}}render(){window.setHeadline(`${this.marke?.markenname||"Marke"} - Details`);const e=`
      <div class="page-header">
        <div class="page-header-left">
    ...
    `}renderAnsprechpartner(){return!this.ansprechpartner||this.ansprechpartner.length===0?`
        <div class="empty-state">
          <div class="empty-icon">👥</div>
      ...
      <div class="ansprechpartner-card">
        ...
        <div class="ansprechpartner-details">
          ...
          <p><strong>Abteilung:</strong> ${t.abteilung||"-"}</p>
        </div>
      </div>
    `).join("")}
      </div>
    ...
    `}bindEvents(){document.addEventListener("click",e=>{if(e.target.classList.contains("tab-button")){const t=e.target.dataset.tab;this.switchTab(t)}}),document.addEventListener("click",e=>{e.target.id==="btn-edit-marke"&&this.showEditForm()}),document.addEventListener("notizenUpdated",()=>{this.loadMarkeData().then(()=>{this.render(),this.bindEvents()})}),document.addEventListener("bewertungenUpdated",()=>{this.loadMarkeData().then(()=>{this.render(),this.bindEvents()})})}switchTab(e){document.querySelectorAll(".tab-button").forEach(a=>{a.classList.remove("active")}),document.querySelectorAll(".tab-pane").forEach(a=>{a.classList.remove("active")});const t=document.querySelector(`[data-tab="${e}"]`),n=document.getElementById(e);t&&t.classList.add("active"),n&&n.classList.add("active")}showEditForm(){console.log("🎯 MARKENDETAIL: Zeige Bearbeitungsformular"),window.setHeadline("Marke bearbeiten");const e=window.formSystem.renderFormOnly("marke",this.marke);window.content.innerHTML=`
      <div class="page-header">
        <div class="page-header-left">
    ...
    `,window.formSystem.bindFormEvents("marke",this.marke);const t=document.getElementById("marke-form");t&&(t.onsubmit=async n=>{n.preventDefault(),await this.handleEditFormSubmit()})}async handleEditFormSubmit(){try{const e=document.getElementById("marke-form"),t=new FormData(e),n={};for(const[i,s]of t.entries())n[i]=s;const a=window.validatorSystem.validateForm(n,{markenname:{type:"text",minLength:2,required:!0}});if(!a.isValid){this.showValidationErrors(a.errors);return}const r=await window.dataService.updateEntity("marke",this.markeId,n);if(r.success)this.showSuccessMessage("Marke erfolgreich aktualisiert!"),setTimeout(async()=>{await this.loadMarkeData(),this.render(),this.bindEvents()},1500);else throw new Error(r.error||"Unbekannter Fehler")}catch(e){console.error("❌ Formular-Submit Fehler:",e),this.showErrorMessage(e.message)}}showValidationErrors(e){console.log("❌ Validierungsfehler:",e),document.querySelectorAll(".validation-error").forEach(t=>t.remove()),Object.keys(e).forEach(t=>{const n=document.querySelector(`[name="${t}"]`);if(n){const a=document.createElement("div");a.className="validation-error",a.textContent=e[t],a.style.color="#dc3545",a.style.fontSize="0.875rem",a.style.marginTop="0.25rem",n.parentNode.appendChild(a),n.style.borderColor="#dc3545"}})}showSuccessMessage(e){const t=document.createElement("div");t.className="alert alert-success",t.textContent=e,t.style.cssText=`
      background: #d4edda;
      color: #155724;
      padding: 1rem;
      border-radius: 4px;
    ...
    `;e.innerHTML=r,this.bindNavigationEvents()}bindNavigationEvents(){document.querySelectorAll(".nav-link").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault();const a=t.getAttribute("data-route");this.navigateTo(a)})})}navigateTo(e){console.log(`🧭 Navigation zu: ${e}`),this.currentRoute=e,this.updateActiveRoute(e),window.navigateTo?window.navigateTo(e):window.location.hash=e}updateActiveRoute(e){document.querySelectorAll(".nav-link").forEach(n=>{n.getAttribute("data-route")===e?n.classList.add("active"):n.classList.remove("active")})}getIcon(e){return{"icon-dashboard":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h7.5" /></svg>',"icon-users":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" /></svg>',"icon-building":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21m-3.75 3.75h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Z" /></svg>',"icon-tag":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z" /><path stroke-linecap="round" stroke-linejoin="round" d="M6 6h.008v.008H6V6z" /></svg>',"icon-briefcase":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg>',"icon-user-circle":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z" /></svg>',"icon-campaign":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.34 15.84c-.688-.06-1.386-.09-2.09-.09H7.5a4.5 4.5 0 1 1 0-9h.75c.704 0 1.402-.03 2.09-.09m0 9.18c.253.962.584 1.892.985 2.783.247.55.06 1.21-.463 1.511l-.657.38c-.551.318-1.26.117-1.527-.461a20.845 20.845 0 0 1-1.44-4.282m3.102.069a18.03 18.03 0 0 1-.59-4.59c0-1.586.205-3.124.59-4.59m0 9.18a23.848 23.848 0 0 1 8.835 2.535M10.34 6.66a23.847 23.847 0 0 0 8.835-2.535m0 0A23.74 23.74 0 0 0 18.795 3m.38 1.125a23.91 23.91 0 0 1 1.014 5.395m-1.014 8.855c-.118.38-.245.754-.38 1.125m.38-1.125a23.91 23.91 0 0 0 1.014-5.395m0-3.46c.495.413.811 1.035.811 1.73 0 .695-.316 1.317-.811 1.73m0-3.46a24.347 24.347 0 0 1 0 3.46" /></svg>',"icon-document":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg>',"icon-handshake":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 0 0 8.716-6.747M12 21a9.004 9.004 0 0 1-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 0 1 7.843 4.582M12 3a8.997 8.997 0 0 0-7.843 4.582m15.686 0A11.953 11.953 0 0 1 12 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0 1 21 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0 1 12 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 0 1 3 12c0-1.605.42-3.113 1.157-4.418" /></svg>',"icon-currency-euro":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 11.625h4.5m-4.5 2.25h4.5m2.121 1.527c-1.171 1.464-3.07 1.464-4.242 0-1.172-1.465-1.172-3.84 0-5.304 1.171-1.464 3.07-1.464 4.242 0M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg>',"icon-list":'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 6.75h12M8.25 12h12m-12 5.25h12M3.75 6.75h.007v.008H3.75V6.75Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0ZM3.75 12h.007v.008H3.75V12Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0ZM3.75 17.25h.007v.008H3.75v-.008Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" /></svg>'}[e]||'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.482-.22-2.121-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>'}init(){console.log("🧭 NavigationSystem: Initialisiere Navigation"),this.renderNavigation()}destroy(){console.log("🗑️ NavigationSystem: Destroy aufgerufen")}}const Q=new xe;class Ie{constructor(){this.entities={creator:{table:"creator",displayField:"vorname",fields:{vorname:"string",nachname:"string",instagram:"string",instagram_follower:"number",tiktok:"string",tiktok_follower:"number",telefonnummer:"string",mail:"string",portfolio_link:"string",budget_letzte_buchung:"number",lieferadresse_strasse:"string",lieferadresse_hausnummer:"string",lieferadresse_plz:"string",lieferadresse_stadt:"string",lieferadresse_land:"string",notiz:"string"},relations:{},manyToMany:{sprachen:{table:"sprachen",junctionTable:"creator_sprachen",localKey:"creator_id",foreignKey:"sprache_id",displayField:"name"},branchen:{table:"branchen_creator",junctionTable:"creator_branchen",localKey:"creator_id",foreignKey:"branche_id",displayField:"name"},creator_types:{table:"creator_type",junctionTable:"creator_creator_type",localKey:"creator_id",foreignKey:"creator_type_id",displayField:"name"}},filters:["vorname","nachname"],sortBy:"created_at",sortOrder:"desc"},ansprechpartner:{table:"ansprechpartner",displayField:"vorname",fields:{vorname:"string",nachname:"string",unternehmen_id:"uuid",position_id:"uuid",email:"string",telefonnummer:"string",telefonnummer_office:"string",linkedin:"string",stadt:"string",sprache_id:"uuid",notiz:"string"},relations:{unternehmen:{table:"unternehmen",foreignKey:"unternehmen_id",displayField:"firmenname"},sprache:{table:"sprachen",foreignKey:"sprache_id",displayField:"name"},position:{table:"positionen",foreignKey:"position_id",displayField:"name"}},manyToMany:{marken:{table:"marke",junctionTable:"ansprechpartner_marke",localKey:"ansprechpartner_id",foreignKey:"marke_id",displayField:"markenname"},sprachen:{table:"sprachen",junctionTable:"ansprechpartner_sprache",localKey:"ansprechpartner_id",foreignKey:"sprache_id",displayField:"name"}},filters:["vorname","nachname","position_id","unternehmen_id","stadt","sprache_id"],sortBy:"created_at",sortOrder:"desc"},unternehmen:{table:"unternehmen",displayField:"firmenname",fields:{firmenname:"string",branche:"string",branche_id:"uuid",ansprechpartner:"string",telefonnummer:"string",invoice_email:"string",rechnungsadresse_strasse:"string",rechnungsadresse_hausnummer:"string",rechnungsadresse_plz:"string",rechnungsadresse_stadt:"string",rechnungsadresse_land:"string",webseite:"string",status:"string",notiz:"string"},relations:{branche:{table:"branchen",foreignKey:"branche_id",displayField:"name"}},filters:["firmenname","branche_id","status","rechnungsadresse_stadt","rechnungsadresse_land"],sortBy:"created_at",sortOrder:"desc"},kampagne:{table:"kampagne",displayField:"kampagnenname",fields:{kampagnenname:"string",unternehmen_id:"uuid",marke_id:"uuid",auftrag_id:"uuid",ziele:"string",art_der_kampagne:"array",start:"date",deadline:"date",kampagnen_nummer:"number",drehort_typ_id:"uuid",drehort_beschreibung:"string",status_id:"uuid",creatoranzahl:"number",videoanzahl:"number",budget_info:"string"},relations:{unternehmen:{table:"unternehmen",foreignKey:"unternehmen_id",displayField:"firmenname"},marke:{table:"marke",foreignKey:"marke_id",displayField:"markenname"},auftrag:{table:"auftrag",foreignKey:"auftrag_id",displayField:"auftragsname"},drehort_typ:{table:"drehort_typen",foreignKey:"drehort_typ_id",displayField:"name"},status:{table:"kampagne_status",foreignKey:"status_id",displayField:"name"}},filters:["kampagnenname","unternehmen_id","marke_id","status_id","art_der_kampagne","start","deadline"],sortBy:"created_at",sortOrder:"desc"},kooperation:{table:"kooperationen",displayField:"id",fields:{name:"string",creator_id:"uuid",kampagne_id:"uuid",unternehmen_id:"uuid",status:"string",status_id:"uuid",content_art:"string",skript_autor:"string",nettobetrag:"number",zusatzkosten:"number",gesamtkosten:"number",vertrag_unterschrieben:"boolean",vertrag_link:"string",videoanzahl:"number",skript_deadline:"date",skript_link:"string",content_deadline:"date",content_link:"string",bewertung:"number",budget:"number",start_datum:"date",end_datum:"date",notiz:"string"},relations:{creator:{table:"creator",foreignKey:"creator_id",displayField:"vorname"},kampagne:{table:"kampagne",foreignKey:"kampagne_id",displayField:"name"}},filters:["creator_id","kampagne_id","status","budget","start_datum","end_datum"],sortBy:"created_at",sortOrder:"desc"},briefing:{table:"briefings",displayField:"product_service_offer",fields:{product_service_offer:"string",produktseite_url:"string",creator_aufgabe:"string",usp:"string",zielgruppe:"string",zieldetails:"string",deadline:"date",dos:"string",donts:"string",rechtlicher_hinweis:"string",unternehmen_id:"uuid",marke_id:"uuid",status:"string",assignee_id:"uuid",kooperation_id:"uuid",created_at:"date",updated_at:"date"},relations:{unternehmen:{table:"unternehmen",foreignKey:"unternehmen_id",displayField:"firmenname"},marke:{table:"marke",foreignKey:"marke_id",displayField:"markenname"},assignee:{table:"benutzer",foreignKey:"assignee_id",displayField:"name"}},filters:["product_service_offer","unternehmen_id","marke_id","status","assignee_id","deadline","created_at"],sortBy:"created_at",sortOrder:"desc"},creator_type:{table:"creator_type",displayField:"name",fields:{name:"string",beschreibung:"string"},filters:["name"],sortBy:"name",sortOrder:"asc"},sprachen:{table:"sprachen",displayField:"name",fields:{name:"string",code:"string"},filters:["name"],sortBy:"name",sortOrder:"asc"},branchen_creator:{table:"branchen_creator",displayField:"name",fields:{name:"string",beschreibung:"string"},filters:["name"],sortBy:"name",sortOrder:"asc"},benutzer:{table:"benutzer",displayField:"name",fields:{name:"string",email:"string",rolle:"string",unterrolle:"string",zugriffsrechte:"json",auth_user_id:"uuid",profile_image_url:"string",mitarbeiter_klasse_id:"uuid"},relations:{mitarbeiter_klasse:{table:"mitarbeiter_klasse",foreignKey:"mitarbeiter_klasse_id",displayField:"name"}}},kampagne_status:{table:"kampagne_status",displayField:"name",fields:{name:"string",beschreibung:"string",sort_order:"number",created_at:"date",updated_at:"date"}},plattform_typen:{table:"plattform_typen",displayField:"name",fields:{name:"string",beschreibung:"string",created_at:"date",updated_at:"date"}},format_typen:{table:"format_typen",displayField:"name",fields:{name:"string",beschreibung:"string",created_at:"date",updated_at:"date"}},marke:{table:"marke",displayField:"markenname",fields:{markenname:"string",unternehmen_id:"uuid",webseite:"string",branche:"string",branche_id:"uuid",created_at:"date",updated_at:"date"},relations:{unternehmen:{table:"unternehmen",foreignKey:"unternehmen_id",displayField:"firmenname"},branche:{table:"branchen",foreignKey:"branche_id",displayField:"name"}},filters:["markenname","unternehmen_id","branche_id"],sortBy:"created_at",sortOrder:"desc"},auftrag:{table:"auftrag",displayField:"auftragsname",fields:{auftragsname:"string",unternehmen_id:"uuid",marke_id:"uuid",status:"string",auftragtype:"string",art_der_kampagne:"array",format_anpassung:"array",start:"date",ende:"date",gesamt_budget:"number",ust_prozent:"number",ust_betrag:"number",ksk_betrag:"number",creator_budget:"number",kampagnenanzahl:"number",gesamtanzahl_videos:"number",influencer:"number",ugc:"number",ugc_organic:"number",ugc_paid:"number",ugc_paid_pro:"number",ugc_pro_organic:"number",influencer_preis:"number",ugc_preis:"number",vor_ort_preis:"number",cut_down:"number",vor_ort_produktion:"number",re_faelligkeit:"date",re_nr:"string",po:"string",rechnung_gestellt:"boolean",ueberwiesen:"boolean",bruttobetrag:"number",nettobetrag:"number",deckungsbeitrag_prozent:"number",deckungsbeitrag_betrag:"number",budgetverteilung:"string"},relations:{unternehmen:{table:"unternehmen",foreignKey:"unternehmen_id",displayField:"firmenname"},marke:{table:"marke",foreignKey:"marke_id",displayField:"markenname"}},filters:["auftragsname","status","auftragtype","art_der_kampagne","gesamt_budget","unternehmen_id","marke_id"],sortBy:"created_at",sortOrder:"desc"},rechnung:{table:"rechnung",displayField:"rechnung_nr",fields:{rechnung_nr:"string",kooperation_id:"uuid",kampagne_id:"uuid",creator_id:"uuid",auftrag_id:"uuid",unternehmen_id:"uuid",videoanzahl:"number",nettobetrag:"number",zusatzkosten:"number",bruttobetrag:"number",gestellt_am:"date",zahlungsziel:"date",bezahlt_am:"date",status:"string",geprueft:"boolean",pdf_url:"string",pdf_path:"string",created_at:"date",updated_at:"date"},relations:{unternehmen:{table:"unternehmen",foreignKey:"unternehmen_id",displayField:"firmenname"},auftrag:{table:"auftrag",foreignKey:"auftrag_id",displayField:"auftragsname"},kooperation:{table:"kooperationen",foreignKey:"kooperation_id",displayField:"name"},creator:{table:"creator",foreignKey:"creator_id",displayField:"vorname"},kampagne:{table:"kampagne",foreignKey:"kampagne_id",displayField:"kampagnenname"}},filters:["rechnung_nr","kooperation_id","kampagne_id","unternehmen_id","auftrag_id","status","gestellt_am","zahlungsziel","bezahlt_am","nettobetrag"],sortBy:"created_at",sortOrder:"desc"},creator_list:{table:"creator_list",displayField:"name",fields:{name:"string",beschreibung:"string",created_by:"uuid",created_at:"date",updated_at:"date"},relations:{},filters:["name","created_at"],sortBy:"created_at",sortOrder:"desc"}}}async createEntity(e,t){try{if(console.log(`✅ ${e} erstellt:`,t),!window.supabase||!window.supabase.auth){console.warn("⚠️ Supabase nicht verfügbar - verwende Mock-Daten");const s={id:Date.now().toString(),...t,created_at:new Date().toISOString()},o=JSON.parse(localStorage.getItem("mock_data")||"{}");return o[e]||(o[e]=[]),o[e].push(s),localStorage.setItem("mock_data",JSON.stringify(o)),console.log(`✅ ${e} Mock-Daten gespeichert:`,s),{success:!0,id:s.id,data:s}}const n=this.entities[e];if(!n)throw new Error(`Unbekannte Entität: ${e}`);if(e==="kooperation"&&(!t.content_art||t.content_art===""))try{const s=Object.entries(t).filter(([o,d])=>o.startsWith("video_content_art_")&&d).map(([,o])=>o)[0];s?t.content_art=s:t.content_art="N/A"}catch{t.content_art="N/A"}const a=await this.prepareDataForSupabase(t,n.fields,e),{data:r,error:i}=await window.supabase.from(n.table).insert([a]).select().single();return i?(console.error(`❌ Supabase Fehler beim Erstellen von ${e}:`,i),{success:!1,error:i.message}):(console.log(`✅ ${e} erfolgreich in Supabase erstellt:`,r),await this.handleManyToManyRelations(e,r.id,t),{success:!0,id:r.id,data:r})}catch(n){return console.error(`❌ Fehler beim Erstellen von ${e}:`,n),{success:!1,error:n.message}}}async updateEntity(e,t,n){try{if(console.log(`✅ ${e} aktualisiert:`,t,n),!window.supabase)return console.warn("⚠️ Supabase nicht verfügbar - verwende Mock-Daten"),{success:!0,id:t,data:n};const a=this.entities[e];if(!a)throw new Error(`Unbekannte Entität: ${e}`);const r=await this.prepareDataForSupabase(n,a.fields,e),{data:i,error:s}=await window.supabase.from(a.table).update(r).eq("id",t).select().single();return s?(console.error(`❌ Supabase Fehler beim Aktualisieren von ${e}:`,s),{success:!1,error:s.message}):(console.log(`✅ ${e} erfolgreich in Supabase aktualisiert:`,i),{success:!0,id:t,data:i})}catch(a){return console.error(`❌ Fehler beim Aktualisieren von ${e}:`,a),{success:!1,error:a.message}}}async deleteEntity(e,t){try{if(console.log(`✅ ${e} gelöscht:`,t),!window.supabase)return console.warn("⚠️ Supabase nicht verfügbar - verwende Mock-Daten"),{success:!0,id:t};const n=this.entities[e];if(!n)throw new Error(`Unbekannte Entität: ${e}`);const{error:a}=await window.supabase.from(n.table).delete().eq("id",t);return a?(console.error(`❌ Supabase Fehler beim Löschen von ${e}:`,a),{success:!1,error:a.message}):(console.log(`✅ ${e} erfolgreich in Supabase gelöscht:`,t),{success:!0,id:t})}catch(n){return console.error(`❌ Fehler beim Löschen von ${e}:`,n),{success:!1,error:n.message}}}async loadEntities(e,t={}){try{if(!window.supabase||!window.supabase.auth){console.warn("⚠️ Supabase nicht verfügbar - verwende Mock-Daten");const s=JSON.parse(localStorage.getItem("mock_data")||"{}");return s[e]&&s[e].length>0?(console.log(`✅ Mock-Daten für ${e} geladen:`,s[e]),s[e]):this.getMockData(e)}const n=this.entities[e];if(!n)throw new Error(`Unbekannte Entität: ${e}`);let a;if(e==="marke")a=window.supabase.from(n.table).select(`
                ...
                `).order("created_at",{ascending:!1}),t&&t.sprache_id){const s=String(t.sprache_id),{data:o,error:d}=await window.supabase.from("ansprechpartner_sprache").select("ansprechpartner_id").eq("sprache_id",s);d&&console.error("❌ Fehler beim Laden der Sprach-Verknüpfungen:",d);const c=(o||[]).map(l=>l.ansprechpartner_id).filter(Boolean);if(c.length===0)return[];a=a.in("id",c),delete t.sprache_id}}else e==="rechnung"?a=window.supabase.from(n.table).select(`
                  *,
                  unternehmen:unternehmen_id (
                    id,
                    firmenname
    ...
    `).join("");this.listEl.innerHTML=e,this.listEl.querySelectorAll(".uploader-remove").forEach(t=>{t.addEventListener("click",()=>{const n=parseInt(t.dataset.index,10);this.files.splice(n,1),this.renderList(),this.onFilesChanged(this.files)})})}formatSize(e){if(!e&&e!==0)return"";const t=["B","KB","MB","GB"];let n=e,a=0;for(;n>=1024&&a<t.length-1;)n/=1024,a++;return`${n.toFixed(1)} ${t[a]}`}}class ee{constructor(){this.validator=new J}getFieldLabel(e){return{unternehmen_id:"Unternehmen",marke_id:"Marke",auftrag_id:"Auftrag",kampagne_id:"Kampagne",creator_id:"Creator"}[e]||e}renderForm(e,t=null){const n=this.getFormConfig(e);if(!n)return console.error(`❌ Keine Konfiguration für Entity: ${e}`),"";const a=!!t,r=this.renderFormOnly(e,t);return`
      <div class="modal-header">
    ...
    `}renderFormOnly(e,t=null){const n=this.getFormConfig(e);if(!n)return"";const a=n.fields||[],r=new Set(["influencer","influencer_preis","ugc","ugc_preis","vor_ort_produktion","vor_ort_preis","ust_prozent","ust_betrag","deckungsbeitrag_prozent","deckungsbeitrag_betrag"]),i=[];let s=!1;for(const o of a){const d=t?t[o.name]:"",c=r.has(o.name);c&&!s&&(i.push('<div class="form-two-col">'),s=!0),!c&&s&&(i.push("</div>"),s=!1);let l=this.renderField(o,d);c&&(l=l.replace('<div class="form-field"','<div class="form-field form-field--half"')),i.push(l)}return s&&i.push("</div>"),`
      <form id="${e}-form" data-entity-id="${t?.id||""}">
        ${i.join("")}
        <div class="form-actions">
    ...
    `}renderField(e,t=""){const n=`field-${e.name}`,a=e.required?"required":"",r=e.required?'<span class="required">*</span>':"";switch(e.type){case"text":case"email":case"tel":case"url":const i=e.dependsOn?`data-depends-on="${e.dependsOn}"`:"",s=e.showWhen?`data-show-when="${e.showWhen}"`:"",o=e.autoGenerate?'data-auto-generate="true"':"",d=e.readonly?"readonly":"",c=e.placeholder?`placeholder="${e.placeholder}"`:"",l=e.dependsOn?'style="display: none;"':"";return`
          <div class="form-field" ${i} ${s} ${l}>
            <label for="${n}">${e.label} ${r}</label>
            <input type="${e.type}" id="${n}" name="${e.name}" value="${this.validator.sanitizeHtml(t)}" ${a} ${o} ${d} ${c}>
            ${e.autoGenerate?'<small style="color: #6b7280; font-size: 12px;">Wird automatisch generiert</small>':""}
          </div>
        `;case"number":const u=e.readonly?"readonly":"",m=e.autoCalculate?'data-auto-calculate="true"':"",h=e.calculatedFrom?`data-calculated-from="${e.calculatedFrom.join(",")}"`:"",g=e.placeholder?`placeholder="${e.placeholder}"`:"",b=e.validation?.min!==void 0?`min="${e.validation.min}"`:"",v=e.validation?.max!==void 0?`max="${e.validation.max}"`:"",w=e.validation?.step!==void 0?`step="${e.validation.step}"`:'step="0.01"',f=e.defaultValue!==void 0&&(t===""||t===void 0||t===null)?`value="${e.defaultValue}"`:`value="${t}"`;return`
          <div class="form-field">
            <label for="${n}">${e.label} ${r}</label>
            <input type="number" id="${n}" name="${e.name}" ${f} ${a} ${u} ${m} ${h} ${g} ${b} ${v} ${w}>
        ...
        `;case"textarea":const _=e.dependsOn?`data-depends-on="${e.dependsOn}"`:"",E=e.showWhen?`data-show-when="${e.showWhen}"`:"";return`
          <div class="form-field form-field-full" ${_} ${E}>
            <label for="${n}">${e.label} ${r}</label>
            <textarea id="${n}" name="${e.name}" rows="4" ${a}>${this.validator.sanitizeHtml(t)}</textarea>
          </div>
        `;case"select":let $="";if(e.dynamic?$=`<option value="">${e.dependsOn?`Erst ${this.getFieldLabel(e.dependsOn)} auswählen...`:e.placeholder||"Bitte wählen..."}</option>`:$=e.options.map(F=>`<option value="${F}" ${t===F?"selected":""}>${F}</option>`).join(""),e.searchable){const F=e.dependsOn?"disabled":"",B=e.readonly?"disabled":F,O=e.dependsOn?`Erst ${this.getFieldLabel(e.dependsOn)} auswählen...`:e.placeholder||"Bitte wählen...";return`
            <div class="form-field">
              <label for="${n}">${e.label} ${r}</label>
              <select id="${n}" name="${e.name}" ${a} ${B} data-searchable="true" data-placeholder="${O}" ${e.readonly?'data-readonly="true"':""}>
                ${$}
              </select>
              ${e.readonly?`<input type="hidden" name="${e.name}" id="${n}-hidden" value="">`:""}
            </div>
          `}const A=e.dependsOn?"disabled":"",y=e.readonly?"disabled":A;return`
          <div class="form-field">
            <label for="${n}">${e.label} ${r}</label>
            <select id="${n}" name="${e.name}" ${a} ${y} ${e.readonly?'data-readonly="true"':""}>
              ${$}
            </select>
            ${e.readonly?`<input type="hidden" name="${e.name}" id="${n}-hidden" value="">`:""}
          </div>
        `;case"multiselect":const L=Array.isArray(t)?t:t?t.split(","):[];let D="";return e.dynamic||(D=e.options.map(F=>{const B=L.includes(F)?"selected":"";return`<option value="${F}" ${B}>${F}</option>`}).join("")),e.searchable?`
            <div class="form-field">
              <label for="${n}">${e.label} ${r}</label>
              <select id="${n}" name="${e.name}" ${a} multiple data-searchable="true" data-tag-based="${e.tagBased||"false"}" data-placeholder="${e.placeholder||"Bitte wählen..."}">
                ${D}
              </select>
            </div>
          `:`
        ...
        `;case"checkbox":const C=t==="on"||t===!0||t==="true"?"checked":"";return`
          <div class="form-field">
            <label class="toggle-container">
              <span>${e.label} ${r}</span>
              <div class="toggle-switch">
                <input type="checkbox" id="${n}" name="${e.name}" ${C} ${a}>
                <span class="toggle-slider"></span>
              </div>
            </label>
          </div>
        `;case"toggle":const I=t==="on"||t===!0||t==="true"||t===1?"checked":"";return`
          <div class="form-field">
            <label class="toggle-container">
              <span>${e.label} ${r}</span>
              <div class="toggle-switch">
                <input type="checkbox" id="${n}" name="${e.name}" ${I} ${a}>
                <span class="toggle-slider"></span>
              </div>
            </label>
          </div>
        `;case"custom":if(e.customType==="addresses")return this.renderAddressesField(e,t);if(e.customType==="videos")return this.renderVideosField(e,t);if(e.customType==="file"){const F=e.accept?`accept="${e.accept}"`:"",B=e.multiple?"multiple":"";return`
            <div class="form-field">
              <label for="${n}">${e.label} ${r}</label>
              <input type="file" id="${n}" name="${e.name}" ${a} ${F} ${B}>
            </div>
          `}if(e.customType==="uploader"){const F=`uploader-${e.name}-${Math.random().toString(36).slice(2)}`;return setTimeout(()=>{const B=document.getElementById(F);B&&new Re({multiple:!!e.multiple,accept:e.accept||"*/*"}).mount(B.querySelector(".uploader"))},0),`
            <div class="form-field" id="${F}">
    ...
    `;e.insertAdjacentHTML("beforeend",n)}getFormConfig(e){return null}}class He{constructor(){this.validator=window.validatorSystem||{sanitizeHtml:e=>e||"",validateForm:(e,t)=>({isValid:!0,errors:{}})}}validateFormData(e,t){const n=[],a=this.getFormConfig(e);return a&&a.fields.forEach(r=>{if(r.required&&(!t[r.name]||t[r.name].toString().trim()==="")&&n.push(`${r.label} ist erforderlich.`),r.validation&&t[r.name]){const i=t[r.name],s=this.validateField(r,i);n.push(...s)}}),n}validateField(e,t){const n=[];if(!e.validation)return n;switch(e.validation.type){case"text":e.validation.minLength&&t.length<e.validation.minLength&&n.push(`${e.label} muss mindestens ${e.validation.minLength} Zeichen lang sein.`),e.validation.maxLength&&t.length>e.validation.maxLength&&n.push(`${e.label} darf maximal ${e.validation.maxLength} Zeichen lang sein.`);break;case"number":const a=parseFloat(t);isNaN(a)?n.push(`${e.label} muss eine Zahl sein.`):(e.validation.min!==void 0&&a<e.validation.min&&n.push(`${e.label} muss mindestens ${e.validation.min} sein.`),e.validation.max!==void 0&&a>e.validation.max&&n.push(`${e.label} darf maximal ${e.validation.max} sein.`));break;case"email":/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t)||n.push(`${e.label} muss eine gültige E-Mail-Adresse sein.`);break;case"url":try{new URL(t)}catch{n.push(`${e.label} muss eine gültige URL sein.`)}break;case"phone":/^[\+]?[0-9\s\-\(\)]{10,}$/.test(t)||n.push(`${e.label} muss eine gültige Telefonnummer sein.`);break}return n}sanitizeHtml(e){return e?e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#x27;"):""}showValidationErrors(e){if(document.querySelectorAll(".field-error").forEach(t=>t.remove()),e.forEach(t=>{const n=this.extractFieldNameFromError(t);if(n){const a=document.querySelector(`[name="${n}"]`);if(a){const r=document.createElement("div");r.className="field-error",r.textContent=t,r.style.cssText=`
            color: #dc2626;
            font-size: 12px;
            margin-top: 4px;
          `,a.parentNode.appendChild(r)}}}),e.length>0){const t=e.join(`
`);alert(`Validierungsfehler:
${t}`)}}extractFieldNameFromError(e){const t=["vorname","nachname","firmenname","kampagnenname","auftragsname","markenname"];for(const n of t)if(e.toLowerCase().includes(n))return n;return null}showSuccessMessage(e){console.log(`✅ ${e}`)}showErrorMessage(e){console.error(`❌ ${e}`),alert(e)}getFormConfig(e){return null}}class te{async autoGenerateKampagnenname(e,t){try{if(!t)return;console.log(`🔧 Generiere Kampagnenname für Auftrag: ${t}`);const{data:n,error:a}=await window.supabase.from("auftrag").select(`
          *,
          unternehmen:unternehmen_id(firmenname),
          marke:marke_id(markenname)
        `).eq("id",t).single();if(a||!n){console.error("❌ Fehler beim Laden des Auftrags:",a);return}const{data:r,error:i}=await window.supabase.from("kampagne").select("id").eq("auftrag_id",t);if(i){console.error("❌ Fehler beim Zählen der Kampagnen:",i);return}const o=r.length+1,d=n.kampagnenanzahl||1,c=e.querySelector('input[name="deadline"]'),l=c?c.value:"";let m=`${n.unternehmen?.firmenname||"Unbekannte Firma"} - ${o}/${d}`;if(l){const b=new Date(l).toLocaleDateString("de-DE");m+=` - ${b}`}const h=e.querySelector('input[name="kampagnenname"]');h&&(h.value=m,h.dispatchEvent(new Event("input",{bubbles:!0})),h.dispatchEvent(new Event("change",{bubbles:!0})),h.focus(),h.blur(),console.log(`✅ Kampagnenname generiert: ${m}`),console.log(`🔍 Feld-Wert nach Setzen: "${h.value}"`))}catch(n){console.error("❌ Fehler beim Generieren des Kampagnennamens:",n)}}async autoGenerateKooperationsname(e){try{const t=e.querySelector('select[name="kampagne_id"]'),n=e.querySelector('select[name="creator_id"]'),a=e.querySelector('input[name="name"]');if(!t||!n||!a)return;const r=t.value,i=n.value;if(!r||!i)return;const{data:s,error:o}=await window.supabase.from("kampagne").select("id, creatoranzahl").eq("id",r).single();if(o||!s){console.error("❌ Fehler beim Laden der Kampagne für Kooperation:",o);return}const{data:d,error:c}=await window.supabase.from("kooperationen").select("id").eq("kampagne_id",r);if(c){console.error("❌ Fehler beim Zählen der Kooperationen:",c);return}const l=(d?.length||0)+1,u=s.creatoranzahl||1,{data:m,error:h}=await window.supabase.from("creator").select("vorname, nachname, instagram").eq("id",i).single();if(h||!m){console.error("❌ Fehler beim Laden des Creators:",h);return}const v=`${`${m.vorname||""} ${m.nachname||""}`.trim()||(m.instagram?`@${m.instagram}`:"Creator")} ${l}/${u}`;a.value=v,a.dispatchEvent(new Event("input",{bubbles:!0})),a.dispatchEvent(new Event("change",{bubbles:!0})),a.focus(),a.blur(),console.log(`✅ Kooperationsname generiert: ${v}`)}catch(t){console.error("❌ Fehler beim Generieren des Kooperationsnamens:",t)}}setupAutoGeneration(e){const t=e.querySelector('input[name="deadline"]');t&&t.addEventListener("change",()=>{const s=e.querySelector('select[name="auftrag_id"]');s&&s.value&&this.autoGenerateKampagnenname(e,s.value)});const n=e.querySelector('select[name="auftrag_id"]');n&&(n.addEventListener("change",()=>{n.value&&this.autoGenerateKampagnenname(e,n.value)}),n.value&&this.autoGenerateKampagnenname(e,n.value));const a=e.querySelector(".searchable-select-container, .tag-based-select");if(a){const s=a.querySelector(".searchable-select-input");if(s){let o;s.addEventListener("input",()=>{clearTimeout(o),o=setTimeout(()=>{const c=a.querySelector('select[style*="display: none"]');c&&c.value&&this.autoGenerateKampagnenname(e,c.value)},300)});const d=a.querySelector('select[style*="display: none"]');d&&d.value&&this.autoGenerateKampagnenname(e,d.value)}}const r=e.querySelector('select[name="kampagne_id"]'),i=e.querySelector('select[name="creator_id"]');if(r&&i){const s=()=>this.autoGenerateKooperationsname(e);r.addEventListener("change",s),i.addEventListener("change",s),r.value&&i.value&&this.autoGenerateKooperationsname(e);const o=d=>{const c=d.previousElementSibling;if(c&&c.classList.contains("searchable-select-container")){const l=c.querySelector(".searchable-select-input");if(l){let u;l.addEventListener("input",()=>{clearTimeout(u),u=setTimeout(()=>this.autoGenerateKooperationsname(e),300)})}}};o(r),o(i)}}}class ne{constructor(){this.calculationRules={deckungsbeitrag_betrag:this.calculateDeckungsbeitragBetrag.bind(this),ust_betrag:this.calculateUstBetrag.bind(this),brutto_gesamt_budget:this.calculateBruttoGesamtBudget.bind(this),ksk_betrag:this.calculateKskBetrag.bind(this),nettobetrag:this.calculateNettoFromItems.bind(this),creator_budget:this.calculateCreatorBudget.bind(this)}}initializeAutoCalculation(e){if(!e)return;console.log("🔧 Initialisiere Auto-Calculation für Formular");const t=e.querySelectorAll("[data-calculated-from]"),n=e.querySelectorAll('[data-auto-calculate="true"]');n.forEach(r=>{r.addEventListener("input",i=>{this.handleFieldChange(e,i.target)}),r.addEventListener("change",i=>{this.handleFieldChange(e,i.target)})}),e.querySelectorAll('input[type="number"]').forEach(r=>{r.addEventListener("input",i=>{this.recalculateAllDependentFields(e)}),r.addEventListener("change",i=>{this.recalculateAllDependentFields(e)})}),console.log(`✅ Auto-Calculation initialisiert für ${t.length} berechnete Felder und ${n.length} Trigger-Felder`)}handleFieldChange(e,t){console.log(`🔄 Feld geändert: ${t.name} = ${t.value}`),this.recalculateAllDependentFields(e)}recalculateAllDependentFields(e){e.querySelectorAll("[data-calculated-from]").forEach(n=>{const a=n.name;if(this.calculationRules[a]){const r=this.calculationRules[a](e,n);r!=null&&(n.value=r,console.log(`💰 ${a} neu berechnet: ${r}`))}})}calculateDeckungsbeitragBetrag(e,t){try{const n=e.querySelector('[name="deckungsbeitrag_prozent"]'),a=e.querySelector('[name="nettobetrag"]');if(!n||!a)return console.warn("⚠️ Erforderliche Felder für Deckungsbeitrag-Berechnung nicht gefunden"),0;const r=parseFloat(n.value)||0,i=parseFloat(a.value)||0;if(r===0||i===0)return 0;const s=i*(r/100);return console.log(`💰 Deckungsbeitrag berechnet: ${i}€ * ${r}% = ${s}€`),Math.round(s*100)/100}catch(n){return console.error("❌ Fehler bei Deckungsbeitrag-Berechnung:",n),0}}calculateUstBetrag(e,t){try{const n=e.querySelector('[name="nettobetrag"]');if(!n)return 0;const a=parseFloat(n.value)||0,r=e.querySelector('[name="ust_prozent"]'),i=r&&parseFloat(r.value)||19,s=a*(i/100);return Math.round(s*100)/100}catch(n){return console.error("❌ Fehler bei USt-Berechnung:",n),0}}calculateBruttoGesamtBudget(e,t){try{const n=e.querySelector('[name="nettobetrag"]'),a=e.querySelector('[name="ust_betrag"]'),r=n&&parseFloat(n.value)||0,i=a?parseFloat(a.value)||this.calculateUstBetrag(e):this.calculateUstBetrag(e),s=r+i;return Math.round(s*100)/100}catch(n){return console.error("❌ Fehler bei Brutto-Berechnung:",n),0}}calculateKskBetrag(e,t){try{const n=e.querySelector('[name="nettobetrag"]');if(!n)return 0;const r=(parseFloat(n.value)||0)*.05;return Math.round(r*100)/100}catch(n){return console.error("❌ Fehler bei KSK-Berechnung:",n),0}}calculateNettoFromItems(e,t){try{const n=l=>{const u=e.querySelector(`[name="${l}"]`);return u&&parseFloat(u.value)||0},a=n("influencer"),r=n("influencer_preis"),i=n("ugc"),s=n("ugc_preis"),o=n("vor_ort_produktion"),d=n("vor_ort_preis"),c=a*r+i*s+o*d;return c>0?Math.round(c*100)/100:null}catch(n){return console.error("❌ Fehler bei Netto-Berechnung aus Items:",n),0}}calculateCreatorBudget(e,t){try{const n=o=>e.querySelector(`[name="${o}"]`),a=parseFloat(n("nettobetrag")?.value||"0")||0;let r=parseFloat(n("ksk_betrag")?.value||"0");(!r||r===0)&&(r=Math.round(a*.05*100)/100);let i=parseFloat(n("deckungsbeitrag_betrag")?.value||"0");if(!i||i===0){const o=parseFloat(n("deckungsbeitrag_prozent")?.value||"0")||0;i=Math.round(a*(Math.min(Math.max(o,0),100)/100)*100)/100}const s=Math.max(0,a-r-i);return Math.round(s*100)/100}catch(n){return console.error("❌ Fehler bei Creator-Budget-Berechnung:",n),0}}formatCurrency(e){return!e||isNaN(e)?"0,00":new Intl.NumberFormat("de-DE",{minimumFractionDigits:2,maximumFractionDigits:2}).format(e)}validatePercentage(e){const t=parseFloat(e);return isNaN(t)||t<0?0:t>100?100:t}}new ne;document.addEventListener("DOMContentLoaded",()=>{console.log("🔧 AutoCalculation Modul geladen")});class ae{constructor(e){this.autoGeneration=e,this.dynamicDataLoader=null}setupDependentFields(e){e.querySelectorAll("[data-depends-on]").forEach(n=>{const a=n.dataset.dependsOn,r=n.dataset.showWhen,i=e.querySelector(`[name="${a}"]`);if(i){const s=()=>{const o=i.value;console.log(`🔍 Prüfe abhängiges Feld: ${a} = "${o}", showWhen = "${r}"`);let d=!1;if(i.tagName==="SELECT"){const l=i.selectedOptions[0],u=l?l.textContent:"";d=r?u.includes(r):!!o,console.log(`🔍 Select-Feld: "${u}" enthält "${r}" = ${d}`)}else d=r?o===r||o.includes(r):!!o;const c=n.closest(".form-field");c&&(c.style.display=d?"block":"none",console.log(`🔍 Feld ${n.dataset.dependsOn} ${d?"angezeigt":"versteckt"}`),d&&n.dataset.autoGenerate==="true"&&this.autoGeneration.autoGenerateKampagnenname(e,o))};i.addEventListener("change",s),s()}}),this.setupDynamicDependentFields(e)}setupDynamicDependentFields(e){const t=e.dataset.entity;if(!t||!this.getFormConfig)return;const n=this.getFormConfig(t);n&&n.fields.forEach(a=>{if(a.dependsOn){const r=e.querySelector(`[name="${a.name}"]`),i=e.querySelector(`[name="${a.dependsOn}"]`);if(r&&i){console.log(`🔧 Setup dynamisches abhängiges Feld: ${a.name} abhängig von ${a.dependsOn}`);const s=async()=>{const d=this.getFieldValue(i);if(console.log(`🔍 Parent-Feld ${a.dependsOn} geändert:`,d),!d){this.clearDependentField(r,a),a.dependsOn==="unternehmen_id"?this.resetCascadeFields(e,["kampagne_id","creator_id","marke_id","auftrag_id"]):a.dependsOn==="marke_id"&&this.resetCascadeFields(e,["auftrag_id"]);return}await this.loadDependentFieldData(r,a,d,e)};i.addEventListener("change",s);const o=i.closest(".searchable-select-container, .tag-based-select");if(o){const d=o.querySelector(".searchable-select-input");if(d){let c;d.addEventListener("input",()=>{clearTimeout(c),c=setTimeout(s,300)})}}s()}}})}getFieldValue(e){const t=e.parentNode.querySelector('select[style*="display: none"]');return t&&t!==e?t.value:e.value}clearDependentField(e,t){console.log(`🧹 Leere abhängiges Feld: ${t.name}`);const a=`Erst ${{unternehmen_id:"Unternehmen",marke_id:"Marke",auftrag_id:"Auftrag"}[t.dependsOn]||t.dependsOn} auswählen...`,r=e.closest(".searchable-select-container, .tag-based-select");if(r){const i=r.querySelector(".searchable-select-input");i&&(i.value="",i.placeholder=a);const s=r.querySelector(".tags-container");s&&(s.innerHTML="")}e.innerHTML=`<option value="">${a}</option>`,e.value="",e.disabled=!0}async loadDependentFieldData(e,t,n,a){if(!this.dynamicDataLoader){console.error("❌ DynamicDataLoader nicht verfügbar");return}console.log(`🔄 Lade Daten für ${t.name} basierend auf ${t.dependsOn}:`,n);try{if(t.name==="marke_id"&&t.dependsOn==="unternehmen_id"){const{data:r,error:i}=await window.supabase.from("marke").select("id, markenname").eq("unternehmen_id",n).order("markenname");if(i){console.error("❌ Fehler beim Laden der Marken:",i);return}const s=r.map(o=>({value:o.id,label:o.markenname}));console.log(`✅ ${s.length} Marken geladen für Unternehmen ${n}`),e.disabled=!1,this.updateDependentFieldOptions(e,t,s),s.length===0&&(console.log(`⚠️ Keine Marken für Unternehmen ${n} gefunden - lade Aufträge direkt`),await this.loadAuftraegeForUnternehmen(n,a))}if(t.name==="auftrag_id"&&t.dependsOn==="marke_id"){const{data:r,error:i}=await window.supabase.from("auftrag").select("id, auftragsname").eq("marke_id",n).order("auftragsname");if(i){console.error("❌ Fehler beim Laden der Aufträge:",i);return}const s=r.map(o=>({value:o.id,label:o.auftragsname}));console.log(`✅ ${s.length} Aufträge geladen für Marke ${n}`),e.disabled=!1,this.updateDependentFieldOptions(e,t,s)}if(t.name==="auftrag_id"&&t.dependsOn==="unternehmen_id"){const{data:r,error:i}=await window.supabase.from("auftrag").select("id, auftragsname").eq("unternehmen_id",n).order("auftragsname");if(i){console.error("❌ Fehler beim Laden der Aufträge für Unternehmen:",i);return}const s=r.map(o=>({value:o.id,label:o.auftragsname}));console.log(`✅ ${s.length} Aufträge direkt für Unternehmen ${n} geladen`),e.disabled=!1,this.updateDependentFieldOptions(e,t,s)}if(t.name==="kampagne_id"&&t.dependsOn==="unternehmen_id"){const{data:r,error:i}=await window.supabase.from("kampagne").select("id, kampagnenname, unternehmen_id, videoanzahl").eq("unternehmen_id",n).order("kampagnenname");if(i){console.error("❌ Fehler beim Laden der Kampagnen für Unternehmen:",i);return}let s=r||[];try{const d=s.map(c=>c.id);if(d.length>0){const{data:c,error:l}=await window.supabase.from("kooperationen").select("kampagne_id, videoanzahl").in("kampagne_id",d);if(!l&&c){const u={};c.forEach(m=>{const h=m.kampagne_id,g=parseInt(m.videoanzahl,10)||0;u[h]=(u[h]||0)+g}),s=s.filter(m=>{const h=parseInt(m.videoanzahl,10)||0,g=u[m.id]||0;return Math.max(0,h-g)>0})}}}catch(d){console.warn("⚠️ Konnte belegte Videos nicht überprüfen, zeige alle Kampagnen:",d)}const o=s.map(d=>({value:d.id,label:d.kampagnenname||"Unbenannte Kampagne"}));e.disabled=!1,this.updateDependentFieldOptions(e,t,o)}if(t.name==="creator_id"&&t.dependsOn==="kampagne_id")try{const{data:r,error:i}=await window.supabase.from("kampagne_creator").select("creator_id, creator:creator_id(id, vorname, nachname)").eq("kampagne_id",n).order("creator_id");if(i){console.error("❌ Fehler beim Laden der Kampagnen-Creator:",i);return}const s=(r||[]).map(o=>{const d=o.creator||{},c=`${d.vorname||""} ${d.nachname||""}`.trim()||o.creator_id;return{value:o.creator_id,label:c}});e.disabled=!1,this.updateDependentFieldOptions(e,t,s)}catch(r){console.error("❌ Fehler beim Laden der Creator für Kampagne:",r)}}catch(r){console.error("❌ Fehler beim Laden der abhängigen Daten:",r)}}async loadAuftraegeForUnternehmen(e,t){try{const{data:n,error:a}=await window.supabase.from("auftrag").select("id, auftragsname, marke_id").eq("unternehmen_id",e).order("auftragsname");if(a){console.error("❌ Fehler beim Laden der Aufträge für Unternehmen:",a);return}if(n.length>0){console.log(`✅ ${n.length} Aufträge direkt für Unternehmen ${e} gefunden`);const r=t.querySelector('[name="auftrag_id"]');if(r){const i=n.map(o=>({value:o.id,label:o.auftragsname}));r.disabled=!1,this.updateDependentFieldOptions(r,{name:"auftrag_id"},i);const s=t.querySelector('[name="marke_id"]');s&&(s.innerHTML='<option value="">Keine Marken verfügbar</option>',s.disabled=!0),console.log("🔄 Auftrag-Feld temporär auf Unternehmen-Abhängigkeit umgestellt")}}else console.log(`⚠️ Keine Aufträge für Unternehmen ${e} gefunden`)}catch(n){console.error("❌ Fehler beim Laden der Aufträge für Unternehmen:",n)}}resetCascadeFields(e,t){console.log("🔄 Setze Kaskaden-Felder zurück:",t),t.forEach(n=>{const a=e.querySelector(`[name="${n}"]`);if(a){a.value="",a.disabled=!0;const r=a.closest(".searchable-select-container, .tag-based-select");if(r){const s=r.querySelector(".searchable-select-input");s&&(s.value="",s.placeholder=`Erst ${this.getFieldLabel(n.replace("_id",""))} auswählen...`);const o=r.querySelector(".tags-container");o&&(o.innerHTML="")}const i=`Erst ${this.getFieldLabel(n.replace("_id",""))} auswählen...`;a.innerHTML=`<option value="">${i}</option>`}})}getFieldLabel(e){return{unternehmen:"Unternehmen",marke:"Marke",auftrag:"Auftrag"}[e]||e}updateDependentFieldOptions(e,t,n){const a=e.closest(".searchable-select-container, .tag-based-select");if(a){const r=a.querySelector(".searchable-select-input");r&&(r.placeholder=t.placeholder||"Suchen...")}e.innerHTML='<option value="">Bitte wählen...</option>',n.forEach(r=>{const i=document.createElement("option");i.value=r.value,i.textContent=r.label,e.appendChild(i)}),this.dynamicDataLoader&&this.dynamicDataLoader.updateSelectOptions&&this.dynamicDataLoader.updateSelectOptions(e,n,t)}}class re{async handleRelationTables(e,t,n,a){try{const r=this.getFormConfig(e);if(!r)return;let i=[];r.sections?r.sections.forEach(s=>{i=i.concat(s.fields)}):i=r.fields;for(const s of i)if(s.relationTable&&s.relationField){const o=n[s.name];o&&await this.handleRelationTable(t,s,o)}}catch(r){console.error("❌ Fehler beim Verarbeiten der Verknüpfungstabellen:",r)}}async handleRelationTable(e,t,n){try{const a=t.relationTable,r=t.relationField;let i;t.name==="mitarbeiter_ids"||t.name==="plattform_ids"||t.name==="format_ids"?i="kampagne_id":i=`${t.name.replace("_ids","_id")}`;const s=`DELETE FROM ${a} WHERE ${i} = $1`;if(await window.dataService.executeQuery(s,[e]),Array.isArray(n))for(const o of n){const d=`INSERT INTO ${a} (${i}, ${r}) VALUES ($1, $2)`;await window.dataService.executeQuery(d,[e,o])}else if(typeof n=="string"&&n.includes(",")){const o=n.split(",").map(d=>d.trim());for(const d of o){const c=`INSERT INTO ${a} (${i}, ${r}) VALUES ($1, $2)`;await window.dataService.executeQuery(c,[e,d])}}console.log(`✅ Verknüpfungstabelle ${a} aktualisiert für ${e}`)}catch(a){console.error(`❌ Fehler beim Verarbeiten der Verknüpfungstabelle ${t.relationTable}:`,a)}}getFormConfig(e){return null}}class qe{constructor(e){this.formSystem=e}async bindFormEvents(e,t){const n=document.getElementById(`${e}-form`);if(!n)return;n.dataset.entity=e,n.onsubmit=async r=>{r.preventDefault(),await this.formSystem.handleFormSubmit(e,t)};const a=n.querySelector(".btn-close");a&&(a.onclick=()=>this.formSystem.closeForm()),this.initializeSearchableSelects(n),await this.formSystem.dataLoader.loadDynamicFormData(e,n),this.formSystem.dependentFields.setupDependentFields(n),this.setupAddressesFields(n),this.setupVideosFields(n),this.formSystem.autoGeneration.setupAutoGeneration(n),this.formSystem.autoCalculation.initializeAutoCalculation(n),this.setupEntitySpecificEvents(e,n)}setupEntitySpecificEvents(e,t){switch(e){case"auftrag":this.setupAuftragEvents(t);break;case"kampagne":this.setupKampagneEvents(t);break;case"kooperation":this.setupKooperationEvents(t);break;case"rechnung":this.setupRechnungEvents(t);break}}setupAuftragEvents(e){const t=e.querySelector('input[name="bruttobetrag"]'),n=e.querySelector('input[name="deckungsbeitrag_prozent"]'),a=e.querySelector('input[name="deckungsbeitrag_betrag"]');if(t&&n&&a){const r=()=>{const i=parseFloat(t.value)||0,s=parseFloat(n.value)||0,o=i*s/100;a.value=o.toFixed(2)};t.addEventListener("input",r),n.addEventListener("input",r)}}setupKampagneEvents(e){const t=e.querySelector('select[name="auftrag_id"]'),n=e.querySelector('input[name="videoanzahl"]');if(!t||!n||!window.supabase)return;(()=>{if(n.dataset.stepperAttached==="true")return;try{n.type="hidden"}catch{n.style.display="none"}const o=document.createElement("div");o.className="number-stepper";const d=document.createElement("button");d.type="button",d.className="stepper-btn stepper-minus secondary-btn",d.textContent="-";const c=document.createElement("button");c.type="button",c.className="stepper-btn stepper-plus secondary-btn",c.textContent="+";const l=document.createElement("span");l.className="stepper-info",n.parentNode.insertBefore(o,n.nextSibling),o.appendChild(d),o.appendChild(c),o.appendChild(l);const u=()=>({min:parseInt(n.min||"0",10)||0,max:parseInt(n.max||"0",10)||0}),m=g=>{const{min:b,max:v}=u(),w=parseInt(g||"0",10)||0;return v?String(Math.max(b,Math.min(w,v))):""},h=()=>{const{max:g}=u(),b=parseInt(n.value||"0",10)||0,v=Math.max(0,g-b),w=b===1?"Video":"Videos";l.textContent=g>0?`${b} ${w} | Rest: ${v}`:"Bitte zuerst Auftrag wählen",d.disabled=g===0||b<=(parseInt(n.min||"0",10)||0),c.disabled=g===0||b>=g};d.addEventListener("click",()=>{const{min:g}=u(),b=parseInt(n.value||"0",10)||0,v=Math.max(g,b-1);n.value=m(String(v)),n.dispatchEvent(new Event("input",{bubbles:!0})),n.dispatchEvent(new Event("change",{bubbles:!0})),h()}),c.addEventListener("click",()=>{const{max:g}=u(),b=parseInt(n.value||"0",10)||0,v=Math.min(g,b+1);n.value=m(String(v)),n.dispatchEvent(new Event("input",{bubbles:!0})),n.dispatchEvent(new Event("change",{bubbles:!0})),h()}),n.addEventListener("input",()=>{n.value=m(n.value),h()}),n.dataset.stepperAttached="true",h()})();const r=()=>{const o=n.parentNode.querySelector(".stepper-info"),d=n.parentNode.querySelector(".stepper-minus"),c=n.parentNode.querySelector(".stepper-plus"),l=parseInt(n.max||"0",10)||0,u=parseInt(n.min||"0",10)||0,m=parseInt(n.value||"0",10)||0,h=Math.max(0,l-m),g=m===1?"Video":"Videos";o&&(o.textContent=l>0?`${m} ${g} | Rest: ${h}`:"Bitte zuerst Auftrag wählen"),d&&(d.disabled=l===0||m<=u),c&&(c.disabled=l===0||m>=l)},i=(o,d,c)=>{const l=parseInt(o,10);return isNaN(l)||c===0?"":String(Math.max(d,Math.min(l,c)))},s=async()=>{const o=t.value;if(!o){n.disabled=!0,n.removeAttribute("max"),n.removeAttribute("min"),n.value="",r();return}try{const{data:d,error:c}=await window.supabase.from("auftrag").select("id, gesamtanzahl_videos").eq("id",o).single();if(c){console.error("❌ Fehler beim Laden des Auftrags (gesamtanzahl_videos):",c);return}const l=parseInt(d?.gesamtanzahl_videos,10)||0;let u=window.supabase.from("kampagne").select("id, videoanzahl").eq("auftrag_id",o);const m=e.dataset.entityId||null;m&&(u=u.neq("id",m));const{data:h,error:g}=await u;if(g){console.error("❌ Fehler beim Laden der Kampagnen (videoanzahl):",g);return}const b=(h||[]).reduce((w,f)=>w+(parseInt(f.videoanzahl,10)||0),0),v=Math.max(0,l-b);n.disabled=v===0,n.min=v>0?"1":"0",n.max=String(v),n.step="1",n.value?n.value=i(n.value,v>0?1:0,v):v>0&&(n.value="1"),r()}catch(d){console.error("❌ Fehler beim Aktualisieren der Kampagnen-Video-Limits:",d)}};t.addEventListener("change",s),n.addEventListener("change",()=>{const o=parseInt(n.max||"0",10)||0;o>0&&(n.value=i(n.value,1,o)),r()}),e.addEventListener("submit",async o=>{const d=t.value;if(d)try{const{data:c}=await window.supabase.from("auftrag").select("gesamtanzahl_videos").eq("id",d).single(),l=parseInt(c?.gesamtanzahl_videos,10)||0;let u=window.supabase.from("kampagne").select("videoanzahl").eq("auftrag_id",d);const m=e.dataset.entityId||null;m&&(u=u.neq("id",m));const{data:h}=await u,g=(h||[]).reduce((w,f)=>w+(parseInt(f.videoanzahl,10)||0),0),b=Math.max(0,l-g);(parseInt(n.value||"0",10)||0)>b&&(o.preventDefault(),n.value=i(n.value,b>0?1:0,b),alert("Die gewählte Video Anzahl überschreitet die verfügbaren Videos dieses Auftrags."),r())}catch{}}),s()}async setupRechnungEvents(e){const t=e.querySelector('select[name="kooperation_id"]');if(!t||!window.supabase)return;const n=e.querySelector('select[name="unternehmen_id"]'),a=e.querySelector('select[name="auftrag_id"]'),r=e.querySelector('select[name="creator_id"]'),i=e.querySelector('select[name="kampagne_id"]'),s=e.querySelector('input[name="videoanzahl"]'),o=e.querySelector('input[name="nettobetrag"]'),d=e.querySelector('input[name="zusatzkosten"]');e.querySelector('input[name="ust"]');const c=e.querySelector('input[name="bruttobetrag"]'),l=(m,h,g)=>{if(!m)return;m.innerHTML="";const b=document.createElement("option");b.value=h||"",b.textContent=g||"—",m.appendChild(b),m.value=h||"",m.getAttribute("data-readonly"),m.disabled=!0;const v=m.parentNode.querySelector(".searchable-select-container");if(v){const f=v.querySelector(".searchable-select-input");f&&(f.value=g||""),m.getAttribute("data-readonly")==="true"&&(f.setAttribute("disabled","true"),f.classList.add("is-disabled"))}const w=document.getElementById(`${m.id}-hidden`);w&&(w.value=h||"")},u=async()=>{const m=t.value;if(!m)return;const{data:h,error:g}=await window.supabase.from("kooperationen").select("id, name, unternehmen_id, kampagne_id, nettobetrag, gesamtkosten, zusatzkosten, videoanzahl").eq("id",m).single();if(g){console.error("❌ Fehler beim Laden der Kooperation:",g);return}let b="";if(h?.unternehmen_id)try{const{data:$}=await window.supabase.from("unternehmen").select("id, firmenname").eq("id",h.unternehmen_id).single();b=$?.firmenname||""}catch{}l(n,h?.unternehmen_id||"",b);let v=null,w="";if(h?.kampagne_id)try{const{data:$}=await window.supabase.from("kampagne").select("auftrag_id").eq("id",h.kampagne_id).single();if(v=$?.auftrag_id||null,i){let A="";try{const{data:y}=await window.supabase.from("kampagne").select("id, kampagnenname").eq("id",h.kampagne_id).single();A=y?.kampagnenname||""}catch{}l(i,h.kampagne_id,A||`Kampagne ${h.kampagne_id}`)}if(v){const{data:A}=await window.supabase.from("auftrag").select("id, auftragsname").eq("id",v).single();w=A?.auftragsname||""}}catch($){console.warn("⚠️ Konnte Auftrag über Kampagne nicht laden:",$)}if(l(a,v,w||(v?`Auftrag ${v}`:"")),r)try{const{data:$}=await window.supabase.from("kooperationen").select("creator_id").eq("id",m).single(),A=$?.creator_id||null;let y="";if(A){const{data:L}=await window.supabase.from("creator").select("id, vorname, nachname").eq("id",A).single();y=L?`${L.vorname||""} ${L.nachname||""}`.trim():""}l(r,A,y)}catch($){console.warn("⚠️ Konnte Creator nicht laden:",$)}s&&(s.value=h?.videoanzahl||"");const f=parseFloat(h?.nettobetrag||0)||0,_=parseFloat(h?.zusatzkosten||0)||0,E=h?.gesamtkosten!=null?h.gesamtkosten:f+_;o&&(o.value=f?String(f):""),d&&(d.value=_?String(_):""),c&&(c.value=isNaN(E)?"":String(E))};t.addEventListener("change",u),u()}setupKooperationEvents(e){const t=e.querySelector('select[name="kampagne_id"]'),n=e.querySelector('input[name="videoanzahl"]'),a=e.querySelector(".videos-container"),r=e.querySelector(".videos-list"),i=(()=>{try{return a?.dataset?.options?JSON.parse(a.dataset.options):[]}catch{return[]}})();if(!t||!n||!window.supabase)return;(()=>{if(n.dataset.stepperAttached==="true")return;try{n.type="hidden"}catch{n.style.display="none"}const l=document.createElement("div");l.className="number-stepper";const u=document.createElement("button");u.type="button",u.className="stepper-btn stepper-minus secondary-btn",u.textContent="-";const m=document.createElement("button");m.type="button",m.className="stepper-btn stepper-plus secondary-btn",m.textContent="+";const h=document.createElement("span");h.className="stepper-info",h.textContent="",n.parentNode.insertBefore(l,n.nextSibling),l.appendChild(u),l.appendChild(m),l.appendChild(h);const g=()=>({min:parseInt(n.min||"0",10)||0,max:parseInt(n.max||"0",10)||0}),b=f=>{const{min:_,max:E}=g(),$=parseInt(f||"0",10)||0;return E?String(Math.max(_,Math.min($,E))):""},v=()=>{const{max:f}=g(),_=parseInt(n.value||"0",10)||0,E=Math.max(0,f-_),$=_===1?"Video":"Videos";h.textContent=f>0?`${_} ${$} | Rest: ${E}`:"Bitte zuerst Kampagne wählen",u.disabled=f===0||_<=(parseInt(n.min||"0",10)||0),m.disabled=f===0||_>=f},w=()=>{if(!r)return;const f=parseInt(n.value||"0",10)||0,_=r.querySelectorAll(".video-item").length;if(f>_)for(let E=0;E<f-_;E++)this.addVideoRow(r,i);else if(f<_)for(let E=0;E<_-f;E++){const $=r.querySelector(".video-item:last-of-type");$&&$.remove()}};u.addEventListener("click",()=>{const{min:f}=g(),_=parseInt(n.value||"0",10)||0,E=Math.max(f,_-1);n.value=b(String(E)),n.dispatchEvent(new Event("input",{bubbles:!0})),n.dispatchEvent(new Event("change",{bubbles:!0})),v(),w()}),m.addEventListener("click",()=>{const{max:f}=g(),_=parseInt(n.value||"0",10)||0,E=Math.min(f,_+1);n.value=b(String(E)),n.dispatchEvent(new Event("input",{bubbles:!0})),n.dispatchEvent(new Event("change",{bubbles:!0})),v(),w()}),n.addEventListener("input",()=>{n.value=b(n.value),v(),w()}),n.dataset.stepperAttached="true",v()})();const o=()=>{const l=n.parentNode.querySelector(".stepper-info"),u=n.parentNode.querySelector(".stepper-minus"),m=n.parentNode.querySelector(".stepper-plus"),h=parseInt(n.max||"0",10)||0,g=parseInt(n.min||"0",10)||0,b=parseInt(n.value||"0",10)||0,v=Math.max(0,h-b),w=b===1?"Video":"Videos";l&&(l.textContent=h>0?`${b} ${w} | Rest: ${v}`:"Bitte zuerst Kampagne wählen"),u&&(u.disabled=h===0||b<=g),m&&(m.disabled=h===0||b>=h)},d=(l,u,m)=>{const h=parseInt(l,10);return isNaN(h)||m===0?"":String(Math.max(u,Math.min(h,m)))},c=async()=>{const l=t.value;if(!l){n.disabled=!0,n.removeAttribute("max"),n.removeAttribute("min"),n.value="",o(),r&&(r.innerHTML="");return}try{const{data:u,error:m}=await window.supabase.from("kampagne").select("videoanzahl").eq("id",l).single();if(m){console.error("❌ Fehler beim Laden der Kampagne (videoanzahl):",m);return}const h=u?.videoanzahl||0,{data:g,error:b}=await window.supabase.from("kooperationen").select("videoanzahl").eq("kampagne_id",l);if(b){console.error("❌ Fehler beim Laden der Kooperationen (videoanzahl):",b);return}const v=(g||[]).reduce((_,E)=>_+(parseInt(E.videoanzahl,10)||0),0),w=Math.max(0,h-v);n.disabled=w===0,n.min=w>0?"1":"0",n.max=String(w),n.step="1",n.value?n.value=d(n.value,1,w):w>0&&(n.value="1"),o();const f=parseInt(n.value||"0",10)||0;if(r){const _=r.querySelectorAll(".video-item").length;if(f!==_){const E=f-_;if(E>0)for(let $=0;$<E;$++)this.addVideoRow(r,i);else for(let $=0;$<Math.abs(E);$++){const A=r.querySelector(".video-item:last-of-type");A&&A.remove()}}}}catch(u){console.error("❌ Fehler beim Aktualisieren der Video-Limits:",u)}};t.addEventListener("change",c),n.addEventListener("change",async()=>{const l=parseInt(n.max||"0",10)||0;if(l>0&&(n.value=d(n.value,1,l)),o(),r){const u=parseInt(n.value||"0",10)||0,m=r.querySelectorAll(".video-item").length;if(u!==m){const h=u-m;if(h>0)for(let g=0;g<h;g++)this.addVideoRow(r,i);else for(let g=0;g<Math.abs(h);g++){const b=r.querySelector(".video-item:last-of-type");b&&b.remove()}}}}),e.addEventListener("submit",async l=>{const u=t.value;if(u)try{const{data:m}=await window.supabase.from("kampagne").select("videoanzahl").eq("id",u).single(),h=m?.videoanzahl||0,{data:g}=await window.supabase.from("kooperationen").select("videoanzahl").eq("kampagne_id",u),b=(g||[]).reduce((f,_)=>f+(parseInt(_.videoanzahl,10)||0),0),v=Math.max(0,h-b);(parseInt(n.value||"0",10)||0)>v&&(l.preventDefault(),n.value=d(n.value,v>0?1:0,v),alert("Die gewählte Video Anzahl überschreitet die verfügbaren Videos dieser Kampagne."),o())}catch{}}),c(),(async()=>{try{const l=e.dataset.entityId;if(!l||!window.supabase||!r)return;const{data:u,error:m}=await window.supabase.from("kooperation_videos").select("id, content_art, titel, asset_url, kommentar, position").eq("kooperation_id",l).order("position",{ascending:!0});if(m)return;(u||[]).forEach(h=>this.addVideoRow(r,i,h)),n.value=String((u||[]).length||""),o()}catch{}})()}initializeSearchableSelects(e){e.querySelectorAll('select[data-searchable="true"]').forEach(n=>{this.formSystem.createSearchableSelect(n,[],{})})}setupAddressesFields(e){e.querySelectorAll(".addresses-container").forEach(n=>{const a=n.querySelector(".add-address-btn"),r=n.querySelector(".addresses-list");a&&r&&a.addEventListener("click",()=>{this.addAddressRow(r)})})}setupVideosFields(e){}addVideoRow(e,t=[],n={}){const a=`video-${Date.now()}`,r=['<option value="">Bitte wählen</option>'].concat(t.map(s=>`<option value="${s}" ${n.content_art===s?"selected":""}>${s}</option>`)).join(""),i=`
    ...
    `;e.insertAdjacentHTML("beforeend",n)}}class ie{constructor(){this.dataService=window.dataService}setDataService(e){this.dataService=e}async loadDynamicFormData(e,t){try{const n=this.getFormConfig(e);if(!n)return;const a=n.fields||[];for(const r of a)r.dynamic&&await this.loadFieldOptions(e,r,t)}catch(n){console.error("❌ Fehler beim Laden der dynamischen Formulardaten:",n)}}async loadFieldOptions(e,t,n){try{if(this.dataService||(console.error("❌ DataService ist nicht verfügbar in DynamicDataLoader"),this.dataService=window.dataService),t.dependsOn){console.log(`⏭️ Überspringe automatisches Laden für abhängiges Feld: ${t.name} (abhängig von ${t.dependsOn})`);return}let a=[];if(e==="rechnung"&&t.name==="kooperation_id")console.log("🔧 Lade Kooperationen ohne bestehende Rechnung für Rechnungsformular"),a=await this.loadKooperationenOhneRechnung(),console.log("✅ kooperation_id Optionen (ohne Rechnung):",a.length);else if(t.table)console.log(`🔧 Lade Daten direkt aus Tabelle ${t.table} für ${t.name}`),a=await this.loadDirectQueryOptions(t,n),console.log(`✅ ${t.name} Optionen geladen:`,a.length,a.slice(0,3));else switch(t.name){case"unternehmen_id":if(!window.supabase){a=(await this.dataService.loadEntities("unternehmen")).map(_=>({value:_.id,label:_.firmenname||"Unbekanntes Unternehmen"}));break}try{if(e==="kooperation"){const{data:f,error:_}=await window.supabase.from("kampagne").select("unternehmen_id").not("unternehmen_id","is",null);if(_){console.error("❌ Fehler beim Laden der Kampagnen-Unternehmen:",_);break}const E=Array.from(new Set((f||[]).map(y=>y.unternehmen_id).filter(Boolean))),{data:$,error:A}=await window.supabase.from("unternehmen").select("id, firmenname").in("id",E).order("firmenname");if(A){console.error("❌ Fehler beim Laden der Unternehmen:",A);break}a=($||[]).map(y=>({value:y.id,label:y.firmenname||"Unbekanntes Unternehmen"}))}else if(e==="kampagne"){const{data:f,error:_}=await window.supabase.from("auftrag").select("unternehmen_id").not("unternehmen_id","is",null);if(_){console.error("❌ Fehler beim Laden der Auftrag-Unternehmen:",_);break}const E=Array.from(new Set((f||[]).map(y=>y.unternehmen_id).filter(Boolean))),{data:$,error:A}=await window.supabase.from("unternehmen").select("id, firmenname").in("id",E).order("firmenname");if(A){console.error("❌ Fehler beim Laden der Unternehmen:",A);break}a=($||[]).map(y=>({value:y.id,label:y.firmenname||"Unbekanntes Unternehmen"}))}else if(e==="marke"){const{data:f,error:_}=await window.supabase.from("unternehmen").select("id, firmenname").order("firmenname");if(_){console.error("❌ Fehler beim Laden aller Unternehmen:",_);break}a=(f||[]).map(E=>({value:E.id,label:E.firmenname||"Unbekanntes Unternehmen"}))}else{const{data:f,error:_}=await window.supabase.from("unternehmen").select("id, firmenname").order("firmenname");if(_){console.error("❌ Fehler beim Laden aller Unternehmen:",_);break}a=(f||[]).map(E=>({value:E.id,label:E.firmenname||"Unbekanntes Unternehmen"}))}}catch(f){console.error("❌ Fehler beim Laden der Unternehmen (kontext-spezifisch):",f)}break;case"auftrag_id":a=(await this.dataService.loadEntities("auftrag")).map(f=>({value:f.id,label:f.auftragsname||"Unbekannter Auftrag"}));break;case"creator_id":a=(await this.dataService.loadEntities("creator")).map(f=>({value:f.id,label:`${f.vorname} ${f.nachname}`||"Unbekannter Creator"}));break;case"kampagne_id":a=(await this.dataService.loadEntities("kampagne")).map(f=>({value:f.id,label:f.kampagnenname||"Unbekannte Kampagne"}));break;case"creator_type_id":const{data:d,error:c}=await window.supabase.from("creator_type").select("id, name").order("name");!c&&d&&(a=d.map(f=>({value:f.id,label:f.name||"Unbekannter Typ"})),console.log("✅ Creator Types geladen:",a.length));break;case"branche_id":const{data:l,error:u}=await window.supabase.from("branchen").select("id, name, beschreibung").order("name");!u&&l&&(a=l.map(f=>({value:f.id,label:f.name||"Unbekannte Branche",description:f.beschreibung})),console.log("✅ Branchen geladen:",a.length));break;case"assignee_id":const{data:m,error:h}=await window.supabase.from("benutzer").select("id, name").order("name");h?console.error("❌ Fehler beim Laden der Benutzer:",h):m?(a=m.map(f=>({value:f.id,label:f.name||"Unbekannter Benutzer"})),console.log("✅ Benutzer geladen:",a.length,a)):console.warn("⚠️ Keine Benutzer gefunden");break;case"art_der_kampagne":const{data:g,error:b}=await window.supabase.from("kampagne_art_typen").select("id, name").order("sort_order, name");!b&&g&&(a=g.map(f=>({value:f.id,label:f.name||"Unbekannte Art"})),console.log("✅ Kampagne Art Typen geladen:",a.length));break;case"format_anpassung":const{data:v,error:w}=await window.supabase.from("format_anpassung_typen").select("id, name").order("sort_order, name");!w&&v&&(a=v.map(f=>({value:f.id,label:f.name||"Unbekanntes Format"})),console.log("✅ Format Anpassung Typen geladen:",a.length));break;default:break}const r=n.querySelector(`[name="${t.name}"]`);r?(console.log(`🔧 Update Select für ${t.name} mit ${a.length} Optionen`),this.updateSelectOptions(r,a,t)):console.log(`❌ Select-Element nicht gefunden für ${t.name}`)}catch(a){console.error(`❌ Fehler beim Laden der Optionen für ${t.name}:`,a)}}async loadKooperationenOhneRechnung(){if(!window.supabase)return[];try{const{data:e,error:t}=await window.supabase.from("rechnung").select("kooperation_id").not("kooperation_id","is",null);if(t)return console.error("❌ Fehler beim Laden vorhandener Rechnungen:",t),[];const n=Array.from(new Set((e||[]).map(o=>o.kooperation_id).filter(Boolean)));let a=window.supabase.from("kooperationen").select("id, name, kampagne_id").order("created_at",{ascending:!1});n.length>0&&(a=a.not("id","in",`(${n.join(",")})`));const{data:r,error:i}=await a;if(i)return console.error("❌ Fehler beim Laden der Kooperationen (ohne Rechnung):",i),[];let s={};try{const o=Array.from(new Set((r||[]).map(d=>d.kampagne_id).filter(Boolean)));if(o.length>0){const{data:d}=await window.supabase.from("kampagne").select("id, kampagnenname").in("id",o);s=(d||[]).reduce((c,l)=>(c[l.id]=l.kampagnenname,c),{})}}catch{}return(r||[]).map(o=>({value:o.id,label:o.name?`${o.name} ${o.kampagne_id?`— ${s[o.kampagne_id]||"Kampagne"}`:""}`:s[o.kampagne_id]||o.id}))}catch(e){return console.error("❌ Unerwarteter Fehler beim Laden der kooperation_id Optionen:",e),[]}}async loadDirectQueryOptions(e,t){try{if(!e.table)return console.warn("⚠️ Keine Tabelle für direktes Laden definiert:",e.name),[];const n=window.supabase.from(e.table).select("*");e.filter&&n.or(e.filter);const{data:a,error:r}=await n;if(r)return console.error(`❌ Fehler beim Laden der Daten aus ${e.table}:`,r),[];const i=a.map(s=>({value:s[e.valueField||"id"],label:s[e.displayField||"name"]||"Unbekannt",description:s.beschreibung||s.description}));if(e.name==="branchen_ids"&&t.dataset.entityId)try{const s=t.dataset.entityId,{data:o,error:d}=await window.supabase.from("unternehmen").select("branche_id, branche").eq("id",s).single();if(!d&&o){if(o.branche_id){const c=i.find(l=>l.value===o.branche_id);c&&(c.selected=!0)}console.log("✅ Bestehende Branche für Edit-Modus geladen:",o.branche_id)}}catch(s){console.error("❌ Fehler beim Laden der bestehenden Branche:",s)}else if(t.dataset.entityId&&e.relationTable&&e.relationField){const s=t.dataset.entityId;let o;e.name==="mitarbeiter_ids"||e.name==="plattform_ids"||e.name==="format_ids"?o="kampagne_id":o=e.name.replace("_ids","_id");const{data:d,error:c}=await window.supabase.from(e.relationTable).select(e.relationField).eq(o,s);if(!c&&d.length>0){const l=d.map(u=>u[e.relationField]);i.forEach(u=>{l.includes(u.value)&&(u.selected=!0)})}}return i}catch(n){return console.error("❌ Fehler beim Laden der direkten Optionen:",n),[]}}updateSelectOptions(e,t,n){console.log("🔧 Update Select-Optionen für:",n.name,"mit",t.length,"Optionen"),e.innerHTML="";const a=document.createElement("option");if(a.value="",a.textContent=n.placeholder||"Bitte wählen...",e.appendChild(a),t.forEach(r=>{const i=document.createElement("option");i.value=r.value,i.textContent=r.label,r.selected&&(i.selected=!0),e.appendChild(i)}),e.dataset.searchable==="true"){console.log("🔧 Reinitialisiere Auto-Suggestion für:",n.name);const r=t.map(i=>({value:i.value,label:i.label}));this.reinitializeSearchableSelect(e,r,n);return}}reinitializeSearchableSelect(e,t,n){console.log("🔧 Reinitialisiere Searchable Select für:",n.name,"mit",t.length,"Optionen");const a=e.parentNode.querySelector(".searchable-select-container");a&&a.remove(),this.createSearchableSelect(e,t,n)}createSearchableSelect(e,t,n){console.log("🔧 Searchable Select erstellen für:",n.name)}getFormConfig(e){return null}}class Ke{updateSelectOptions(e,t,n){if(console.log("🔧 Update Select-Optionen für:",n.name,"mit",t.length,"Optionen"),e.dataset.searchable==="true"){console.log("🔧 Reinitialisiere Auto-Suggestion für:",n.name),this.reinitializeSearchableSelect(e,t,n);return}e.innerHTML="";const a=document.createElement("option");a.value="",a.textContent=n.placeholder||"Bitte wählen...",e.appendChild(a),t.forEach(r=>{const i=document.createElement("option");i.value=r.value,i.textContent=r.label,r.selected&&(i.selected=!0),e.appendChild(i)})}reinitializeSearchableSelect(e,t,n){const a=e.parentNode.querySelector(".searchable-select-container, .tag-based-select");a&&(console.log("🔧 Entferne bestehenden Container für:",n.name),a.remove()),e.style.display="none",this.createSearchableSelect(e,t,n)}createSearchableSelect(e,t,n){console.log("🔧 Searchable Select erstellen für:",n.name)}updateDropdownItems(e,t,n){e.innerHTML="",t.filter(r=>r.label.toLowerCase().includes(n.toLowerCase())).forEach(r=>{const i=document.createElement("div");i.className="searchable-select-item",i.textContent=r.label,i.style.cssText=`
        padding: 8px 12px;
        cursor: pointer;
        border-bottom: 1px solid #f3f4f6;
      `,i.addEventListener("click",()=>{const s=e.parentNode.parentNode.querySelector("select");s.value=r.value;const o=e.parentNode.querySelector("input");o.value=r.label,s.dispatchEvent(new Event("change")),e.classList.remove("show")}),i.addEventListener("mouseenter",()=>{i.style.backgroundColor="#f3f4f6"}),i.addEventListener("mouseleave",()=>{i.style.backgroundColor="white"}),e.appendChild(i)})}createTagBasedSelect(e,t,n){console.log(`🏷️ Erstelle Tag-basiertes Select für ${n.name} mit ${t.length} Optionen:`,t.slice(0,3));const a=document.createElement("div");a.className="searchable-select-container tag-based-select";const r=document.createElement("input");r.type="text",r.className="searchable-select-input",r.placeholder=n.placeholder||e.dataset?.placeholder||"Suchen und Tags hinzufügen...";const i=document.createElement("div");i.className="tags-container";const s=document.createElement("div");s.className="searchable-select-dropdown";const o=document.createElement("select");o.name=`${e.name}[]`,o.id=e.id+"_hidden",o.multiple=!0,o.style.display="none";const d=new Set,c=(h,g)=>{const b=document.createElement("div");b.className="tag";const v=document.createElement("span");v.textContent=g;const w=document.createElement("span");return w.textContent="×",w.className="tag-remove",w.addEventListener("click",()=>{d.delete(h),i.removeChild(b),l()}),b.appendChild(v),b.appendChild(w),b},l=()=>{o.innerHTML="",d.forEach(g=>{const b=document.createElement("option");b.value=g,b.selected=!0,o.appendChild(b)});const h=new Event("change",{bubbles:!0});o.dispatchEvent(h)},u=(h=t)=>{if(s.innerHTML="",h.length===0){const g=document.createElement("div");g.textContent="Keine Ergebnisse",g.className="searchable-select-empty",s.appendChild(g);return}h.forEach(g=>{if(d.has(g.value))return;const b=document.createElement("div");b.className="searchable-select-item",b.dataset.value=g.value;const v=document.createElement("span");if(v.className="branch-name",v.textContent=g.label,b.appendChild(v),g.description){const w=document.createElement("span");w.className="branch-description",w.textContent=g.description,b.appendChild(w)}b.classList.add("searchable-select-item"),b.addEventListener("mouseenter",()=>b.classList.add("hover")),b.addEventListener("mouseleave",()=>b.classList.remove("hover")),b.addEventListener("click",()=>{d.add(g.value);const w=c(g.value,g.label);i.appendChild(w),r.value="",s.classList.remove("show"),l(),console.log(`✅ Tag hinzugefügt: ${g.value}`)}),s.appendChild(b)})};r.addEventListener("focus",()=>{s.classList.add("show"),u()}),r.addEventListener("input",h=>{const g=h.target.value.toLowerCase(),b=t.filter(v=>v.label.toLowerCase().includes(g)&&!d.has(v.value));u(b)}),document.addEventListener("click",h=>{a.contains(h.target)||s.classList.remove("show")}),r.addEventListener("keydown",h=>{h.key==="Escape"&&s.classList.remove("show")}),a.appendChild(r),a.appendChild(i),a.appendChild(s),a.appendChild(o),e.style.display="none",e.parentNode.insertBefore(a,e);const m=a.closest("form");m&&!m.querySelector(`select[name="${o.name}"]`)&&m.appendChild(o),console.log(`✅ Tag-basierte Auto-Suggestion Select erstellt für ${n.name}`)}}let Ue=class{constructor(){this.config=new U,this.renderer=new ee,this.validator=new He,this.autoGeneration=new te,this.autoCalculation=new ne,this.dependentFields=new ae(this.autoGeneration),this.relationTables=new re,this.dataLoader=new ie,this.optionsManager=new Ke,this.formEvents=new qe(this),this.renderer.getFormConfig=this.config.getFormConfig.bind(this.config),this.relationTables.getFormConfig=this.config.getFormConfig.bind(this.config),this.dataLoader.getFormConfig=this.config.getFormConfig.bind(this.config),this.validator.getFormConfig=this.config.getFormConfig.bind(this.config),this.dependentFields.getFormConfig=this.config.getFormConfig.bind(this.config),this.dataLoader.createSearchableSelect=this.createSearchableSelect.bind(this),this.dataLoader.reinitializeSearchableSelect=this.reinitializeSearchableSelect.bind(this),this.optionsManager.createSearchableSelect=this.createSearchableSelect.bind(this),this.dependentFields.dynamicDataLoader=this.dataLoader,this.injectDataService(),this.currentForm=null}async openForm(e,t=null){try{const n=this.renderer.renderForm(e,t),a=document.createElement("div");a.className="modal-overlay",a.innerHTML=`
        <div class="modal-content">
          ${n}
        </div>
      `,document.body.appendChild(a),this.currentForm={entity:e,data:t,modal:a},await this.formEvents.bindFormEvents(e,t),console.log(`✅ Formular geöffnet: ${e}`)}catch(n){console.error("❌ Fehler beim Öffnen des Formulars:",n)}}closeForm(){this.currentForm&&this.currentForm.modal&&(document.body.removeChild(this.currentForm.modal),this.currentForm=null,console.log("✅ Formular geschlossen"))}renderFormOnly(e,t=null){return this.renderer.renderFormOnly(e,t)}async bindFormEvents(e,t){return this.formEvents.bindFormEvents(e,t)}injectDataService(){window.dataService?(this.dataLoader.setDataService(window.dataService),console.log("✅ DataService erfolgreich injiziert")):(console.warn("⚠️ DataService noch nicht verfügbar, wird später injiziert"),setTimeout(()=>this.injectDataService(),100))}async handleFormSubmit(e,t){try{const n=document.getElementById(`${e}-form`);if(!n)return;const a=new FormData(n),r={},i=n.querySelectorAll("input, select, textarea");for(const d of i){const c=d.name;if(!c)continue;let l="";d.type==="checkbox"?l=d.checked?d.value||"true":"":d.type==="radio"?d.checked&&(l=d.value):l=d.value||"",(l!==""||d.readOnly)&&(r[c]=l)}const s=this.validator.validateFormData(e,r);if(s.length>0){this.validator.showValidationErrors(s);return}let o;t&&t.id?o=await window.dataService.updateEntity(e,t.id,r):o=await window.dataService.createEntity(e,r),o.success?(await this.relationTables.handleRelationTables(e,o.id,r,n),e==="kampagne"&&await this.handleKampagneAddresses(o.id,n),e==="kooperation"&&await this.handleKooperationVideos(o.id,n),this.validator.showSuccessMessage(t?"Erfolgreich aktualisiert!":"Erfolgreich erstellt!"),this.closeForm(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:e,id:o.id,action:t?"updated":"created"}}))):this.validator.showErrorMessage(`Fehler beim ${t?"Aktualisieren":"Erstellen"}: ${o.error}`)}catch(n){console.error("❌ Fehler beim Formular-Submit:",n),this.validator.showErrorMessage("Ein unerwarteter Fehler ist aufgetreten.")}}async handleKampagneAddresses(e,t){try{const n=t.querySelector(".addresses-list");if(!n)return;const a=n.querySelectorAll(".address-item"),r=[];if(a.forEach(i=>{const s=i.dataset.addressId,o={kampagne_id:e,adressname:t.querySelector(`input[name="adressname_${s}"]`)?.value||"",strasse:t.querySelector(`input[name="strasse_${s}"]`)?.value||"",hausnummer:t.querySelector(`input[name="hausnummer_${s}"]`)?.value||"",plz:t.querySelector(`input[name="plz_${s}"]`)?.value||"",stadt:t.querySelector(`input[name="stadt_${s}"]`)?.value||"",land:t.querySelector(`input[name="land_${s}"]`)?.value||"",notiz:t.querySelector(`textarea[name="notiz_${s}"]`)?.value||""};o.adressname.trim()&&r.push(o)}),r.length>0){await window.supabase.from("kampagne_adressen").delete().eq("kampagne_id",e);const{error:i}=await window.supabase.from("kampagne_adressen").insert(r);i?console.error("❌ Fehler beim Speichern der Adressen:",i):console.log(`✅ ${r.length} Adressen für Kampagne ${e} gespeichert`)}}catch(n){console.error("❌ Fehler beim Verarbeiten der Kampagnen-Adressen:",n)}}setupAddressesFields(e){e.querySelectorAll(".addresses-container").forEach(n=>{const a=n.querySelector(".add-address-btn"),r=n.querySelector(".addresses-list");a&&r&&a.addEventListener("click",()=>{this.renderer.addAddressRow(r)})})}async handleKooperationVideos(e,t){try{if(!window.supabase)return;const n=t.querySelector(".videos-list");if(!n)return;const r=Array.from(n.querySelectorAll(".video-item")).map((i,s)=>{const o=i.getAttribute("data-video-id"),d=t.querySelector(`select[name="video_content_art_${o}"]`)?.value||null;return{kooperation_id:e,content_art:d,titel:null,asset_url:null,kommentar:null,position:s+1,created_at:new Date().toISOString(),updated_at:new Date().toISOString()}}).filter(i=>i.content_art||i.titel||i.asset_url||i.kommentar);if(await window.supabase.from("kooperation_videos").delete().eq("kooperation_id",e),r.length>0){const{data:i,error:s}=await window.supabase.from("kooperation_videos").insert(r).select("id, content_art, position");s?console.error("❌ Fehler beim Speichern der Kooperation-Videos:",s):(console.log(`✅ ${i?.length||0} Videos für Kooperation ${e} gespeichert`,i),(!i||i.length===0)&&console.warn("⚠️ Insert meldete Erfolg, aber keine Zeilen wurden zurückgegeben. Prüfe RLS/Policies für kooperation_videos."))}}catch(n){console.error("❌ Fehler in handleKooperationVideos:",n)}}initializeSearchableSelects(e){e.querySelectorAll('select[data-searchable="true"]').forEach(n=>{this.createSearchableSelect(n,[],{placeholder:n.dataset.placeholder||"Bitte wählen...",type:n.multiple?"multiselect":"select",tagBased:n.dataset.tagBased==="true"})})}createSearchableSelect(e,t,n){return(n?.type==="multiselect"||e.multiple)&&(n?.tagBased===!0||e.dataset.tagBased==="true")?((!t||t.length===0)&&(t=Array.from(e.options).slice(1).map(r=>({value:r.value,label:r.textContent}))),this.optionsManager.createTagBasedSelect(e,t,n)):((!t||t.length===0)&&(t=Array.from(e.options).slice(1).map(r=>({value:r.value,label:r.textContent}))),this.createSimpleSearchableSelect(e,t,n))}createSimpleSearchableSelect(e,t,n){const a=e.parentNode.querySelector(".searchable-select-container");a&&a.remove();const r=document.createElement("div");r.className="searchable-select-container";const i=document.createElement("input");i.type="text",i.className="searchable-select-input",i.placeholder=n.placeholder||"Suchen...";const s=document.createElement("div");s.className="searchable-select-dropdown",e.style.display="none",e.parentNode.insertBefore(r,e),r.appendChild(i),r.appendChild(s),i.addEventListener("focus",()=>{s.classList.add("show"),this.updateDropdownItems(s,t,i.value)}),i.addEventListener("blur",()=>{setTimeout(()=>{s.classList.remove("show")},200)}),i.addEventListener("input",()=>{this.updateDropdownItems(s,t,i.value)}),this.updateDropdownItems(s,t,"")}updateDropdownItems(e,t,n){e.innerHTML="",t.filter(r=>r.label.toLowerCase().includes(n.toLowerCase())).forEach(r=>{const i=document.createElement("div");i.className="searchable-select-item",i.textContent=r.label,i.addEventListener("click",()=>{const s=e.parentNode.parentNode.querySelector("select");s.value=r.value;const o=e.parentNode.querySelector("input");o.value=r.label,s.dispatchEvent(new Event("change")),e.classList.remove("show")}),i.addEventListener("mouseenter",()=>i.classList.add("hover")),i.addEventListener("mouseleave",()=>i.classList.remove("hover")),e.appendChild(i)})}reinitializeSearchableSelect(e,t,n){this.createSearchableSelect(e,t,n)}validateFormData(e,t){const n=[],a=this.config.getFormConfig(e);return a&&a.fields.forEach(r=>{if(r.required&&(!t[r.name]||t[r.name].toString().trim()==="")&&n.push(`${r.label} ist erforderlich.`),r.validation&&t[r.name]){const i=t[r.name];switch(r.validation.type){case"text":r.validation.minLength&&i.length<r.validation.minLength&&n.push(`${r.label} muss mindestens ${r.validation.minLength} Zeichen lang sein.`);break;case"number":r.validation.min!==void 0&&parseFloat(i)<r.validation.min&&n.push(`${r.label} muss mindestens ${r.validation.min} sein.`),r.validation.max!==void 0&&parseFloat(i)>r.validation.max&&n.push(`${r.label} darf maximal ${r.validation.max} sein.`);break;case"email":/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(i)||n.push(`${r.label} muss eine gültige E-Mail-Adresse sein.`);break;case"url":try{new URL(i)}catch{n.push(`${r.label} muss eine gültige URL sein.`)}break}}}),n}showValidationErrors(e){const t=e.join(`
`);alert(`Validierungsfehler:
${t}`)}showSuccessMessage(e){console.log(`✅ ${e}`)}showErrorMessage(e){console.error(`❌ ${e}`),alert(e)}};const se=new Ue;typeof window<"u"&&(window.formSystem=se);class Oe{constructor(){this.currentEntity=null,this.currentEntityId=null}async init(e,t){console.log("📝 NOTIZENSYSTEM: Initialisiere für",e,t),this.currentEntity=e,this.currentEntityId=t}async loadNotizen(e,t){console.log("🔄 NOTIZENSYSTEM: Lade Notizen für",e,t);try{const n=this.getNotizenTable(e);if(!n)return console.warn("⚠️ NOTIZENSYSTEM: Keine Notizen-Tabelle für",e),[];let a="*";n==="creator_notiz"&&(a+=`,
          kampagne:kampagne_id (
            id,
            kampagnenname,
            unternehmen:unternehmen_id (
    ...
    `),this.loadKampagnenForSelect(),this.bindStarEvents(),this.bindBewertungFormEvents(e,t)}showEditBewertungModal(e,t){console.log("✏️ BEWERTUNGSSYSTEM: Zeige Edit Bewertung Modal für",e,t),console.log("TODO: Edit Bewertung Modal implementieren")}async loadKampagnenForSelect(){try{const{data:e,error:t}=await window.supabase.from("kampagne").select(`
      ...
      <tr data-id="${o||""}">
        ...
        <td>${o?`<a href="#" class="table-link" data-table="creator" data-id="${o}">${d}</a>`:d}</td>
        <td>${c}</td>
        <td>${l}</td>
        <td>${u}</td>
        <td>${m}</td>
  ...
  `}class Ze{constructor(){this.unternehmenId=null,this.unternehmen=null,this.notizen=[],this.ratings=[],this.marken=[],this.auftraege=[],this.ansprechpartner=[],this.kampagnen=[],this.kooperationen=[],this.creators=[],this.rechnungen=[],this._creatorMap={}}async init(e){console.log("🎯 UNTERNEHMENDETAIL: Initialisiere Unternehmen-Detailseite für ID:",e);try{this.unternehmenId=e,await this.loadUnternehmenData(),this.render(),this.bindEvents(),console.log("✅ UNTERNEHMENDETAIL: Initialisierung abgeschlossen")}catch(t){console.error("❌ UNTERNEHMENDETAIL: Fehler bei der Initialisierung:",t),window.ErrorHandler.handle(t,"UnternehmenDetail.init")}}async loadUnternehmenData(){console.log("🔄 UNTERNEHMENDETAIL: Lade Unternehmen-Daten...");try{const{data:e,error:t}=await window.supabase.from("unternehmen").select("*").eq("id",this.unternehmenId).single();if(t)throw t;this.unternehmen=e,console.log("✅ UNTERNEHMENDETAIL: Unternehmen-Basisdaten geladen:",this.unternehmen),window.notizenSystem&&(this.notizen=await window.notizenSystem.loadNotizen("unternehmen",this.unternehmenId),console.log("✅ UNTERNEHMENDETAIL: Notizen geladen:",this.notizen.length)),window.bewertungsSystem&&(this.ratings=await window.bewertungsSystem.loadBewertungen("unternehmen",this.unternehmenId),console.log("✅ UNTERNEHMENDETAIL: Ratings geladen:",this.ratings.length));const{data:n,error:a}=await window.supabase.from("marke").select("*").eq("unternehmen_id",this.unternehmenId);a||(this.marken=n||[],console.log("✅ UNTERNEHMENDETAIL: Marken geladen:",this.marken.length));const{data:r,error:i}=await window.supabase.from("auftrag").select("*").eq("unternehmen_id",this.unternehmenId);i||(this.auftraege=r||[],console.log("✅ UNTERNEHMENDETAIL: Aufträge geladen:",this.auftraege.length));try{const{data:d}=await window.supabase.from("kampagne").select("id, kampagnenname, status, start, deadline, unternehmen_id").eq("unternehmen_id",this.unternehmenId).order("created_at",{ascending:!1});this.kampagnen=d||[],console.log("✅ UNTERNEHMENDETAIL: Kampagnen geladen:",this.kampagnen.length)}catch(d){console.warn("⚠️ Kampagnen konnten nicht geladen werden",d),this.kampagnen=[]}try{const{data:d,error:c}=await window.supabase.from("rechnung").select("id, rechnung_nr, status, nettobetrag, bruttobetrag, gestellt_am, zahlungsziel, bezahlt_am, pdf_url").eq("unternehmen_id",this.unternehmenId).order("gestellt_am",{ascending:!1});if(c)throw c;this.rechnungen=d||[]}catch(d){console.warn("⚠️ Rechnungen konnten nicht geladen werden",d),this.rechnungen=[]}try{const d=(this.kampagnen||[]).map(c=>c.id).filter(Boolean);if(d.length>0){const{data:c}=await window.supabase.from("kooperationen").select("id, name, status, videoanzahl, gesamtkosten, kampagne_id, creator_id, created_at").in("kampagne_id",d).order("created_at",{ascending:!1});this.kooperationen=c||[];const l=Array.from(new Set((this.kooperationen||[]).map(u=>u.creator_id).filter(Boolean)));if(l.length>0){const{data:u}=await window.supabase.from("creator").select("id, vorname, nachname, instagram, lieferadresse_strasse, lieferadresse_hausnummer, lieferadresse_plz, lieferadresse_stadt, lieferadresse_land").in("id",l);this.creators=u||[]}else this.creators=[];console.log("✅ UNTERNEHMENDETAIL: Kooperationen geladen:",this.kooperationen.length,"Creators:",this.creators.length)}else this.kooperationen=[],this.creators=[]}catch(d){console.warn("⚠️ Kooperationen/Creators konnten nicht geladen werden",d),this.kooperationen=[],this.creators=[]}const{data:s,error:o}=await window.supabase.from("ansprechpartner").select("*").eq("unternehmen_id",this.unternehmenId);o||(this.ansprechpartner=s||[],console.log("✅ UNTERNEHMENDETAIL: Ansprechpartner geladen:",this.ansprechpartner.length));try{const{data:d}=await window.supabase.from("kampagne").select("id").eq("unternehmen_id",this.unternehmenId),c=(d||[]).map(u=>u.id);let l=[];if(c.length>0){const{data:u}=await window.supabase.from("kooperationen").select("creator_id").in("kampagne_id",c);l=Array.from(new Set((u||[]).map(m=>m.creator_id).filter(Boolean)))}if(l.length>0){const{data:u}=await window.supabase.from("creator").select("id, vorname, nachname, instagram_follower, tiktok_follower, lieferadresse_stadt, lieferadresse_land").in("id",l);this.creators=u||[],this._creatorMap=(u||[]).reduce((m,h)=>(m[h.id]=h,m),{})}else this.creators=[],this._creatorMap={}}catch(d){console.warn("⚠️ Creator für Unternehmen konnten nicht geladen werden",d),this.creators=[],this._creatorMap={}}}catch(e){throw console.error("❌ UNTERNEHMENDETAIL: Fehler beim Laden der Unternehmen-Daten:",e),e}}render(){window.setHeadline(`${this.unternehmen?.firmenname||"Unternehmen"} - Details`);const e=`
    ...
    `).join("")}</div>`}renderCreators(){return!this.creators||this.creators.length===0?`
      ...
      `:N(this.creators)}renderAnsprechpartner(){return!this.ansprechpartner||this.ansprechpartner.length===0?`
        <div class="empty-state">
          <div class="empty-icon">👥</div>
          <h3>Keine Ansprechpartner vorhanden</h3>
    ...
    `,window.formSystem.bindFormEvents("unternehmen",this.unternehmen);const t=document.getElementById("unternehmen-form");t&&(t.onsubmit=async n=>{n.preventDefault(),await this.handleEditFormSubmit()})}async handleEditFormSubmit(){try{const e=document.getElementById("unternehmen-form"),t=new FormData(e),n={},a={};for(let[s,o]of t.entries())a[s]?(Array.isArray(a[s])||(a[s]=[a[s]]),a[s].push(o)):a[s]=o;for(let[s,o]of Object.entries(a))n[s]=(Array.isArray(o),o);const r=window.validatorSystem.validateForm(n,{firmenname:{type:"text",minLength:2,required:!0}});if(!r.isValid){this.showValidationErrors(r.errors);return}const i=await window.dataService.updateEntity("unternehmen",this.unternehmenId,n);if(i.success)this.showSuccessMessage("Unternehmen erfolgreich aktualisiert!"),setTimeout(async()=>{await this.loadUnternehmenData(),this.render(),this.bindEvents()},1500);else throw new Error(i.error||"Unbekannter Fehler")}catch(e){console.error("❌ Formular-Submit Fehler:",e),this.showErrorMessage(e.message)}}showValidationErrors(e){console.log("❌ Validierungsfehler:",e),document.querySelectorAll(".validation-error").forEach(t=>t.remove()),Object.keys(e).forEach(t=>{const n=document.querySelector(`[name="${t}"]`);if(n){const a=document.createElement("div");a.className="validation-error",a.textContent=e[t],a.style.color="#dc3545",a.style.fontSize="0.875rem",a.style.marginTop="0.25rem",n.parentNode.appendChild(a),n.style.borderColor="#dc3545"}})}showSuccessMessage(e){const t=document.createElement("div");t.className="alert alert-success",t.textContent=e,t.style.cssText=`
      background: #d4edda;
      color: #155724;
      padding: 1rem;
      border-radius: 4px;
    ...
    `;const n=document.querySelector(".form-page");n&&n.insertBefore(t,n.firstChild)}async getBranchenNamen(e){try{const{data:t,error:n}=await window.supabase.from("branchen").select("id, name").in("id",e);return n?(console.error("❌ Fehler beim Laden der Branche-Namen:",n),e):e.map(a=>{const r=t.find(i=>i.id===a);return r?r.name:a})}catch(t){return console.error("❌ Fehler beim Laden der Branche-Namen:",t),e}}destroy(){console.log("UnternehmenDetail: Cleaning up...")}}const oe=new Ze;class Ge{constructor(){this.auftragId=null,this.auftrag=null,this.notizen=[],this.ratings=[],this.creator=[],this.marke=null,this.unternehmen=null,this.rechnungen=[],this.rechnungSummary={count:0,sumNetto:0,sumBrutto:0,paidCount:0,openCount:0},this.koopSummary={count:0,sumNetto:0,sumGesamt:0}}async init(e){console.log("🎯 AUFTRAGDETAIL: Initialisiere Auftrags-Detailseite für ID:",e);try{this.auftragId=e,await this.loadAuftragData(),this.render(),this.bindEvents(),console.log("✅ AUFTRAGDETAIL: Initialisierung abgeschlossen")}catch(t){console.error("❌ AUFTRAGDETAIL: Fehler bei der Initialisierung:",t),window.ErrorHandler.handle(t,"AuftragDetail.init")}}async loadAuftragData(){console.log("🔄 AUFTRAGDETAIL: Lade Auftrags-Daten...");try{const{data:e,error:t}=await window.supabase.from("auftrag").select(`
        ...
        `).eq("id",this.auftragId).single();if(t)throw t;this.auftrag=e,console.log("✅ AUFTRAGDETAIL: Auftrags-Basisdaten geladen:",this.auftrag),window.notizenSystem&&(this.notizen=await window.notizenSystem.loadNotizen("auftrag",this.auftragId),console.log("✅ AUFTRAGDETAIL: Notizen geladen:",this.notizen.length)),window.bewertungsSystem&&(this.ratings=await window.bewertungsSystem.loadBewertungen("auftrag",this.auftragId),console.log("✅ AUFTRAGDETAIL: Ratings geladen:",this.ratings.length));const{data:n,error:a}=await window.supabase.from("creator_auftrag").select(`
          creator:creator_id(*)
        `).eq("auftrag_id",this.auftragId);a||(this.creator=n?.map(r=>r.creator)||[],console.log("✅ AUFTRAGDETAIL: Creator geladen:",this.creator.length));try{const{data:r}=await window.supabase.from("rechnung").select("id, rechnung_nr, status, nettobetrag, bruttobetrag, gestellt_am, bezahlt_am, pdf_url").eq("auftrag_id",this.auftragId).order("gestellt_am",{ascending:!1});this.rechnungen=r||[];const i=(this.rechnungen||[]).reduce((c,l)=>c+(parseFloat(l.nettobetrag)||0),0),s=(this.rechnungen||[]).reduce((c,l)=>c+(parseFloat(l.bruttobetrag)||0),0),o=(this.rechnungen||[]).filter(c=>c.status==="Bezahlt").length,d=(this.rechnungen||[]).filter(c=>c.status!=="Bezahlt").length;this.rechnungSummary={count:(this.rechnungen||[]).length,sumNetto:i,sumBrutto:s,paidCount:o,openCount:d}}catch{this.rechnungen=[],this.rechnungSummary={count:0,sumNetto:0,sumBrutto:0,paidCount:0,openCount:0}}try{const{data:r}=await window.supabase.from("kampagne").select("id").eq("auftrag_id",this.auftragId),i=(r||[]).map(s=>s.id);if(i.length>0){const{data:s}=await window.supabase.from("kooperationen").select("nettobetrag, gesamtkosten").in("kampagne_id",i),o=(s||[]).reduce((c,l)=>c+(parseFloat(l.nettobetrag)||0),0),d=(s||[]).reduce((c,l)=>c+(parseFloat(l.gesamtkosten)||0),0);this.koopSummary={count:(s||[]).length,sumNetto:o,sumGesamt:d}}else this.koopSummary={count:0,sumNetto:0,sumGesamt:0}}catch{this.koopSummary={count:0,sumNetto:0,sumGesamt:0}}}catch(e){throw console.error("❌ AUFTRAGDETAIL: Fehler beim Laden der Auftrags-Daten:",e),e}}render(){window.setHeadline(`${this.auftrag?.auftragsname||"Auftrag"} - Details`);const e=`
      <div class="page-header">
        <div class="page-header-left">
    ...
    `;window.setContentSafely(window.content,e)}renderBudget(){const e=d=>d||d===0?new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(d):"-",t=d=>d||d===0?new Intl.NumberFormat("de-DE").format(d):"-",n=this.auftrag||{},a=n.ust_prozent!=null?n.ust_prozent:19,r=n.ust_betrag!=null?n.ust_betrag:parseFloat(n.nettobetrag||0)*(parseFloat(a)/100),i=n.deckungsbeitrag_prozent!=null?n.deckungsbeitrag_prozent:0,s=n.deckungsbeitrag_betrag!=null?n.deckungsbeitrag_betrag:parseFloat(n.nettobetrag||0)*(parseFloat(i)/100),o=parseFloat(n.influencer||0)*parseFloat(n.influencer_preis||0)+parseFloat(n.ugc||0)*parseFloat(n.ugc_preis||0)+parseFloat(n.vor_ort_produktion||0)*parseFloat(n.vor_ort_preis||0);return`
      <div class="detail-section">
        <div class="detail-grid">
          <div class="detail-card">
    ...
    `;const n=document.querySelector(".form-page");n&&n.insertBefore(t,n.firstChild)}showErrorMessage(e){const t=document.createElement("div");t.className="alert alert-danger",t.textContent=e,t.style.cssText=`
      ...
      margin-bottom: 1rem;
      border: 1px solid #f5c6cb;
    `;const n=document.querySelector(".form-page");n&&n.insertBefore(t,n.firstChild)}destroy(){console.log("AuftragDetail: Cleaning up...")}}const le=new Ge;class We{constructor(){this.selectedKooperation=new Set,this._boundEventListeners=new Set,this.statusOptions=[]}async init(){if(window.setHeadline("Kooperationen Übersicht"),!(window.canViewPage&&window.canViewPage("kooperation")||await window.checkUserPermission("kooperation","can_view"))){window.content.innerHTML=`
        <div class="error-message">
          <p>Sie haben keine Berechtigung, Kooperationen anzuzeigen.</p>
        </div>
      `;return}this.bindEvents(),await this.loadAndRender()}async loadAndRender(){try{const e=await window.dataService.loadFilterData("kooperation");await this.render(e),await this.initializeFilterBar();const t=k.getFilters("kooperation");console.log("🔍 Lade Kooperationen mit Filter:",t);const n=await this.loadKooperationenWithRelations();console.log("📊 Kooperationen geladen:",n?.length||0),this.updateTable(n)}catch(e){window.ErrorHandler.handle(e,"KooperationList.loadAndRender")}}async loadKooperationenWithRelations(){try{if(!window.supabase)return console.warn("⚠️ Supabase nicht verfügbar - verwende Mock-Daten"),await window.dataService.loadEntities("kooperation");try{const{data:l}=await window.supabase.from("kampagne_status").select("id, name, sort_order").order("sort_order",{ascending:!0}).order("name",{ascending:!0});this.statusOptions=l||[]}catch(l){console.warn("⚠️ Konnte Status-Optionen nicht laden",l),this.statusOptions=[]}const e=window.currentUser?.rolle==="admin";let t=[];if(!e)try{const{data:l}=await window.supabase.from("kampagne_mitarbeiter").select("kampagne_id").eq("mitarbeiter_id",window.currentUser?.id);t=(l||[]).map(u=>u.kampagne_id).filter(Boolean)}catch{}let n=window.supabase.from("kooperationen").select("id, name, status, status_id, videoanzahl, gesamtkosten, kampagne_id, creator_id, assignee_id, skript_deadline, content_deadline, created_at").order("created_at",{ascending:!1});e||(n=n.or(`assignee_id.eq.${window.currentUser?.id}${t.length?`,kampagne_id.in.(${t.join(",")})`:""}`));const{data:a,error:r}=await n;if(r)throw console.error("❌ Fehler beim Laden der Kooperationen mit Beziehungen:",r),r;const i=Array.from(new Set((a||[]).map(l=>l.kampagne_id).filter(Boolean))),s=Array.from(new Set((a||[]).map(l=>l.creator_id).filter(Boolean)));let o={},d={};try{if(i.length>0){const{data:l}=await window.supabase.from("kampagne").select("id, kampagnenname, status, start, deadline").in("id",i);o=(l||[]).reduce((u,m)=>(u[m.id]=m,u),{})}if(s.length>0){const{data:l}=await window.supabase.from("creator").select("id, vorname, nachname, instagram").in("id",s);d=(l||[]).reduce((u,m)=>(u[m.id]=m,u),{})}}catch(l){console.warn("⚠️ Beziehungen konnten nicht vollständig geladen werden:",l)}const c=(a||[]).map(l=>({...l,creator:d[l.creator_id]||null,kampagne:o[l.kampagne_id]||null}));return this._kampagneMap=o,this._creatorMap=d,console.log("✅ Kooperationen mit Beziehungen geladen:",c),c}catch(e){return console.error("❌ Fehler beim Laden der Kooperationen mit Beziehungen:",e),await window.dataService.loadEntities("kooperation")}}async render(e){const t=window.currentUser?.permissions?.kooperation?.can_edit||!1,n=k.getFilters("kooperation");Object.entries(n).forEach(([i,s])=>{});let a=`<div class="filter-bar">
      <div class="filter-left">
        <div id="filter-container"></div>
      </div>
      <div class="filter-right">
    ...
    `;window.setContentSafely(window.content,r)}async initializeFilterBar(){const e=document.getElementById("filter-container");e&&await k.renderFilterBar("kooperation",e,t=>this.onFiltersApplied(t),()=>this.onFiltersReset())}onFiltersApplied(e){console.log("🔍 KooperationList: Filter angewendet:",e),k.applyFilters("kooperation",e),this.loadAndRender()}onFiltersReset(){console.log("🔄 KooperationList: Filter zurückgesetzt"),k.resetFilters("kooperation"),this.loadAndRender()}bindEvents(){this.boundFilterResetHandler=e=>{e.target.id==="btn-filter-reset"&&this.onFiltersReset()},document.addEventListener("click",this.boundFilterResetHandler),document.addEventListener("click",e=>{(e.target.id==="btn-kooperation-new"||e.target.id==="btn-kooperation-new-filter")&&(e.preventDefault(),window.navigateTo("/kooperation/new"))}),document.addEventListener("click",e=>{if(e.target.id==="btn-select-all"){e.preventDefault(),document.querySelectorAll(".kooperation-check").forEach(a=>{a.checked=!0,a.dataset.id&&this.selectedKooperation.add(a.dataset.id)});const n=document.getElementById("select-all-kooperationen");n&&(n.indeterminate=!1,n.checked=!0),this.updateSelection()}}),document.addEventListener("click",e=>{if(e.target.id==="btn-deselect-all"){e.preventDefault(),document.querySelectorAll(".kooperation-check").forEach(a=>{a.checked=!1}),this.selectedKooperation.clear();const n=document.getElementById("select-all-kooperationen");n&&(n.indeterminate=!1,n.checked=!1),this.updateSelection()}}),document.addEventListener("click",e=>{if(e.target.classList.contains("table-link")&&e.target.dataset.table==="kooperation"){e.preventDefault();const t=e.target.dataset.id;console.log("🎯 KOOPERATIONLIST: Navigiere zu Kooperation Details:",t),window.navigateTo(`/kooperation/${t}`)}}),window.addEventListener("entityUpdated",e=>{e.detail.entity==="kooperation"&&this.loadAndRender()}),document.addEventListener("click",e=>{if(e.target.classList.contains("tag-x")){e.preventDefault(),e.stopPropagation();const n=e.target.closest(".filter-tag").dataset.key,a=k.getFilters("kooperation");delete a[n],k.applyFilters("kooperation",a),this.loadAndRender()}}),document.addEventListener("change",e=>{e.target.id==="select-all-kooperationen"&&(document.querySelectorAll(".kooperation-check").forEach(n=>{n.checked=e.target.checked,e.target.checked?this.selectedKooperation.add(n.dataset.id):this.selectedKooperation.delete(n.dataset.id)}),this.updateSelection())}),document.addEventListener("change",e=>{e.target.classList.contains("kooperation-check")&&(e.target.checked?this.selectedKooperation.add(e.target.dataset.id):this.selectedKooperation.delete(e.target.dataset.id),this.updateSelection())})}hasActiveFilters(){const e=k.getFilters("kooperation");return Object.keys(e).length>0}updateSelection(){const e=this.selectedKooperation.size,t=document.getElementById("selected-count"),n=document.getElementById("btn-deselect-all"),a=document.getElementById("btn-delete-selected");t&&(t.textContent=`${e} ausgewählt`,t.style.display=e>0?"inline":"none"),n&&(n.style.display=e>0?"inline-block":"none"),a&&(a.style.display=e>0?"inline-block":"none")}async updateTable(e){const t=document.getElementById("kooperationen-table-body");if(!t)return;if(!e||e.length===0){const{renderEmptyState:a}=await x(async()=>{const{renderEmptyState:r}=await import("./core-C7Vz5Umf.js").then(i=>i.F);return{renderEmptyState:r}},[]);a(t);return}const n=e.map(a=>{const r=s=>s?new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(s):"-",i=s=>s?new Date(s).toLocaleDateString("de-DE"):"-";return`
        <tr data-id="${a.id}">
            ...
            <div class="actions-dropdown-container" data-entity-type="kooperation">
              ...
              <div class="actions-dropdown">
                <div class="action-submenu">
                  <a href="#" class="action-item has-submenu" data-submenu="status">${z.getHeroIcon("invoice")}<span>Status ändern</span></a>
                  <div class="submenu" data-submenu="status">
                    ${(this.statusOptions||[]).map(s=>`
                      <a href="#" class="submenu-item" data-action="set-field" data-field="status_id" data-value="${s.id}" data-status-name="${s.name.replace(/\"/g,'\\"')}" data-id="${a.id}">${z.getStatusIcon(s.name)}<span>${s.name}</span>${a.status_id===s.id||a.status===s.name?'<span class=\\"submenu-check\\">'+z.getHeroIcon("check")+"</span>":""}</a>
                    `).join("")}
                  </div>
                </div>
                <a href="#" class="action-item" data-action="view" data-id="${a.id}">
    ...
    `}renderVideos(){return!this.videos||this.videos.length===0?'<p class="empty-state">Keine Videos angelegt.</p>':`
      <div class="data-table-container">
        <table class="data-table">
          ...
          <tbody>${this.videos.map(t=>{const n=r=>!r||r.length===0?"-":r.map(i=>{const s=i.created_at?new Date(i.created_at).toLocaleDateString("de-DE"):"",o=i.author_name||"",d=window.validatorSystem.sanitizeHtml(i.text||"");return`<div class="fb-line"><span class="fb-meta">${o}${s?" • "+s:""}</span><div class="fb-text">${d}</div></div>`}).join(""),a=`
    ...
    `}showValidationErrors(e){console.error("❌ Validierungsfehler:",e),document.querySelectorAll(".validation-error").forEach(t=>t.remove()),Object.entries(e).forEach(([t,n])=>{const a=document.querySelector(`[name="${t}"]`);if(a){const r=document.createElement("div");r.className="validation-error",r.textContent=n,a.parentNode.appendChild(r)}})}showSuccessMessage(e){const t=document.createElement("div");t.className="alert alert-success",t.textContent=e;const n=document.getElementById("kooperation-form");n&&(n.parentNode.insertBefore(t,n),setTimeout(()=>{t.remove()},5e3))}showErrorMessage(e){const t=document.createElement("div");t.className="alert alert-danger",t.textContent=e;const n=document.getElementById("kooperation-form");n&&(n.parentNode.insertBefore(t,n),setTimeout(()=>{t.remove()},5e3))}formatNumber(e){return e?new Intl.NumberFormat("de-DE").format(e):"-"}formatCurrency(e){return e?new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(e):"-"}formatDate(e){return e?new Date(e).toLocaleDateString("de-DE"):"-"}destroy(){console.log("KooperationDetail: Cleaning up...")}}const ce=new Ye,ue={videoId:null,video:null,kooperation:null,comments:[],assets:[],async init(p){try{if(this.videoId=p==="new"?null:p,!this.videoId){window.setHeadline("Neues Video"),window.content.innerHTML='<p class="empty-state">Neuanlage für Videos ist noch nicht implementiert.</p>';return}await this.loadData(),this.render(),this.bindEvents()}catch(e){console.error("KooperationVideoDetail init error:",e),window.notificationSystem?.error?.("Video-Detail konnte nicht geladen werden.")}},async loadData(){const{data:p,error:e}=await window.supabase.from("kooperation_videos").select("id, kooperation_id, titel, content_art, asset_url, kommentar, status, position, created_at").eq("id",this.videoId).single();if(e)throw e;this.video=p;try{const{data:t}=await window.supabase.from("kooperationen").select("id, name, kampagne:kampagne_id(id, kampagnenname)").eq("id",p.kooperation_id).single();this.kooperation=t||null}catch{this.kooperation=null}try{const{data:t}=await window.supabase.from("kooperation_video_comment").select("id, video_id, runde, text, author_name, author_benutzer_id, created_at").eq("video_id",this.videoId).order("created_at",{ascending:!0});this.comments=t||[]}catch{this.comments=[]}try{const{data:t}=await window.supabase.from("kooperation_video_asset").select("id, file_url, file_path, created_at").eq("video_id",this.videoId).order("created_at",{ascending:!0});this.assets=t||[]}catch{this.assets=[]}},render(){const p=this.video||{},e=p.titel||`Video #${p.id}`,t=this.kooperation?.name||"-",n=this.kooperation?.kampagne?.kampagnenname||"-",a=m=>window.validatorSystem?.sanitizeHtml?.(m)??m,r=m=>m?new Date(m).toLocaleString("de-DE"):"-";typeof window.setHeadline=="function"&&window.setHeadline(`Video: ${a(e)}`);const i=window.currentUser?.permissions?.kooperation?.can_edit||window.currentUser?.rolle==="admin",s=p.asset_url||"",o=/\.(mp4|webm|ogg)(\?|$)/i.test(s),d=s?o?`<video src="${s}" controls style="width:100%;max-height:60vh;border-radius:8px;"></video>`:`<a href="${s}" target="_blank" rel="noopener" class="primary-btn">Asset öffnen</a>`:'<p class="empty-state">Kein Asset hinterlegt.</p>',c={r1:[],r2:[]};(this.comments||[]).forEach(m=>{const h=m.runde===2||m.runde==="2"?"r2":"r1";c[h].push(m)});const l=this.assets.length?this.assets.map(m=>`<li><a href="${m.file_url||"#"}" target="_blank" rel="noopener">${m.file_path||m.file_url}</a></li>`).join(""):"<li>Keine weiteren Assets.</li>",u=`
      <div class="page-header">
        <div class="page-header-left">
          <h1>${a(e)}</h1>
        ...
        <div class="page-header-right">
          ${p.kooperation_id?'<button id="btn-back-kooperation" class="secondary-btn">Zur Kooperation</button>':""}
        </div>
      </div>
      ...
      <div class="content-section">
        <div class="detail-grid">
          <div class="detail-card">
            <h3>Asset</h3>
            ${d}
            ${this.assets.length?`<div class="asset-list"><h4>Zusätzliche Assets</h4><ul>${l}</ul></div>`:""}
          </div>

          <div class="detail-card">
            <h3>Informationen</h3>
            <div class="detail-grid-2">
              <div class="detail-item"><label>Content-Art</label><span>${a(p.content_art||"-")}</span></div>
              <div class="detail-item"><label>Position</label><span>${p.position||"-"}</span></div>
              <div class="detail-item"><label>Status</label>
                <span class="status-badge status-${(p.status||"produktion").toLowerCase()}">${p.status==="abgeschlossen"?"Abgeschlossen":"Produktion"}</span>
              </div>
              <div class="detail-item"><label>Erstellt</label><span>${r(p.created_at)}</span></div>
            </div>
            ${i?`
            <div class="form-inline" style="margin-top:12px;gap:8px;display:flex;align-items:center;">
              ...
              <select id="video-status" class="form-input" style="max-width:220px;">
                <option value="produktion" ${p.status!=="abgeschlossen"?"selected":""}>Produktion</option>
    ...
    `;window.setContentSafely(window.content,u)},renderCommentsTable(p){const e=a=>window.validatorSystem?.sanitizeHtml?.(a)??a,t=a=>a?new Date(a).toLocaleString("de-DE"):"-";return!p||p.length===0?'<p class="empty-state">Keine Kommentare vorhanden.</p>':`
      <div class="data-table-container">
        <table class="data-table">
          <thead>
            <tr>
    ...
    `},bindEvents(){document.getElementById("btn-back-kooperation")?.addEventListener("click",n=>{n.preventDefault(),this.video?.kooperation_id&&window.navigateTo(`/kooperation/${this.video.kooperation_id}`)});const p=document.getElementById("btn-save-status");p&&p.addEventListener("click",async n=>{n.preventDefault();try{const r=document.getElementById("video-status")?.value||"produktion",{error:i}=await window.supabase.from("kooperation_videos").update({status:r,updated_at:new Date().toISOString()}).eq("id",this.videoId);if(i)throw i;await this.loadData(),this.render(),this.bindEvents(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:"kooperation_videos",action:"updated",id:this.videoId,field:"status",value:r}}))}catch(a){console.error("Status speichern fehlgeschlagen",a),alert("Status konnte nicht gespeichert werden.")}});const e=document.getElementById("comment-form");e&&e.addEventListener("submit",async n=>{n.preventDefault();const a=new FormData(e),r=parseInt(a.get("runde")||"1",10)===2?2:1,i=String(a.get("text")||"").trim();if(i)try{const s={video_id:this.videoId,runde:r,text:i,author_benutzer_id:window.currentUser?.id||null,author_name:window.currentUser?.name||null,created_at:new Date().toISOString()},{error:o}=await window.supabase.from("kooperation_video_comment").insert(s);if(o)throw o;e.reset(),await this.loadData(),this.render(),this.bindEvents()}catch(s){console.error("Kommentar speichern fehlgeschlagen",s),alert("Kommentar konnte nicht gespeichert werden.")}});const t=document.querySelector(".content-section");t&&t.addEventListener("click",async n=>{const a=n.target.closest(".comment-delete");if(!a)return;n.preventDefault();const r=a.dataset.id;if(r&&confirm("Kommentar wirklich entfernen?"))try{const{error:i}=await window.supabase.from("kooperation_video_comment").delete().eq("id",r);if(i)throw i;await this.loadData(),this.render(),this.bindEvents()}catch(i){console.error("Kommentar löschen fehlgeschlagen",i),alert("Kommentar konnte nicht gelöscht werden.")}})},destroy(){}};class Qe{constructor(){this.selectedKampagnen=new Set,this._boundEventListeners=new Set,this.statusOptions=[],this.kampagneArtMap=new Map}async init(){if(window.setHeadline("Kampagnen Übersicht"),!(window.canViewPage&&window.canViewPage("kampagne")||await window.checkUserPermission("kampagne","can_view"))){window.content.innerHTML=`
        <div class="error-message">
          <p>Sie haben keine Berechtigung, Kampagnen anzuzeigen.</p>
        </div>
      `;return}this.bindEvents(),await this.loadAndRender()}async loadAndRender(){try{const e=await window.dataService.loadFilterData("kampagne");await this.render(e),await this.initializeFilterBar();const t=k.getFilters("kampagne");console.log("🔍 Lade Kampagnen mit Filter:",t);const n=await this.loadKampagnenWithRelations();console.log("📊 Kampagnen geladen:",n?.length||0),this.updateTable(n)}catch(e){window.ErrorHandler.handle(e,"KampagneList.loadAndRender")}}async loadKampagnenWithRelations(){try{if(!window.supabase)return console.warn("⚠️ Supabase nicht verfügbar - verwende Mock-Daten"),await window.dataService.loadEntities("kampagne");try{const[{data:s},{data:o}]=await Promise.all([window.supabase.from("kampagne_status").select("id, name, sort_order").order("sort_order",{ascending:!0}).order("name",{ascending:!0}),window.supabase.from("kampagne_art_typen").select("id, name")]);this.statusOptions=s||[],this.kampagneArtMap=new Map((o||[]).map(d=>[d.id,d.name]))}catch(s){console.warn("⚠️ Konnte Status/Arten nicht laden",s)}const e=window.currentUser?.rolle==="admin";let t=[];if(!e)try{const{data:s}=await window.supabase.from("kampagne_mitarbeiter").select("kampagne_id").eq("mitarbeiter_id",window.currentUser?.id);if(t=(s||[]).map(o=>o.kampagne_id).filter(Boolean),t.length===0)return[]}catch{return[]}let n=window.supabase.from("kampagne").select(`
          *,
          unternehmen:unternehmen_id(firmenname),
          marke:marke_id(markenname),
          auftrag:auftrag_id(auftragsname),
          status_ref:status_id(id, name)
        `).order("created_at",{ascending:!1});!e&&t.length>0&&(n=n.in("id",t));const{data:a,error:r}=await n;if(r)throw console.error("❌ Fehler beim Laden der Kampagnen mit Beziehungen:",r),r;const i=a.map(s=>{const d=(Array.isArray(s.art_der_kampagne)?s.art_der_kampagne:[]).map(c=>this.kampagneArtMap.get(c)||c);return{...s,art_der_kampagne_display:d,unternehmen:s.unternehmen?{firmenname:s.unternehmen.firmenname}:null,marke:s.marke?{markenname:s.marke.markenname}:null,auftrag:s.auftrag?{auftragsname:s.auftrag.auftragsname}:null,status_name:s.status_ref?.name||s.status||null}});return console.log("✅ Kampagnen mit Beziehungen geladen:",i),i}catch(e){return console.error("❌ Fehler beim Laden der Kampagnen mit Beziehungen:",e),await window.dataService.loadEntities("kampagne")}}async render(e){const t=window.currentUser?.permissions?.kampagne?.can_edit||!1,n=k.getFilters("kampagne");Object.entries(n).forEach(([i,s])=>{});let a=`<div class="filter-bar">
      <div class="filter-left">
        <div id="filter-container"></div>
      </div>
      <div class="filter-right">
                  ...
                  <div class="submenu" data-submenu="status">
                    ${(this.statusOptions||[]).map(s=>`
                      <a href="#" class="submenu-item" data-action="set-field" data-field="status_id" data-value="${s.id}" data-status-name="${s.name}" data-id="${a.id}">
                        ${z.getStatusIcon(s.name)}
                        <span>${s.name}</span>
                        ${a.status_id&&a.status_id===s.id||a.status_name===s.name?'<span class="submenu-check">'+z.getHeroIcon("check")+"</span>":""}
                      </a>
                    `).join("")}
                  </div>
                </div>
...
`)}`)),alert(i),this.deselectAll(),await this.loadAndRender(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:"kampagne",action:"bulk-deleted",count:n}}))}}const me=new Qe;class Je{constructor(){this.kampagneId=null,this.kampagneData=null,this.creator=[],this.notizen=[],this.ratings=[],this.kooperationen=[],this.koopBudgetSum=0,this.koopVideosUsed=0,this.sourcingCreators=[],this.favoriten=[],this.rechnungen=[],this.history=[],this.historyCount=0,this.koopHistory=[],this.koopHistoryCount=0}async init(e){if(console.log("🎯 KAMPAGNEDETAIL: Initialisiere Kampagnen-Detailseite für ID:",e),this.kampagneId=e,window.moduleRegistry?.currentModule!==this){console.log("⚠️ KAMPAGNEDETAIL: Nicht mehr das aktuelle Modul, breche ab");return}try{await this.loadKampagneData(),await this.render(),this.bindEvents(),console.log("✅ KAMPAGNEDETAIL: Initialisierung abgeschlossen")}catch(t){console.error("❌ KAMPAGNEDETAIL: Fehler bei der Initialisierung:",t),window.ErrorHandler.handle(t,"KampagneDetail.init")}}async loadKooperationen(){try{const{data:e,error:t}=await window.supabase.from("kooperationen").select("id, name, status, gesamtkosten, videoanzahl, creator_id").eq("kampagne_id",this.kampagneId).order("created_at",{ascending:!1});if(t)throw t;const n=Array.from(new Set((e||[]).map(r=>r.creator_id).filter(Boolean)));let a={};if(n.length>0){const{data:r}=await window.supabase.from("creator").select("id, vorname, nachname").in("id",n);a=(r||[]).reduce((i,s)=>(i[s.id]=s,i),{})}this.kooperationen=(e||[]).map(r=>({...r,creator:a[r.creator_id]||null})),this.koopBudgetSum=this.kooperationen.reduce((r,i)=>r+(parseFloat(i.gesamtkosten)||0),0),this.koopVideosUsed=this.kooperationen.reduce((r,i)=>r+(parseInt(i.videoanzahl,10)||0),0),console.log("✅ KAMPAGNEDETAIL: Kooperationen geladen:",this.kooperationen.length,"Budgetsumme:",this.koopBudgetSum)}catch(e){console.error("❌ KAMPAGNEDETAIL: Fehler beim Laden der Kooperationen:",e),this.kooperationen=[],this.koopBudgetSum=0}}renderKooperationen(){const e=l=>l?new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(l):"-",t=this.kampagneData?.auftrag?.creator_budget||0,n=this.koopBudgetSum,a=t>0?Math.min(100,Math.round(n/t*100)):0,r=this.kampagneData?.videoanzahl||0,i=this.koopVideosUsed||0,s=Math.max(0,r-i),o=r>0?Math.min(100,Math.round(i/r*100)):0,d=`
      ...
      `;const c=this.kooperationen.map(l=>`
      <tr>
        <td><a href="/kooperation/${l.id}" onclick="event.preventDefault(); window.navigateTo('/kooperation/${l.id}')">${window.validatorSystem.sanitizeHtml(l.name||"—")}</a></td>
        <td>${window.validatorSystem.sanitizeHtml(l.creator?`${l.creator.vorname} ${l.creator.nachname}`:"Unbekannt")}</td>
        <td>${l.videoanzahl||0}</td>
        <td><span class="status-badge status-${(l.status||"unknown").toLowerCase()}">${l.status||"-"}</span></td>
        <td>${e(l.gesamtkosten)}</td>
      </tr>
    `).join("");return`
      ${d}
      <div class="data-table-container">
    ...
    `}renderCreatorSourcing(){return!this.sourcingCreators||this.sourcingCreators.length===0?`
      ...
      ${N(this.sourcingCreators,{showFavoriteAction:!0,showSelection:!0,kampagneId:this.kampagneId})}
    `}renderFavoriten(){return!this.favoriten||this.favoriten.length===0?`
      ...
      ${N(this.favoriten,{showFavoritesMenu:!0,showSelection:!0,kampagneId:this.kampagneId})}
    `}renderRechnungen(){if(!this.rechnungen||this.rechnungen.length===0)return`
         <div class="empty-state">
     ...
     `}async loadKampagneData(){console.log("🔄 KAMPAGNEDETAIL: Lade Kampagnen-Daten...");try{const{data:e,error:t}=await window.supabase.from("kampagne").select(`
        ...
        `).eq("id",this.kampagneId).single();if(t)throw t;if(this.kampagneData=e,console.log("✅ KAMPAGNEDETAIL: Kampagnen-Basisdaten geladen:",this.kampagneData),this.kampagneData.art_der_kampagne&&this.kampagneData.art_der_kampagne.length>0){const{data:r,error:i}=await window.supabase.from("kampagne_art_typen").select("id, name, beschreibung").in("id",this.kampagneData.art_der_kampagne);i||(this.kampagneData.kampagne_art_typen=r||[],console.log("✅ KAMPAGNEDETAIL: Kampagnen-Arten geladen:",this.kampagneData.kampagne_art_typen.length))}const{data:n,error:a}=await window.supabase.from("kampagne_creator").select(`
          *,
          creator:creator_id(
            id,
            vorname,
        ...
        `).eq("kampagne_id",this.kampagneId);a||(this.creator=n||[],console.log("✅ KAMPAGNEDETAIL: Creators geladen:",this.creator.length));try{const{data:r}=await window.supabase.from("kampagne_creator_sourcing").select(`
            id,
            creator:creator_id (
              id,
              vorname,
          ...
          `).eq("kampagne_id",this.kampagneId);this.sourcingCreators=(r||[]).map(i=>{const s=i.creator||{};return{id:s.id,vorname:s.vorname,nachname:s.nachname,creator_types:(s.creator_types||[]).map(o=>o.creator_type).filter(Boolean),sprachen:(s.sprachen||[]).map(o=>o.sprachen).filter(Boolean),branchen:(s.branchen||[]).map(o=>o.branchen_creator).filter(Boolean),instagram_follower:s.instagram_follower,tiktok_follower:s.tiktok_follower,lieferadresse_stadt:s.lieferadresse_stadt,lieferadresse_land:s.lieferadresse_land}})}catch(r){console.warn("⚠️ KAMPAGNEDETAIL: Creator Sourcing konnte nicht geladen werden",r),this.sourcingCreators=[]}try{const{data:r}=await window.supabase.from("kampagne_creator_favoriten").select(`
            id,
            creator:creator_id (
              id,
              vorname,
          ...
          `).eq("kampagne_id",this.kampagneId);this.favoriten=(r||[]).map(i=>{const s=i.creator||{};return{id:s.id,vorname:s.vorname,nachname:s.nachname,creator_types:(s.creator_types||[]).map(o=>o.creator_type).filter(Boolean),sprachen:(s.sprachen||[]).map(o=>o.sprachen).filter(Boolean),branchen:(s.branchen||[]).map(o=>o.branchen_creator).filter(Boolean),instagram_follower:s.instagram_follower,tiktok_follower:s.tiktok_follower,lieferadresse_stadt:s.lieferadresse_stadt,lieferadresse_land:s.lieferadresse_land}})}catch(r){console.warn("⚠️ KAMPAGNEDETAIL: Favoriten konnten nicht geladen werden",r),this.favoriten=[]}window.notizenSystem&&(this.notizen=await window.notizenSystem.loadNotizen("kampagne",this.kampagneId),console.log("✅ KAMPAGNEDETAIL: Notizen geladen:",this.notizen.length)),window.bewertungsSystem&&(this.ratings=await window.bewertungsSystem.loadBewertungen("kampagne",this.kampagneId),console.log("✅ KAMPAGNEDETAIL: Ratings geladen:",this.ratings.length)),await this.loadKooperationen();try{const{data:r}=await window.supabase.from("rechnung").select("id, rechnung_nr, status, nettobetrag, bruttobetrag, gestellt_am, bezahlt_am, pdf_url, kooperation:kooperation_id(id, name), creator:creator_id(id, vorname, nachname)").eq("kampagne_id",this.kampagneId).order("gestellt_am",{ascending:!1});this.rechnungen=r||[]}catch{this.rechnungen=[]}try{const{data:r}=await window.supabase.from("kampagne_history").select("id, old_status, new_status, comment, created_at, benutzer:changed_by(name)").eq("kampagne_id",this.kampagneId).order("created_at",{ascending:!1});this.history=(r||[]).map(i=>({id:i.id,old_status:i.old_status||null,new_status:i.new_status||null,comment:i.comment||"",created_at:i.created_at,user_name:i.benutzer?.name||"-"})),this.historyCount=this.history.length}catch{this.history=[],this.historyCount=0}try{const r=(this.kooperationen||[]).map(i=>i.id).filter(Boolean);if(r.length>0){const i=(this.kooperationen||[]).reduce((o,d)=>(o[d.id]=d,o),{}),{data:s}=await window.supabase.from("kooperation_history").select("id, kooperation_id, old_status, new_status, comment, created_at, benutzer:changed_by(name)").in("kooperation_id",r).order("created_at",{ascending:!1});this.koopHistory=(s||[]).map(o=>({id:o.id,kooperation_id:o.kooperation_id,kooperation_name:i[o.kooperation_id]?.name||o.kooperation_id,old_status:o.old_status||null,new_status:o.new_status||null,comment:o.comment||"",created_at:o.created_at,user_name:o.benutzer?.name||"-"})),this.koopHistoryCount=this.koopHistory.length}else this.koopHistory=[],this.koopHistoryCount=0}catch{this.koopHistory=[],this.koopHistoryCount=0}}catch(e){throw console.error("❌ KAMPAGNEDETAIL: Fehler beim Laden der Kampagnen-Daten:",e),e}}async render(){if(!this.kampagneData){this.showNotFound();return}window.setHeadline(`Kampagne: ${this.kampagneData.kampagnenname}`);const e=window.currentUser?.permissions?.kampagne?.can_edit||!1,t=window.currentUser?.permissions?.kampagne?.can_delete||!1,n=s=>s?new Date(s).toLocaleDateString("de-DE"):"-",a=s=>s?new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(s):"-",r=s=>s?Array.isArray(s)?s.map(o=>o.name||o).join(", "):String(s):"-",i=`
      <div class="page-header">
        <div class="page-header-left">
    ...
    `;window.setContentSafely(window.content,n)}async initializeFilterBar(){const e=document.getElementById("filter-container");e&&await k.renderFilterBar("briefing",e,t=>this.onFiltersApplied(t),()=>this.onFiltersReset())}onFiltersApplied(e){console.log("🔍 BriefingList: Filter angewendet:",e),k.applyFilters("briefing",e),this.loadAndRender()}onFiltersReset(){console.log("🔄 BriefingList: Filter zurückgesetzt"),k.resetFilters("briefing"),this.loadAndRender()}bindEvents(){this.boundFilterResetHandler=e=>{e.target.id==="btn-filter-reset"&&this.onFiltersReset()},document.addEventListener("click",this.boundFilterResetHandler),document.addEventListener("click",e=>{(e.target.id==="btn-briefing-new"||e.target.id==="btn-briefing-new-filter")&&(e.preventDefault(),window.navigateTo("/briefing/new"))}),document.addEventListener("click",e=>{if(e.target.id==="btn-select-all"){e.preventDefault(),document.querySelectorAll(".briefing-check").forEach(a=>{a.checked=!0,a.dataset.id&&this.selectedBriefings.add(a.dataset.id)});const n=document.getElementById("select-all-briefings");n&&(n.indeterminate=!1,n.checked=!0),this.updateSelection()}}),document.addEventListener("click",e=>{if(e.target.id==="btn-deselect-all"){e.preventDefault(),document.querySelectorAll(".briefing-check").forEach(a=>{a.checked=!1}),this.selectedBriefings.clear();const n=document.getElementById("select-all-briefings");n&&(n.indeterminate=!1,n.checked=!1),this.updateSelection()}}),document.addEventListener("click",e=>{if(e.target.classList.contains("table-link")&&e.target.dataset.table==="briefing"){e.preventDefault();const t=e.target.dataset.id;window.navigateTo(`/briefing/${t}`)}}),window.addEventListener("entityUpdated",e=>{e.detail.entity==="briefing"&&this.loadAndRender()}),document.addEventListener("change",e=>{if(e.target.id==="select-all-briefings"){const t=document.querySelectorAll(".briefing-check"),n=e.target.checked;t.forEach(a=>{a.checked=n,n?this.selectedBriefings.add(a.dataset.id):this.selectedBriefings.delete(a.dataset.id)}),this.updateSelection()}}),document.addEventListener("change",e=>{e.target.classList.contains("briefing-check")&&(e.target.checked?this.selectedBriefings.add(e.target.dataset.id):this.selectedBriefings.delete(e.target.dataset.id),this.updateSelection(),this.updateSelectAllCheckbox())})}hasActiveFilters(){const e=k.getFilters("briefing");return Object.keys(e).length>0}updateSelection(){const e=this.selectedBriefings.size,t=document.getElementById("btn-deselect-all"),n=document.getElementById("btn-delete-selected");t&&(t.style.display=e>0?"inline-block":"none"),n&&(n.style.display=e>0?"inline-block":"none")}updateSelectAllCheckbox(){const e=document.getElementById("select-all-briefings"),t=document.querySelectorAll(".briefing-check");if(!e||t.length===0)return;const n=document.querySelectorAll(".briefing-check:checked"),a=n.length===t.length,r=n.length>0;e.checked=a,e.indeterminate=r&&!a}async updateTable(e){const t=document.getElementById("briefings-table-body");if(!t)return;if(!e||e.length===0){const{renderEmptyState:i}=await x(async()=>{const{renderEmptyState:s}=await import("./core-C7Vz5Umf.js").then(o=>o.F);return{renderEmptyState:s}},[]);i(t);return}const n=i=>i?new Date(i).toLocaleDateString("de-DE"):"-",a=i=>window.validatorSystem.sanitizeHtml(i||"—"),r=e.map(i=>`
      <tr data-id="${i.id}">
    ...
    `}destroy(){window.setContentSafely("")}}const ge=new nt;class at{constructor(){this.formData={}}async init(){console.log("🎯 ANSPRECHPARTNERCREATE: Initialisiere Ansprechpartner-Erstellung"),this.showCreateForm()}showCreateForm(){console.log("🎯 ANSPRECHPARTNERCREATE: Zeige Ansprechpartner-Erstellungsformular mit FormSystem"),window.setHeadline("Neuen Ansprechpartner anlegen");const e=window.formSystem.renderFormOnly("ansprechpartner"),t=document.getElementById("dashboard-content");t.innerHTML=`
      <div class="page-header">
        <div class="page-header-left">
    ...
    `,window.formSystem.bindFormEvents("ansprechpartner",null);const n=document.getElementById("ansprechpartner-form");n&&(n.onsubmit=async a=>{a.preventDefault(),await this.handleFormSubmit()})}async handleFormSubmit(){let e="Ansprechpartner anlegen";try{console.log("🎯 ANSPRECHPARTNERCREATE: Verarbeite Formular-Submit");const t=document.querySelector('#ansprechpartner-form button[type="submit"]');t&&(e=t.innerHTML,t.innerHTML='<div class="loading-spinner"></div> Wird angelegt...',t.disabled=!0);const n=document.getElementById("ansprechpartner-form"),a=new FormData(n),r={},i={},s=n.querySelectorAll('select[data-tag-based="true"]');console.log("🏷️ Tag-basierte Selects gefunden:",s.length),s.forEach(c=>{let l=n.querySelector(`select[name="${c.name}"][style*="display: none"]`);if(!l){const u=n.querySelectorAll(`select[name="${c.name}"]`);u.length>1&&(l=u[1])}if(l){const u=Array.from(l.selectedOptions).map(m=>m.value).filter(m=>m!=="");u.length>0&&(i[c.name]=u,console.log(`🏷️ Tag-basiertes Multi-Select ${c.name}:`,u))}});for(let[c,l]of a.entries())if(!i.hasOwnProperty(c))if(c.includes("[]")){const u=c.replace("[]","");i[u]||(i[u]=[]),i[u].push(l)}else i[c]?(Array.isArray(i[c])||(i[c]=[i[c]]),i[c].push(l)):i[c]=l;for(let[c,l]of Object.entries(i))r[c]=Array.isArray(l)?l:l.trim();console.log("📤 Finale Ansprechpartner-Daten:",r);const o=window.validatorSystem.validateForm(r,{vorname:{type:"text",minLength:2,required:!0},nachname:{type:"text",minLength:2,required:!0}});if(!o.isValid){this.showValidationErrors(o.errors);return}const d=await window.dataService.createEntity("ansprechpartner",r);if(d.success)this.showSuccessMessage("Ansprechpartner erfolgreich erstellt!"),setTimeout(()=>{window.navigateTo("/ansprechpartner")},1500);else throw new Error(d.error||"Fehler beim Erstellen des Ansprechpartners")}catch(t){console.error("❌ Fehler beim Erstellen des Ansprechpartners:",t),this.showErrorMessage(t.message||"Ein unerwarteter Fehler ist aufgetreten")}finally{const t=document.querySelector('#ansprechpartner-form button[type="submit"]');t&&(t.innerHTML=e,t.disabled=!1)}}showValidationErrors(e){document.querySelectorAll(".field-error").forEach(t=>t.remove());for(const[t,n]of Object.entries(e)){const a=document.querySelector(`[name="${t}"]`);if(a){const r=document.createElement("div");r.className="field-error",r.textContent=n,r.style.color="red",r.style.fontSize="12px",r.style.marginTop="4px",a.parentNode.appendChild(r)}}}showSuccessMessage(e){const t=document.createElement("div");t.className="toast success",t.textContent=e,t.style.cssText=`
      position: fixed;
      top: 20px;
      right: 20px;
      background: #10b981;
    ...
    `,await this.initializeFilterBar()}async initializeFilterBar(){const e=document.getElementById("filter-container");e&&await k.renderFilterBar("ansprechpartner",e,t=>this.onFiltersApplied(t),()=>this.onFiltersReset())}onFiltersApplied(e){console.log("Filter angewendet:",e),k.applyFilters("ansprechpartner",e),this.loadAndRender()}onFiltersReset(){console.log("Filter zurückgesetzt"),k.resetFilters("ansprechpartner"),this.loadAndRender()}bindEvents(){this.boundFilterResetHandler=e=>{e.target.id==="btn-filter-reset"&&this.onFiltersReset()},document.addEventListener("click",this.boundFilterResetHandler),document.addEventListener("click",e=>{e.target.id==="btn-ansprechpartner-new"&&(e.preventDefault(),window.navigateTo("/ansprechpartner/new"))}),document.addEventListener("click",e=>{if(e.target.classList.contains("table-link")&&e.target.dataset.table==="ansprechpartner"){e.preventDefault();const t=e.target.dataset.id;console.log("🎯 ANSPRECHPARTNERLIST: Navigiere zu Ansprechpartner Details:",t),window.navigateTo(`/ansprechpartner/${t}`)}}),window.addEventListener("entityUpdated",e=>{e.detail.entity==="ansprechpartner"&&this.loadAndRender()}),document.addEventListener("change",e=>{if(e.target.id==="select-all-ansprechpartner"){const t=document.querySelectorAll(".ansprechpartner-check"),n=e.target.checked;t.forEach(a=>{a.checked=n,n?this.selectedAnsprechpartner.add(a.dataset.id):this.selectedAnsprechpartner.delete(a.dataset.id)}),this.updateSelection(),console.log(`${n?"✅ Alle Ansprechpartner ausgewählt":"❌ Alle Ansprechpartner abgewählt"}: ${this.selectedAnsprechpartner.size}`)}}),document.addEventListener("change",e=>{e.target.classList.contains("ansprechpartner-check")&&(e.target.checked?this.selectedAnsprechpartner.add(e.target.dataset.id):this.selectedAnsprechpartner.delete(e.target.dataset.id),this.updateSelection(),this.updateSelectAllCheckbox())}),window.bulkActionSystem&&window.bulkActionSystem.registerList("ansprechpartner",this)}hasActiveFilters(){const e=k.getFilters("ansprechpartner");return Object.keys(e).length>0}updateSelection(){const e=this.selectedAnsprechpartner.size,t=document.getElementById("selected-count"),n=document.getElementById("btn-deselect-all"),a=document.getElementById("btn-delete-selected"),r=document.getElementById("bulk-actions");t&&(t.textContent=`${e} ausgewählt`),r&&(r.style.display=e>0?"flex":"none"),n&&(n.style.display=e>0?"inline-block":"none"),a&&(a.style.display=e>0?"inline-block":"none")}updateTable(e){const t=document.querySelector(".data-table tbody");if(!t)return;if(!e||e.length===0){t.innerHTML=`
        <tr>
...
Fehler:
${r.join(`
`)}`)),alert(i),this.deselectAll(),await this.loadAndRender(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:"ansprechpartner",action:"bulk-deleted",count:n}}))}showCreateForm(){console.log("🎯 Zeige Ansprechpartner-Erstellungsformular mit AnsprechpartnerCreate"),q.showCreateForm()}destroy(){console.log("AnsprechpartnerList: Cleaning up..."),this.boundFilterResetHandler&&document.removeEventListener("click",this.boundFilterResetHandler)}}const be=new rt;class it{constructor(){this.config=new U,this.renderer=new ee,this.autoGeneration=new te,this.dependentFields=new ae(this.autoGeneration),this.relationTables=new re,this.dataLoader=new ie,this.renderer.getFormConfig=this.config.getFormConfig.bind(this.config),this.relationTables.getFormConfig=this.config.getFormConfig.bind(this.config),this.dataLoader.getFormConfig=this.config.getFormConfig.bind(this.config),this.dataLoader.createSearchableSelect=this.createSearchableSelect.bind(this),this.dataLoader.reinitializeSearchableSelect=this.reinitializeSearchableSelect.bind(this),this.currentForm=null}async openForm(e,t=null){try{const n=this.renderer.renderForm(e,t),a=document.createElement("div");a.className="modal-overlay",a.innerHTML=`
        <div class="modal-content">
          ${n}
        </div>
      `,document.body.appendChild(a),this.currentForm={entity:e,data:t,modal:a},await this.bindFormEvents(e,t),console.log(`✅ Formular geöffnet: ${e}`)}catch(n){console.error("❌ Fehler beim Öffnen des Formulars:",n)}}closeForm(){this.currentForm&&this.currentForm.modal&&(document.body.removeChild(this.currentForm.modal),this.currentForm=null,console.log("✅ Formular geschlossen"))}async bindFormEvents(e,t){const n=document.getElementById(`${e}-form`);if(!n)return;n.onsubmit=async r=>{r.preventDefault(),await this.handleFormSubmit(e,t)};const a=n.querySelector(".btn-close");a&&(a.onclick=()=>this.closeForm()),await this.dataLoader.loadDynamicFormData(e,n),this.initializeSearchableSelects(n),this.dependentFields.setupDependentFields(n),this.setupAddressesFields(n),this.autoGeneration.setupAutoGeneration(n)}async handleFormSubmit(e,t){try{const n=document.getElementById(`${e}-form`);if(!n)return;const a=new FormData(n),r={};console.log("📤 FormData sammeln...");const i=n.querySelectorAll('select[data-tag-based="true"]');console.log("🏷️ Tag-basierte Selects gefunden:",i.length),i.forEach(l=>{console.log(`🔍 Verarbeite Tag-basiertes Select: ${l.name}`);const u=n.querySelectorAll(`select[name^="${l.name}"]`);console.log(`🔍 Alle Selects für ${l.name}:`,u.length,Array.from(u).map(h=>({name:h.name,hidden:h.style.display==="none",options:h.options.length})));let m=n.querySelector(`select[name="${l.name}[]"][style*="display: none"]`);if(console.log("🔍 Verstecktes Select mit [] gefunden:",!!m),m||(m=n.querySelector(`select[name="${l.name}"][style*="display: none"]`),console.log("🔍 Verstecktes Select ohne [] gefunden:",!!m)),m||u.length>1&&(m=u[1],console.log("🔍 Fallback: Zweites Select verwendet:",!!m)),m){console.log("🔍 Verstecktes Select Details:",{name:m.name,optionsCount:m.options.length,selectedCount:m.selectedOptions.length});const h=Array.from(m.selectedOptions).map(g=>g.value).filter(g=>g!=="");console.log("🔍 Ausgewählte Werte:",h),h.length>0&&(r[l.name]=h,console.log(`🏷️ Tag-basiertes Multi-Select ${l.name}:`,h))}else console.log(`⚠️ Verstecktes Select für ${l.name} nicht gefunden`)});const s=n.querySelectorAll('select[multiple]:not([data-tag-based="true"])');console.log("🔧 Normale Multi-Select Felder gefunden:",s.length),s.forEach(l=>{const u=Array.from(l.selectedOptions).map(m=>m.value).filter(m=>m!=="");u.length>0&&(r[l.name]=u,console.log(`✅ Multi-Select ${l.name}:`,u))});for(const[l,u]of a.entries())if(u!=="")if(l.includes("[]")){const m=l.replace("[]","");r[m]||(r[m]=[]),r[m].push(u),console.log(`📤 Multi-Select Array ${m}: ${u}`)}else r.hasOwnProperty(l)||(r[l]=u,console.log(`📤 FormData ${l}: ${u}`));const o=n.querySelectorAll('select[data-searchable="true"]');console.log("🔧 Searchable Selects gefunden:",o.length),o.forEach(l=>{console.log(`🔧 Prüfe Select ${l.name}:`,{value:l.value,options:l.options.length,selectedIndex:l.selectedIndex}),l.value&&l.value!==""?(r[l.name]=l.value,console.log(`✅ Searchable Select ${l.name}: ${l.value}`)):console.log(`❌ Searchable Select ${l.name}: Kein Wert`)}),console.log("📤 Finale Submit-Daten:",r);const d=this.validateFormData(e,r);if(d.length>0){this.showValidationErrors(d);return}let c;t&&t.id?c=await window.dataService.updateEntity(e,t.id,r):c=await window.dataService.createEntity(e,r),c.success?(await this.relationTables.handleRelationTables(e,c.id,r,n),e==="kampagne"&&await this.handleKampagneAddresses(c.id,n),this.showSuccessMessage(t?"Erfolgreich aktualisiert!":"Erfolgreich erstellt!"),this.closeForm(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:e,id:c.id,action:t?"updated":"created"}}))):this.showErrorMessage(`Fehler beim ${t?"Aktualisieren":"Erstellen"}: ${c.error}`)}catch(n){console.error("❌ Fehler beim Formular-Submit:",n),this.showErrorMessage("Ein unerwarteter Fehler ist aufgetreten.")}}async handleKampagneAddresses(e,t){try{const n=t.querySelector(".addresses-list");if(!n)return;const a=n.querySelectorAll(".address-item"),r=[];if(a.forEach(i=>{const s=i.dataset.addressId,o={kampagne_id:e,adressname:t.querySelector(`input[name="adressname_${s}"]`)?.value||"",strasse:t.querySelector(`input[name="strasse_${s}"]`)?.value||"",hausnummer:t.querySelector(`input[name="hausnummer_${s}"]`)?.value||"",plz:t.querySelector(`input[name="plz_${s}"]`)?.value||"",stadt:t.querySelector(`input[name="stadt_${s}"]`)?.value||"",land:t.querySelector(`input[name="land_${s}"]`)?.value||"",notiz:t.querySelector(`textarea[name="notiz_${s}"]`)?.value||""};o.adressname.trim()&&r.push(o)}),r.length>0){await window.supabase.from("kampagne_adressen").delete().eq("kampagne_id",e);const{error:i}=await window.supabase.from("kampagne_adressen").insert(r);i?console.error("❌ Fehler beim Speichern der Adressen:",i):console.log(`✅ ${r.length} Adressen für Kampagne ${e} gespeichert`)}}catch(n){console.error("❌ Fehler beim Verarbeiten der Kampagnen-Adressen:",n)}}setupAddressesFields(e){e.querySelectorAll(".addresses-container").forEach(n=>{const a=n.querySelector(".add-address-btn"),r=n.querySelector(".addresses-list");a&&r&&a.addEventListener("click",()=>{this.renderer.addAddressRow(r)})})}initializeSearchableSelects(e){e.querySelectorAll('select[data-searchable="true"]').forEach(n=>{if(n.options.length>1){const r=Array.from(n.options).slice(1).map(i=>({value:i.value,label:i.textContent}));console.log(`🔧 Initialisiere Searchable Select ${n.name} mit ${r.length} Optionen`),this.createSearchableSelect(n,r,{placeholder:n.dataset.placeholder||"Bitte wählen..."})}else console.log(`🔧 Initialisiere Searchable Select ${n.name} mit leeren Optionen`),this.createSearchableSelect(n,[],{placeholder:n.dataset.placeholder||"Bitte wählen..."})})}createSearchableSelect(e,t,n){const a=e.parentNode.querySelector(".searchable-select-container");a&&a.remove();const r=document.createElement("div");r.className="searchable-select-container",r.style.position="relative";const i=document.createElement("input");i.type="text",i.className="searchable-select-input",i.placeholder=n.placeholder||"Suchen...",i.style.cssText=`
      width: 100%;
      padding: 8px 12px;
      ...
      font-size: 14px;
      background: white;
    `;const s=document.createElement("div");s.className="searchable-select-dropdown",s.style.cssText=`
      position: absolute;
      top: 100%;
      left: 0;
      right: 0;
      ...
      z-index: 1000;
      display: none;
    `,e.style.display="none",e.parentNode.insertBefore(r,e),r.appendChild(i),r.appendChild(s),i.addEventListener("focus",()=>{s.style.display="block",this.updateDropdownItems(s,t,i.value)}),i.addEventListener("blur",()=>{setTimeout(()=>{s.style.display="none"},200)}),i.addEventListener("input",()=>{this.updateDropdownItems(s,t,i.value)}),this.updateDropdownItems(s,t,"")}updateDropdownItems(e,t,n){e.innerHTML="",t.filter(r=>r.label.toLowerCase().includes(n.toLowerCase())).forEach(r=>{const i=document.createElement("div");i.className="searchable-select-item",i.textContent=r.label,i.style.cssText=`
        padding: 8px 12px;
        cursor: pointer;
        border-bottom: 1px solid #f3f4f6;
      `,i.addEventListener("click",()=>{const s=e.parentNode,o=s.parentNode.querySelector("select");if(o){o.value=r.value,console.log(`🔧 Select ${o.name} auf Wert gesetzt:`,r.value);const d=s.querySelector("input");d&&(d.value=r.label),o.dispatchEvent(new Event("change"))}else console.error("❌ Select-Element nicht gefunden für Dropdown");e.style.display="none"}),i.addEventListener("mouseenter",()=>{i.style.backgroundColor="#f3f4f6"}),i.addEventListener("mouseleave",()=>{i.style.backgroundColor="white"}),e.appendChild(i)})}reinitializeSearchableSelect(e,t,n){this.createSearchableSelect(e,t,n)}validateFormData(e,t){const n=[],a=this.config.getFormConfig(e);return a&&a.fields.forEach(r=>{if(r.required&&(!t[r.name]||t[r.name].toString().trim()==="")&&n.push(`${r.label} ist erforderlich.`),r.validation&&t[r.name]){const i=t[r.name];switch(r.validation.type){case"text":r.validation.minLength&&i.length<r.validation.minLength&&n.push(`${r.label} muss mindestens ${r.validation.minLength} Zeichen lang sein.`);break;case"number":r.validation.min!==void 0&&parseFloat(i)<r.validation.min&&n.push(`${r.label} muss mindestens ${r.validation.min} sein.`),r.validation.max!==void 0&&parseFloat(i)>r.validation.max&&n.push(`${r.label} darf maximal ${r.validation.max} sein.`);break;case"email":/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(i)||n.push(`${r.label} muss eine gültige E-Mail-Adresse sein.`);break;case"url":try{new URL(i)}catch{n.push(`${r.label} muss eine gültige URL sein.`)}break}}}),n}showValidationErrors(e){const t=e.join(`
`);alert(`Validierungsfehler:
${t}`)}showSuccessMessage(e){console.log(`✅ ${e}`)}showErrorMessage(e){console.error(`❌ ${e}`),alert(e)}}class st{constructor(){this.ansprechpartner=null,this.ansprechpartnerId=null,this.formConfig=null,this.formRenderer=null,this.dataLoader=null,this.formSystem=null}async init(e){if(this.ansprechpartnerId=e,console.log("🎯 ANSPRECHPARTNERDETAIL: Initialisiere Detail-Seite für:",e),e==="new"){console.log("🎯 ANSPRECHPARTNERDETAIL: Verwende AnsprechpartnerCreate für neuen Ansprechpartner"),q.showCreateForm();return}else await this.loadAnsprechpartnerData(),this.render(),this.bindEvents()}async loadAnsprechpartnerData(){try{console.log("🔄 ANSPRECHPARTNERDETAIL: Lade Ansprechpartner-Daten...");const{data:e,error:t}=await window.supabase.from("ansprechpartner").select(`
          *,
          unternehmen:unternehmen_id (
            id,
            firmenname
        ...
        `).eq("id",this.ansprechpartnerId).single();if(t){console.error("❌ ANSPRECHPARTNERDETAIL: Fehler beim Laden:",t),this.showError("Ansprechpartner konnte nicht geladen werden.");return}this.ansprechpartner=e,console.log("✅ ANSPRECHPARTNERDETAIL: Ansprechpartner geladen:",this.ansprechpartner)}catch(e){console.error("❌ ANSPRECHPARTNERDETAIL: Unerwarteter Fehler:",e),this.showError("Ein unerwarteter Fehler ist aufgetreten.")}}render(){if(!this.ansprechpartner){this.showError("Ansprechpartner nicht gefunden.");return}window.setHeadline(`${this.ansprechpartner.vorname} ${this.ansprechpartner.nachname}`);const e=document.getElementById("dashboard-content");e&&(e.innerHTML=`
      <div class="page-header">
      ...
      <div class="detail-grid">
        ...
        <div class="detail-section">
          ...
          <div class="detail-content">
            ...
            <div class="detail-item">
              <label>Sprache:</label>
              <span>${this.ansprechpartner.sprachen&&this.ansprechpartner.sprachen.length>0?this.ansprechpartner.sprachen.map(t=>t.name).join(", "):this.ansprechpartner.sprache?.name||this.ansprechpartner.sprache||"-"}</span>
            </div>
          </div>
        </div>
    ...
    `)}renderMarkenList(){return!this.ansprechpartner.ansprechpartner_marke||this.ansprechpartner.ansprechpartner_marke.length===0?'<p class="empty-state">Keine Marken zugeordnet.</p>':`<div class="tag-list">${this.ansprechpartner.ansprechpartner_marke.map(t=>{const n=t.marke;return`
      ...
      `}).join("")}</div>`}bindEvents(){document.addEventListener("click",e=>{(e.target.id==="btn-back"||e.target.closest("#btn-back"))&&(e.preventDefault(),window.navigateTo("/ansprechpartner"))}),document.addEventListener("click",e=>{(e.target.id==="btn-edit"||e.target.closest("#btn-edit"))&&(e.preventDefault(),window.navigateTo(`/ansprechpartner/${this.ansprechpartnerId}/edit`))}),document.addEventListener("click",e=>{if(e.target.classList.contains("table-link")){e.preventDefault();const t=e.target.dataset.table,n=e.target.dataset.id;window.navigateTo(`/${t}/${n}`)}})}async renderCreateForm(){window.setHeadline("Neuen Ansprechpartner anlegen");const e=document.getElementById("dashboard-content");if(!e)return;const t=new U;if(this.formConfig=t.getFormConfig("ansprechpartner"),!this.formConfig){console.error("❌ Keine FormConfig für ansprechpartner gefunden"),this.showError("Formular-Konfiguration nicht gefunden.");return}this.formSystem=new it,this.formRenderer=this.formSystem.renderer,console.log("🎯 ANSPRECHPARTNERDETAIL: Rendere Formular..."),e.innerHTML=`
      <div class="page-header">
        <div class="page-header-left">
    ...
    `,document.addEventListener("click",n=>{(n.target.id==="btn-back-error"||n.target.closest("#btn-back-error"))&&(n.preventDefault(),window.navigateTo("/ansprechpartner"))}))}destroy(){console.log("AnsprechpartnerDetail: Cleaning up..."),this.ansprechpartner=null,this.ansprechpartnerId=null}}const fe=new st;class ot{constructor(){this.selectedRechnungen=new Set}async init(){window.setHeadline("Rechnungen"),await this.loadAndRender(),window.addEventListener("entityUpdated",e=>{e?.detail?.entity==="rechnung"&&this.loadAndRender()}),document.addEventListener("click",e=>{e.target&&e.target.id==="btn-rechnung-new"&&(e.preventDefault(),this.showCreateForm())})}async loadAndRender(){try{const e=await window.dataService.loadFilterData("rechnung");await this.render(e),await this.initializeFilterBar();const t=k.getFilters("rechnung"),n=window.currentUser?.rolle==="admin";let a=[],r=[];if(!n&&window.supabase)try{const{data:s}=await window.supabase.from("kampagne_mitarbeiter").select("kampagne_id").eq("mitarbeiter_id",window.currentUser?.id);if(a=(s||[]).map(o=>o.kampagne_id).filter(Boolean),a.length>0){const{data:o}=await window.supabase.from("kooperationen").select("id").in("kampagne_id",a);r=(o||[]).map(d=>d.id)}}catch{}let i;if(!n&&(a.length||r.length)){const s={...t};i=await window.dataService.loadEntities("rechnung",s),i=(i||[]).filter(o=>o.kampagne_id&&a.includes(o.kampagne_id)||o.kooperation_id&&r.includes(o.kooperation_id))}else i=await window.dataService.loadEntities("rechnung",t);this.updateTable(i)}catch(e){window.ErrorHandler?.handle?.(e,"RechnungList.loadAndRender")}}async render(){const e=`
      <div class="page-header">
        <div class="page-header-left">
          <h1>Rechnungen</h1>
    ...
    `;window.setContentSafely(window.content,e)}async initializeFilterBar(){const e=document.getElementById("filter-container");e&&await k.renderFilterBar("rechnung",e,t=>this.onFiltersApplied(t),()=>this.onFiltersReset())}onFiltersApplied(e){k.applyFilters("rechnung",e),this.loadAndRender()}onFiltersReset(){k.resetFilters("rechnung"),this.loadAndRender()}hasActiveFilters(){const e=k.getFilters("rechnung");return Object.keys(e).length>0}async updateTable(e){const t=document.getElementById("rechnungen-table-body");if(!t)return;if(!e||e.length===0){const{renderEmptyState:s}=await x(async()=>{const{renderEmptyState:o}=await import("./core-C7Vz5Umf.js").then(d=>d.F);return{renderEmptyState:o}},[]);s(t);return}const n=s=>s==null?"-":new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(s),a=s=>s?new Intl.DateTimeFormat("de-DE").format(new Date(s)):"-",r=e.map(s=>`
      <tr data-id="${s.id}">
                ...
                <div class="submenu" data-submenu="status">
                  ${[{label:"Offen",icon:"status-offen"},{label:"Rückfrage",icon:"status-rueckfrage"},{label:"Bezahlt",icon:"status-bezahlt"},{label:"An Qonto gesendet",icon:"status-qonto"}].map(o=>`
                    <a href="#" class="submenu-item" data-action="set-field" data-field="status" data-value="${o.label}" data-id="${s.id}">
                      ${z.getHeroIcon(o.icon)}
                      <span>${o.label}</span>
                      ${s.status===o.label?'<span class="submenu-check">'+z.getHeroIcon("check")+"</span>":""}
                    </a>
                  `).join("")}
                </div>
              </div>
    ...
    `).join("");t.innerHTML=r,document.querySelectorAll(".rechnung-check").forEach(s=>{s.addEventListener("change",o=>{const d=o.target.dataset.id;o.target.checked?this.selectedRechnungen.add(d):this.selectedRechnungen.delete(d),this.updateSelectionUI(),this.updateHeaderSelectAll()})});const i=document.getElementById("select-all-rechnungen");i&&i.addEventListener("change",s=>{const o=s.target.checked;document.querySelectorAll(".rechnung-check").forEach(d=>{d.checked=o,o?this.selectedRechnungen.add(d.dataset.id):this.selectedRechnungen.delete(d.dataset.id)}),this.updateSelectionUI()}),document.getElementById("btn-select-all")?.addEventListener("click",s=>{s.preventDefault(),document.querySelectorAll(".rechnung-check").forEach(d=>{d.checked=!0,this.selectedRechnungen.add(d.dataset.id)});const o=document.getElementById("select-all-rechnungen");o&&(o.indeterminate=!1,o.checked=!0),this.updateSelectionUI()}),document.getElementById("btn-deselect-all")?.addEventListener("click",s=>{s.preventDefault(),document.querySelectorAll(".rechnung-check").forEach(d=>{d.checked=!1}),this.selectedRechnungen.clear();const o=document.getElementById("select-all-rechnungen");o&&(o.indeterminate=!1,o.checked=!1),this.updateSelectionUI()}),document.getElementById("btn-delete-selected")?.addEventListener("click",async s=>{s.preventDefault(),await this.deleteSelected()})}updateHeaderSelectAll(){const e=document.getElementById("select-all-rechnungen"),t=document.querySelectorAll(".rechnung-check");if(!e||t.length===0)return;const n=document.querySelectorAll(".rechnung-check:checked").length;e.checked=n===t.length,e.indeterminate=n>0&&n<t.length}updateSelectionUI(){const e=this.selectedRechnungen.size,t=document.getElementById("selected-count"),n=document.getElementById("btn-deselect-all"),a=document.getElementById("btn-delete-selected");t&&(t.textContent=`${e} ausgewählt`,t.style.display=e>0?"inline":"none"),n&&(n.style.display=e>0?"inline-block":"none"),a&&(a.style.display=e>0?"inline-block":"none")}async deleteSelected(){const e=Array.from(this.selectedRechnungen);if(e.length===0||!confirm(e.length===1?"Ausgewählte Rechnung löschen?":`${e.length} Rechnungen löschen?`))return;let n=0,a=0;for(const r of e)try{(await window.dataService.deleteEntity("rechnung",r)).success?n++:a++}catch{a++}alert(`Löschvorgang abgeschlossen: ${n} erfolgreich${a?`, ${a} fehlgeschlagen`:""}.`),this.selectedRechnungen.clear(),await this.loadAndRender()}showCreateForm(){window.navigateTo("/rechnung/new")}}const lt=new ot;class dt{constructor(){this.id=null,this.data=null,this.belege=[]}async init(e){if(this.id=e,e==="new")return this.showCreateForm();await this.load(),this.render(),this.bindEvents()}async load(){const{data:e,error:t}=await window.supabase.from("rechnung").select(`*,
        unternehmen:unternehmen_id(id, firmenname),
    ...
    `,window.formSystem.bindFormEvents("rechnung",null);const t=document.getElementById("rechnung-form");t&&(t.onsubmit=async n=>{n.preventDefault(),await this.handleCreateSubmit()})}async handleCreateSubmit(){try{const e=document.getElementById("rechnung-form"),t=new FormData(e),n={};for(const[o,d]of t.entries())n[o]=d;const a=e.querySelector('.uploader[data-name="pdf_file"]');let r=null,i=null;if(a&&a.__uploaderInstance&&a.__uploaderInstance.files.length&&window.supabase){const o=a.__uploaderInstance.files[0],d="rechnungen",c=`${n.unternehmen_id||"unknown"}/${Date.now()}_${o.name}`,{error:l}=await window.supabase.storage.from(d).upload(c,o,{cacheControl:"3600",upsert:!1,contentType:o.type});if(l)throw l;const{data:u}=window.supabase.storage.from(d).getPublicUrl(c);r=u.publicUrl,i=c}r&&(n.pdf_url=r,n.pdf_path=i);const s=await window.dataService.createEntity("rechnung",n);if(s.success){try{const o=e.querySelector('.uploader[data-name="belege_files"]');if(o&&o.__uploaderInstance&&o.__uploaderInstance.files.length&&window.supabase){const d=s.id,c=Array.from(o.__uploaderInstance.files);for(const l of c){const u="rechnung-belege",m=`${d}/${Date.now()}_${Math.random().toString(36).slice(2)}_${l.name}`,{error:h}=await window.supabase.storage.from(u).upload(m,l,{cacheControl:"3600",upsert:!1,contentType:l.type});if(h)throw h;const{data:g}=await window.supabase.storage.from(u).createSignedUrl(m,3600*24*7),b=g?.signedUrl||"";await window.supabase.from("rechnung_belege").insert({rechnung_id:d,file_name:l.name,file_path:m,file_url:b,content_type:l.type,size:l.size,uploaded_by:window.currentUser?.id||null})}}}catch(o){console.warn("⚠️ Belege-Upload teilweise fehlgeschlagen:",o)}alert("Rechnung erstellt"),window.navigateTo(`/rechnung/${s.id}`)}else throw new Error(s.error||"Unbekannter Fehler")}catch(e){alert(`Fehler: ${e.message}`)}}bindEvents(){document.addEventListener("click",e=>{e.target.id==="btn-edit-rechnung"&&this.showEditForm()})}showEditForm(){const e=window.formSystem.renderFormOnly("rechnung",this.data);window.content.innerHTML=`
      <div class="page-header">
        <div class="page-header-left">
      ...
      <div class="form-page">${e}</div>
    `,window.formSystem.bindFormEvents("rechnung",this.data)}}const ct=new dt;class ut{constructor(){this.rows=[]}async init(){if(window.setHeadline("Mitarbeiter"),!(window.currentUser?.rolle==="admin"||window.canViewPage?.("mitarbeiter")||window.checkUserPermission("dashboard","can_view"))){window.content.innerHTML=`
        <div class="error-message">
          <p>Keine Berechtigung.</p>
        </div>
      `;return}await this.load(),await this.render(),this.bind()}async load(){try{if(window.supabase){const{data:e,error:t}=await window.supabase.rpc("execute_sql",{sql_query:`
            ...
            order by u.name`});if(t){console.warn("⚠️ Mitarbeiter-Liste via RPC fehlgeschlagen, fallback ohne Email",t);const{data:n}=await window.supabase.from("benutzer").select("id, name, rolle, unterrolle, mitarbeiter_klasse:mitarbeiter_klasse_id(name)").order("name");this.rows=n||[]}else this.rows=(e||[]).map(n=>({id:n.id,name:n.name,rolle:n.rolle,unterrolle:n.unterrolle,email:n.email,mitarbeiter_klasse:{name:n.klasse_name}}))}else this.rows=await window.dataService.loadEntities("benutzer")}catch(e){console.error("❌ Fehler beim Laden der Mitarbeiter:",e),this.rows=[]}}async render(){const t=`
      <div class="page-header">
        <div class="page-header-left">
          <h1>Mitarbeiter</h1>
              ...
              <div class="action-submenu">
                ...
                <div class="submenu" data-submenu="status">
                  ${(this.statusOptions||[]).map(n=>`
                    <a href="#" class="submenu-item" data-action="set-field" data-field="status_id" data-value="${n.id}" data-status-name="${n.name.replace(/\"/g,'\\"')}" data-id="${t.id}">${z.getStatusIcon(n.name)}<span>${n.name}</span></a>
                  `).join("")}
                </div>
              </div>
    ...
    `;window.setContentSafely(window.content,t)}formatCurrency(e){const t=Number(e||0);return new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(t)}renderBudget(){const e=(this.assignments.kooperationen||[]).map(r=>{const i=this.budget.invoicesByKoop[r.id]||[],s=i.length?i.map(l=>`<div><a href="/rechnung/${l.id}" onclick="event.preventDefault(); window.navigateTo('/rechnung/${l.id}')">${window.validatorSystem.sanitizeHtml(l.rechnung_nr||l.id)}</a> — ${this.formatCurrency(l.bruttobetrag)} <span class="status-badge status-${(l.status||"").toLowerCase().replace(/\s+/g,"-")}">${l.status||"-"}</span></div>`).join(""):'<span class="muted">Keine Rechnung</span>',o=Number(r.nettobetrag||0),d=Number(r.zusatzkosten||0),c=r.gesamtkosten!=null?Number(r.gesamtkosten):o+d;return`
        <tr>
          ...
          <td style="text-align:right;">${this.formatCurrency(d)}</td>
...
`)}`)),alert(d),this.genericDeselectAll(),this.currentListInstance&&typeof this.currentListInstance.loadAndRender=="function"?await this.currentListInstance.loadAndRender():window.location.reload(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:e,action:"bulk-deleted",count:r}}))}detectCurrentEntityType(){const e=window.location.pathname.split("/").filter(n=>n);if(e.length>0){const n=e[0];if(["creator","unternehmen","kampagne","marke","auftrag","ansprechpartner","kooperation"].includes(n))return n}const t=["creator","unternehmen","kampagne","marke","auftrag","ansprechpartner","kooperation"];for(const n of t){const a=this.getEntityConfig(n);if(document.querySelectorAll(a.checkboxSelector).length>0)return console.log(`🔧 BulkActionSystem: Entity-Type ${n} aus Checkboxen erkannt`),n}return null}getEntityConfig(e){return{creator:{checkboxSelector:".creator-check",selectAllId:"select-all-creators",displayName:"Creator"},unternehmen:{checkboxSelector:".unternehmen-check",selectAllId:"select-all-unternehmen",displayName:"Unternehmen"},kampagne:{checkboxSelector:".kampagne-check",selectAllId:"select-all-kampagnen",displayName:"Kampagnen"},marke:{checkboxSelector:".marke-check",selectAllId:"select-all-marken",displayName:"Marken"},auftrag:{checkboxSelector:".auftrag-check",selectAllId:"select-all-auftraege",displayName:"Aufträge"},ansprechpartner:{checkboxSelector:".ansprechpartner-check",selectAllId:"select-all-ansprechpartner",displayName:"Ansprechpartner"},kooperation:{checkboxSelector:".kooperation-check",selectAllId:"select-all-kooperationen",displayName:"Kooperationen"}}[e]||{checkboxSelector:`.${e}-check`,selectAllId:`select-all-${e}`,displayName:e}}hideButtons(){const e=document.getElementById("selected-count"),t=document.getElementById("btn-deselect-all"),n=document.getElementById("btn-delete-selected");e&&(e.style.display="none"),t&&(t.style.display="none"),n&&(n.style.display="none")}destroy(){this.boundEventListeners.forEach(({element:e,type:t,handler:n})=>{e.removeEventListener(t,n)}),this.boundEventListeners.clear()}}const we=new gt;class bt{constructor(){this.notifications=[],this.unreadCount=0,this.pollIntervalMs=6e4,this._timer=null,this._initialized=!1}init(){this._initialized||(this._initialized=!0,this.bindUI(),this.refresh(!0),this.startPolling(),window.addEventListener("notificationsRefresh",()=>this.refresh()))}destroy(){this._timer&&clearInterval(this._timer),this._timer=null,this._initialized=!1}startPolling(){this._timer&&clearInterval(this._timer),this._timer=setInterval(()=>this.refresh(),this.pollIntervalMs)}bindUI(){const e=document.querySelector("#notificationBell, .notification-bell"),t=document.querySelector("#notificationDropdown, .notification-dropdown");if(!e||!t)return;const n=()=>{t.classList.toggle("show")};e.addEventListener("click",a=>{a.preventDefault(),n()}),document.addEventListener("click",a=>{!t.contains(a.target)&&!e.contains(a.target)&&t.classList.remove("show")})}async refresh(e=!1){try{if(!window.supabase||!window.currentUser?.id)return;const{data:t,error:n}=await window.supabase.from("notifications").select("id, user_id, type, entity, entity_id, title, message, created_at, read_at").eq("user_id",window.currentUser.id).order("created_at",{ascending:!1}).limit(20);if(n)throw n;this.notifications=t||[],this.unreadCount=(this.notifications||[]).filter(a=>!a.read_at).length,this.renderBadge(),this.renderDropdown(),!e&&this.unreadCount>0}catch(t){console.warn("⚠️ Notifications refresh failed",t)}}renderBadge(){const e=document.querySelector("#notificationBadge, .notification-badge");if(!e)return;const t=this.unreadCount;e.textContent=t>99?"99+":String(t||""),e.style.display=t>0?"":"none"}renderDropdown(){const e=document.querySelector("#notificationDropdown, .notification-dropdown");if(!e)return;const t=(this.notifications||[]).map(a=>{const r=this.getEntityUrl(a.entity,a.entity_id),i=new Date(a.created_at).toLocaleString("de-DE");return`
      ...
      </div>`;e.innerHTML=`<div class="notification-list">${t||'<div class="notification-empty">Keine Benachrichtigungen</div>'}</div>${n}`,e.querySelector("#markAllRead")?.addEventListener("click",async a=>{a.preventDefault(),await this.markAllAsRead(),this.refresh()}),e.querySelectorAll(".notification-open").forEach(a=>{a.addEventListener("click",async r=>{r.preventDefault();const s=r.target.closest(".notification-item")?.dataset?.id;s&&await this.markAsRead(s);const o=a.getAttribute("data-route");o&&window.navigateTo&&window.navigateTo(o),e.classList.remove("show")})})}getEntityUrl(e,t){return!e||!t?"":{kampagne:`/kampagne/${t}`,kooperation:`/kooperation/${t}`,briefing:`/briefing/${t}`,auftrag:`/auftrag/${t}`}[e]||""}async markAsRead(e){try{if(!window.supabase)return;await window.supabase.from("notifications").update({read_at:new Date().toISOString()}).eq("id",e)}catch(t){console.warn("⚠️ markAsRead failed",t)}}async markAllAsRead(){try{if(!window.supabase||!window.currentUser?.id)return;await window.supabase.from("notifications").update({read_at:new Date().toISOString()}).is("read_at",null).eq("user_id",window.currentUser.id)}catch(e){console.warn("⚠️ markAllAsRead failed",e)}}async pushNotification(e,t){try{if(!window.supabase||!e)return;const n={user_id:e,type:t?.type||"info",entity:t?.entity||null,entity_id:t?.entityId||null,title:t?.title||"Benachrichtigung",message:t?.message||"",created_at:new Date().toISOString()};await window.supabase.from("notifications").insert(n)}catch(n){console.warn("⚠️ pushNotification failed",n)}}escape(e){if(!e)return"";const t=document.createElement("div");return t.textContent=e,t.innerHTML}}const ve=new bt;class ft{constructor(){this.modules=new Map,this.currentModule=null}register(e,t){this.modules.set(e,t)}navigateTo(e){const t=e.replace(/^\//,""),[n,a]=t.split("/");this.currentModule&&this.currentModule.destroy&&(console.log("🗑️ Zerstöre aktuelles Modul:",this.currentModule.constructor.name),this.currentModule.destroy(),this.currentModule=null);let r=n,i=this.modules.get(r);if(a&&n==="creator"&&a!=="new"&&(r="creator-detail",i=this.modules.get(r),console.log(`🎯 Creator-Details erkannt, verwende Modul: ${r}`)),n==="creator-lists"&&a&&(r="creator-list-detail",i=this.modules.get(r),console.log("🎯 Creator-Listen-Detail erkannt")),a&&n==="mitarbeiter"&&a!=="new"&&(r="mitarbeiter-detail",i=this.modules.get(r),console.log(`🎯 Mitarbeiter-Details erkannt, verwende Modul: ${r}`)),a&&n==="marke"&&a!=="new"&&(r="marke-detail",i=this.modules.get(r),console.log(`🎯 Marken-Details erkannt, verwende Modul: ${r}`)),a==="neu"&&n==="unternehmen"?(r="unternehmen-create",i=this.modules.get(r),console.log(`🎯 Unternehmen-Erstellung erkannt, verwende Modul: ${r}`)):a&&n==="unternehmen"&&a!=="new"&&a!=="neu"&&(r="unternehmen-detail",i=this.modules.get(r),console.log(`🎯 Unternehmen-Details erkannt, verwende Modul: ${r}`)),a&&n==="auftrag"&&a!=="new"&&(r="auftrag-detail",i=this.modules.get(r),console.log(`🎯 Auftrags-Details erkannt, verwende Modul: ${r}`)),a&&n==="kooperation"&&a!=="new"&&(r="kooperation-detail",i=this.modules.get(r),console.log(`🎯 Kooperations-Details erkannt, verwende Modul: ${r}`)),a&&n==="video"&&a!=="new"&&(r="kooperation-video-detail",i=this.modules.get(r),console.log(`🎯 Video-Details erkannt, verwende Modul: ${r}`)),a&&n==="kampagne"&&a!=="new"&&(r="kampagne-detail",i=this.modules.get(r),console.log(`🎯 Kampagnen-Details erkannt, verwende Modul: ${r}`)),a&&n==="briefing"&&a!=="new"&&(r="briefing-detail",i=this.modules.get(r),console.log(`🎯 Briefing-Details erkannt, verwende Modul: ${r}`)),a&&n==="rechnung"&&a!=="new"&&(r="rechnung-detail",i=this.modules.get(r),console.log(`🎯 Rechnung-Details erkannt, verwende Modul: ${r}`)),a&&n==="ansprechpartner"&&(r="ansprechpartner-detail",i=this.modules.get(r),console.log(`🎯 Ansprechpartner-Details/Erstellung erkannt, verwende Modul: ${r}`)),i)return console.log(`✅ Modul gefunden: ${r}`,i),this.currentModule=i,a==="new"?n==="rechnung"?(r="rechnung-detail",i=this.modules.get(r),i.init?.("new")):(console.log(`📝 Zeige Erstellungsformular für: ${n}`),r.includes("-detail")?i.init?.("new"):i.showCreateForm?.()):a?(console.log(`👁️ Zeige Details für: ${n}/${a}`),i.init?.(a)):(console.log(`🚀 Initialisiere Modul: ${n}`),i.init());console.warn(`❌ Modul nicht gefunden: ${r}`),this.loadDashboard()}loadDashboard(){window.setHeadline("Dashboard"),window.content.innerHTML=`
      <div class="dashboard">
        <h1>Willkommen im CRM</h1>
        <p>Dashboard wird geladen...</p>
      </div>
    `}}const S=new ft;window.moduleRegistry=S;S.register("creator",P);S.register("creator-detail",V);S.register("creator-lists",$e);S.register("creator-list-detail",Le);S.register("unternehmen",j);S.register("unternehmen-create",Z);S.register("auftrag",G);S.register("marke",W);S.register("marke-detail",Y);S.register("marke-create",K);S.register("unternehmen-detail",oe);S.register("auftrag-detail",le);S.register("kooperation",de);S.register("kooperation-detail",ce);S.register("kooperation-video-detail",ue);S.register("kampagne",me);S.register("kampagne-detail",he);S.register("ansprechpartner",be);S.register("ansprechpartner-detail",fe);S.register("ansprechpartner-create",q);S.register("briefing",pe);S.register("briefing-detail",ge);S.register("rechnung",lt);S.register("rechnung-detail",ct);S.register("mitarbeiter",mt);S.register("mitarbeiter-detail",pt);window.navigateTo=p=>{S.navigateTo(p)};window.filterSystem=k;window.creatorList=P;window.creatorDetail=V;window.unternehmenList=j;window.unternehmenCreate=Z;window.auftragList=G;window.markeList=W;window.markeDetail=Y;window.markeCreate=K;window.unternehmenDetail=oe;window.auftragDetail=le;window.kooperationList=de;window.kooperationDetail=ce;window.kooperationVideoDetail=ue;window.kampagneList=me;window.kampagneDetail=he;window.briefingDetail=ge;window.kampagneUtils=et;window.briefingList=pe;window.AuthService=R;window.AuthUtils=H;window.authService=R;window.authUtils=H;window.navigationSystem=Q;window.permissionSystem=ke;window.dataService=T;window.validatorSystem=M;window.creatorUtils=X;window.formSystem=se;window.notizenSystem=Pe;window.bewertungsSystem=je;window.ActionsDropdown=z;window.bulkActionSystem=we;window.notificationSystem=ve;window.ansprechpartnerList=be;window.ansprechpartnerDetail=fe;window.ansprechpartnerCreate=q;document.addEventListener("DOMContentLoaded",async()=>{if(console.log("🎯 Initialisiere Event-basiertes Modul-System..."),window.appRoot=document.getElementById("app-root"),window.loginRoot=document.getElementById("login-root"),window.content=document.getElementById("dashboard-content"),window.nav=document.getElementById("main-nav"),window.supabase&&window.CONFIG?.SUPABASE?.URL&&window.CONFIG?.SUPABASE?.KEY)try{window.supabase=window.supabase.createClient(window.CONFIG.SUPABASE.URL,window.CONFIG.SUPABASE.KEY),console.log("✅ Supabase initialisiert")}catch(e){console.error("❌ Supabase-Initialisierung fehlgeschlagen:",e)}else console.warn("⚠️ Supabase nicht verfügbar - verwende Offline-Modus");if(await R.checkAuth()){console.log("✅ Benutzer ist authentifiziert"),Q.init(),z.init(),we.init(),ve.init(),window.appRoot.style.display="",window.loginRoot.style.display="none";const e=location.pathname;e==="/"||e==="/dashboard"?S.loadDashboard():S.navigateTo(e),window.setupHeaderUI?.()}else console.log("🔐 Benutzer nicht authentifiziert - zeige Login"),H.showLogin()});window.handleLogout=async()=>{try{await R.signOut()}catch(p){console.warn("Logout warn:",p)}finally{window.appRoot&&(window.appRoot.style.display="none"),window.loginRoot&&(window.loginRoot.style.display="");const p=document.getElementById("mitarbeiterPanelOverlay"),e=document.getElementById("mitarbeiterPanel");p&&(p.style.display="none"),e&&(e.style.display="none"),H.showLogin?.()}};window.setupHeaderUI=()=>{try{const p=document.querySelector(".profile-img"),e=document.querySelector(".profile-initials"),t=(window.currentUser?.name||"").trim();if(t&&e){const r=t.split(/\s+/).filter(Boolean),i=(r[0]?.[0]||"").toUpperCase()+(r[1]?.[0]||"").toUpperCase();e.textContent=i||(t[0]||"?").toUpperCase()}window.currentUser?.avatar_url&&p&&(p.src=window.currentUser.avatar_url,p.style.display="",e&&(e.style.display="none"));const n=document.querySelector(".quick-menu-btn"),a=document.querySelector(".quick-menu-dropdown");if(n&&a&&!n.dataset.bound){const r=()=>{a.classList.remove("show"),n.setAttribute("aria-expanded","false")};n.addEventListener("click",s=>{s.preventDefault();const o=a.classList.toggle("show");n.setAttribute("aria-expanded",String(o))}),document.addEventListener("click",s=>{!a.contains(s.target)&&!n.contains(s.target)&&r()});const i={unternehmen:"unternehmen",marke:"marke",auftrag:"auftrag",ansprechpartner:"ansprechpartner",kampagne:"kampagne",briefing:"briefing",kooperation:"kooperation",creator:"creator"};a.querySelectorAll(".quick-menu-item").forEach(s=>{const o=s.getAttribute("data-entity"),d=i[o],c=d?!!window.permissionSystem?.getEntityPermissions(d)?.can_edit:!1;s.style.display=c||window.currentUser?.rolle==="admin"?"":"none"}),a.addEventListener("click",s=>{const o=s.target.closest(".quick-menu-item");if(!o)return;const d=o.getAttribute("data-entity");if(d){switch(d){case"unternehmen":window.navigateTo("/unternehmen/neu");break;case"marke":window.navigateTo("/marke/new");break;case"auftrag":window.navigateTo("/auftrag/new");break;case"ansprechpartner":window.navigateTo("/ansprechpartner/new");break;case"kampagne":window.navigateTo("/kampagne/new");break;case"briefing":window.navigateTo("/briefing/new");break;case"kooperation":window.navigateTo("/kooperation/new");break;case"creator":window.navigateTo("/creator/new");break}r()}}),n.dataset.bound="true"}}catch(p){console.warn("Header/Quick-Menu setup warn:",p)}};
