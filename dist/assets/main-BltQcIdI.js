...
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
    ...
    `}handleAction(e,t,n,a){switch(console.log(`🎯 ACTIONSDROPDOWN: Action ${e} für ${n} ${t}`),e){case"view":window.navigateTo(`/${n}/${t}`);break;case"edit":window.navigateTo(`/${n}/${t}/edit`);break;case"delete":this.confirmDelete(t,n);break;case"notiz":this.openNotizModal(t,n);break;case"rating":this.openRatingModal(t,n);break;case"marken":window.navigateTo(`/unternehmen/${t}/marken`);break;case"auftraege":window.navigateTo(`/unternehmen/${t}/auftraege`);break;case"kampagnen":window.navigateTo(`/auftrag/${t}/kampagnen`);break;case"quickview":this.openKooperationQuickView(t);break;case"assign-staff":if(window.currentUser?.rolle!=="admin"){alert("Nur Admins dürfen Mitarbeiter zuordnen.");break}this.openAssignStaffModal(t);break;case"rechnung":this.openRechnungModal(t,n);break;case"invoice-view":window.navigateTo(`/rechnung/${t}`);break;case"invoice-edit":window.navigateTo(`/rechnung/${t}/edit`);break;case"add_to_campaign":this.openAddToCampaignModal(t);break;case"favorite":{const r=document.querySelector("[data-kampagne-id]")?.dataset?.kampagneId;this.addToFavorites(t,r);break}case"add_to_list":{this.openAddToListModal(t);break}case"add_ansprechpartner":{this.openAddAnsprechpartnerModal(t);break}case"add_ansprechpartner_kampagne":{this.openAddAnsprechpartnerToKampagneModal(t);break}case"unassign-kampagne":{const r=a?.dataset?.mitarbeiterId||window.location.pathname.split("/").pop();if(!r){alert("Mitarbeiter-ID nicht gefunden");break}const i=t;if(!i||!confirm("Zuweisung dieser Kampagne vom Mitarbeiter entfernen?"))break;(async()=>{try{const{error:s}=await window.supabase.from("kampagne_mitarbeiter").delete().eq("mitarbeiter_id",r).eq("kampagne_id",i);if(s)throw s;const o=a.closest("tr");o&&o.remove();const l=document.querySelector('.tab-button[data-tab="kampagnen"] .tab-count');if(l){const c=parseInt(l.textContent||"1",10);l.textContent=String(Math.max(0,c-1))}alert("Zuweisung entfernt")}catch(s){console.error("❌ Zuweisung entfernen fehlgeschlagen",s),alert("Entfernen fehlgeschlagen")}})();break}case"video-view":window.navigateTo(`/video/${t}`);break;case"video-edit":window.navigateTo(`/video/${t}`);break;case"video-delete":this.confirmDelete(t,"kooperation_videos");break;default:console.warn(`⚠️ Unbekannte Action: ${e}`)}}async openKooperationQuickView(e){try{const t=document.createElement("div");t.className="drawer-overlay";const n=document.createElement("div");n.setAttribute("role","dialog"),n.className="drawer-panel";const a=document.createElement("div");a.className="drawer-header";const r=document.createElement("div"),i=document.createElement("h1");i.textContent="Kooperation · Schnellansicht";const s=document.createElement("p");s.style.margin="0",s.style.color="#6b7280",s.textContent="Videos & Kommentare",r.appendChild(i),r.appendChild(s);const o=document.createElement("div"),l=document.createElement("button");l.className="drawer-close",l.id="kvq-close",l.textContent="Schließen",o.appendChild(l),a.appendChild(r),a.appendChild(o);const c=document.createElement("div");c.className="drawer-body";const d=document.createElement("div");d.className="detail-section";const u=document.createElement("h2");u.textContent="Videos";const h=document.createElement("div");h.id="kvq-table",h.textContent="Lade...",d.appendChild(u),d.appendChild(h),c.replaceChildren(d),n.appendChild(a),n.appendChild(c),document.body.appendChild(t),document.body.appendChild(n);const m=()=>{try{t.remove(),n.remove()}catch{}};t.addEventListener("click",m),a.querySelector("#kvq-close")?.addEventListener("click",m),document.addEventListener("keydown",function k(D){D.key==="Escape"&&(m(),document.removeEventListener("keydown",k))}),requestAnimationFrame(()=>{n.classList.add("show")});const{data:p}=await window.supabase.from("kooperation_videos").select("id, position, content_art, titel, asset_url, status, created_at").eq("kooperation_id",e).order("position",{ascending:!0}),b=p||[];let f={};if(b.length){const k=b.map(M=>M.id),{data:D}=await window.supabase.from("kooperation_video_comment").select("id, video_id, runde, text, author_name, created_at").in("video_id",k).order("created_at",{ascending:!0});(D||[]).forEach(M=>{const z=M.video_id;f[z]||(f[z]={r1:[],r2:[]});const I=M.runde===2||M.runde==="2"?"r2":"r1";f[z][I].push(M)})}const w=k=>window.validatorSystem?.sanitizeHtml?.(k)??k,v=k=>k?new Date(k).toLocaleDateString("de-DE"):"-",y=k=>!k||!k.length?"-":k.map(D=>{const M=w(D.text||""),z=w(D.author_name||"-"),I=v(D.created_at);return`<div class="fb-line"><span class="fb-meta">${z} • ${I}</span><div class="fb-text">${M}</div></div>`}).join(""),$=b.map(k=>{const D=f[k.id]||{r1:[],r2:[]},M=k.asset_url?`<a class="kvq-link-btn" href="${k.asset_url}" target="_blank" rel="noopener">${this.getHeroIcon("view")}<span>Öffnen</span></a>`:"-";return`
        ...
        </div>`:'<p class="empty-state">Keine Videos vorhanden.</p>',A=c.querySelector("#kvq-table");if(A.innerHTML="",b.length){const k=document.createElement("div");k.className="data-table-container",k.innerHTML=E,A.appendChild(k)}else{const k=document.createElement("p");k.className="empty-state",k.textContent="Keine Videos vorhanden.",A.appendChild(k)}this.normalizeIcons(c)}catch(t){console.error("❌ Quickview öffnen fehlgeschlagen",t),alert("Schnellansicht konnte nicht geöffnet werden.")}}async openAssignStaffModal(e){const t=document.createElement("div");t.className="modal overlay-modal",t.innerHTML=`
      ...
      </div>`,document.body.appendChild(a);const r=a.querySelector("#ansprechpartner-search"),i=a.querySelector("#ansprechpartner-dropdown");let s=null;const o=(u="")=>{if(!u||u.trim().length===0){i.innerHTML='<div class="dropdown-item no-results">Beginnen Sie zu tippen, um Ansprechpartner zu suchen...</div>';return}const h=u.toLowerCase(),m=t.filter(p=>{const b=`${p.vorname} ${p.nachname}`.toLowerCase(),f=(p.email||"").toLowerCase(),w=(p.unternehmen?.firmenname||"").toLowerCase();return b.includes(h)||f.includes(h)||w.includes(h)});i.innerHTML=m.length?m.map(p=>{const b=`${p.vorname} ${p.nachname}`,f=[p.email,p.unternehmen?.firmenname,p.position?.name].filter(Boolean).join(" • ");return`<div class="dropdown-item" data-id="${p.id}">
    ...
    `).join(""),L.normalizeIcons(document)}destroy(){}}const Me=new Fe;function Be(g,e={}){const{showActions:t=!1}=e,n=i=>i?new Date(i).toLocaleDateString("de-DE"):"-",a=i=>Array.isArray(i)&&i.length>0?i.join(", "):"-",r=(g||[]).map(i=>{const s=i.id,o=i.kampagnenname||"Unbekannt",l=i.unternehmen?.firmenname||"-",c=i.marke?.markenname||"-",d=a(i.art_der_kampagne),u=i.status||"-",h=n(i.start),m=n(i.deadline),p=i.creatoranzahl??"-",b=i.videoanzahl??"-";return`
      <tr data-id="${s||""}">
        <td>
  ...
  `}class Te{constructor(){this.creatorId=null,this.creator=null,this.notizen=[],this.ratings=[],this.kampagnen=[],this.lists=[],this.kooperationen=[],this.rechnungen=[]}async init(e){if(console.log("🎯 CREATORDETAIL: Initialisiere Creator-Detailseite für ID:",e),this.creatorId=e,window.moduleRegistry?.currentModule!==this){console.log("⚠️ CREATORDETAIL: Nicht mehr das aktuelle Modul, breche ab");return}try{await this.loadCreatorData(),await this.render(),this.bindEvents(),console.log("✅ CREATORDETAIL: Initialisierung abgeschlossen")}catch(t){console.error("❌ CREATORDETAIL: Fehler bei der Initialisierung:",t),window.ErrorHandler.handle(t,"CreatorDetail.init")}}async loadCreatorData(){console.log("🔄 CREATORDETAIL: Lade Creator-Daten...");const{data:e,error:t}=await window.supabase.from("creator").select("*").eq("id",this.creatorId).single();if(t)throw new Error(`Fehler beim Laden der Creator-Daten: ${t.message}`);this.creator=e,console.log("✅ CREATORDETAIL: Creator-Basisdaten geladen:",e);try{const{data:s}=await window.supabase.from("creator_sprachen").select("sprache_id, sprachen:sprache_id(id, name)").eq("creator_id",this.creatorId);this.creator.sprachen=(s||[]).map(o=>o.sprachen).filter(Boolean)}catch{}try{const{data:s}=await window.supabase.from("creator_branchen").select("branche_id, branchen_creator:branche_id(id, name)").eq("creator_id",this.creatorId);this.creator.branchen=(s||[]).map(o=>o.branchen_creator).filter(Boolean)}catch{}try{const{data:s}=await window.supabase.from("creator_creator_type").select("creator_type_id, creator_type:creator_type_id(id, name)").eq("creator_id",this.creatorId);this.creator.creator_types=(s||[]).map(o=>o.creator_type).filter(Boolean)}catch{}this.notizen=await window.notizenSystem.loadNotizen("creator",this.creatorId),console.log("✅ CREATORDETAIL: Notizen geladen:",this.notizen.length),this.ratings=await window.bewertungsSystem.loadBewertungen("creator",this.creatorId),console.log("✅ CREATORDETAIL: Ratings geladen:",this.ratings.length);const{data:n,error:a}=await window.supabase.from("kampagne_creator").select(`
    ...
    `;window.setContentSafely(window.content,e)}renderTagList(e){if(!e||e.length===0)return"-";if(Array.isArray(e))return`<div class="tags">${e.map(n=>{const a=typeof n=="object"&&(n.name||n.label)||n;return`<span class="tag">${String(a).trim()}</span>`}).join("")}</div>`;if(typeof e=="object"){const t=e.name||e.label;return t?`<div class="tags"><span class="tag">${t}</span></div>`:"-"}return`<div class="tags"><span class="tag">${String(e)}</span></div>`}renderNotizen(){return window.notizenSystem.renderNotizenContainer(this.notizen,"creator",this.creatorId)}renderRatings(){return window.bewertungsSystem.renderBewertungenContainer(this.ratings,"creator",this.creatorId)}renderKampagnen(){if(!this.kampagnen||this.kampagnen.length===0)return`
      ...
      `;const e=this.kampagnen.map(t=>{const n=t.kampagne||t;return{id:n.id,kampagnenname:n.kampagnenname,unternehmen:n.unternehmen||null,marke:n.marke||null,art_der_kampagne:n.art_der_kampagne,status:n.status,start:n.start,deadline:n.deadline,creatoranzahl:n.creatoranzahl,videoanzahl:n.videoanzahl}});return Be(e,{showActions:!1})}renderLists(){return this.lists.length===0?`
        <div class="empty-state">
    ...
    `}destroy(){}}const Ie=new Ce;class xe{constructor(){this.selectedUnternehmen=new Set,this._boundEventListeners=new Set}async init(){if(window.setHeadline("Unternehmen Übersicht"),!(window.canViewPage&&window.canViewPage("unternehmen")||await window.checkUserPermission("unternehmen","can_view"))){window.content.innerHTML=`
        <div class="error-message">
...
${a} Marken konnten nicht gelöscht werden.`):alert(i),r.forEach(o=>{const l=document.querySelector(`tr[data-id="${o}"]`);l&&l.remove()}),this.selectedMarken.clear(),this.updateSelection(),this.updateSelectAllCheckbox();const s=document.getElementById("marken-table-body");s&&s.children.length===0&&await this.loadAndRender(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:"marke",action:"bulk-deleted",count:n}}))}else alert("Keine Marken konnten gelöscht werden.")}showCreateForm(){console.log("🎯 Zeige Marken-Erstellungsformular mit MarkeCreate"),U.showCreateForm()}}const Q=new qe;class He{constructor(){this.markeId=null,this.marke=null,this.notizen=[],this.ratings=[],this.kampagnen=[],this.auftraege=[],this.ansprechpartner=[],this.rechnungen=[]}async init(e){console.log("🎯 MARKENDETAIL: Initialisiere Marken-Detailseite für ID:",e);try{this.markeId=e,await this.loadMarkeData(),this.render(),this.bindEvents(),console.log("✅ MARKENDETAIL: Initialisierung abgeschlossen")}catch(t){console.error("❌ MARKENDETAIL: Fehler bei der Initialisierung:",t),window.ErrorHandler.handle(t,"MarkeDetail.init")}}async loadMarkeData(){console.log("🔄 MARKENDETAIL: Lade Marken-Daten...");try{const{data:e,error:t}=await window.supabase.from("marke").select(`
    ...
    `}renderField(e,t=""){const n=`field-${e.name}`,a=e.required?"required":"",r=e.required?'<span class="required">*</span>':"";switch(e.type){case"text":case"email":case"tel":case"url":const i=e.dependsOn?`data-depends-on="${e.dependsOn}"`:"",s=e.showWhen?`data-show-when="${e.showWhen}"`:"",o=e.autoGenerate?'data-auto-generate="true"':"",l=e.readonly?"readonly":"",c=e.placeholder?`placeholder="${e.placeholder}"`:"",d=e.dependsOn?'style="display: none;"':"";return`
        ...
        `;case"number":const u=e.readonly?"readonly":"",h=e.autoCalculate?'data-auto-calculate="true"':"",m=e.calculatedFrom?`data-calculated-from="${e.calculatedFrom.join(",")}"`:"",p=e.placeholder?`placeholder="${e.placeholder}"`:"",b=e.validation?.min!==void 0?`min="${e.validation.min}"`:"",f=e.validation?.max!==void 0?`max="${e.validation.max}"`:"",w=e.validation?.step!==void 0?`step="${e.validation.step}"`:'step="0.01"',v=e.defaultValue!==void 0&&(t===""||t===void 0||t===null)?`value="${e.defaultValue}"`:`value="${t}"`;return`
          <div class="form-field">
            <label for="${n}">${e.label} ${r}</label>
            <input type="number" id="${n}" name="${e.name}" ${v} ${a} ${u} ${h} ${m} ${p} ${b} ${f} ${w}>
        ...
        `;case"textarea":const y=e.dependsOn?`data-depends-on="${e.dependsOn}"`:"",$=e.showWhen?`data-show-when="${e.showWhen}"`:"";return`
          <div class="form-field form-field-full" ${y} ${$}>
            <label for="${n}">${e.label} ${r}</label>
            <textarea id="${n}" name="${e.name}" rows="4" ${a}>${this.validator.sanitizeHtml(t)}</textarea>
          </div>
        `;case"select":let E="";if(e.dynamic?E=`<option value="">${e.dependsOn?`Erst ${this.getFieldLabel(e.dependsOn)} auswählen...`:e.placeholder||"Bitte wählen..."}</option>`:E=e.options.map(F=>`<option value="${F}" ${t===F?"selected":""}>${F}</option>`).join(""),e.searchable){const F=e.dependsOn?"disabled":"",T=e.readonly?"disabled":F,j=e.dependsOn?`Erst ${this.getFieldLabel(e.dependsOn)} auswählen...`:e.placeholder||"Bitte wählen...",N=[];e.table&&N.push(`data-table="${e.table}"`),e.displayField&&N.push(`data-display-field="${e.displayField}"`),e.valueField&&N.push(`data-value-field="${e.valueField}"`);const Ae=N.join(" ");return`
            <div class="form-field">
              <label for="${n}">${e.label} ${r}</label>
              <select id="${n}" name="${e.name}" ${a} ${T} data-searchable="true" data-placeholder="${j}" ${e.readonly?'data-readonly="true"':""} ${Ae}>
                ${E}
              </select>
              ${e.readonly?`<input type="hidden" name="${e.name}" id="${n}-hidden" value="">`:""}
            </div>
          `}const A=e.dependsOn?"disabled":"",k=e.readonly?"disabled":A,D=[];e.table&&D.push(`data-table="${e.table}"`),e.displayField&&D.push(`data-display-field="${e.displayField}"`),e.valueField&&D.push(`data-value-field="${e.valueField}"`);const M=D.join(" ");return`
          <div class="form-field">
            <label for="${n}">${e.label} ${r}</label>
            <select id="${n}" name="${e.name}" ${a} ${k} ${e.readonly?'data-readonly="true"':""} ${M}>
              ${E}
            </select>
            ${e.readonly?`<input type="hidden" name="${e.name}" id="${n}-hidden" value="">`:""}
          </div>
        `;case"multiselect":const z=Array.isArray(t)?t:t?t.split(","):[];let I="";if(e.dynamic||(I=e.options.map(F=>{const T=z.includes(F)?"selected":"";return`<option value="${F}" ${T}>${F}</option>`}).join("")),e.searchable){const F=z.length>0?`data-existing-values='${JSON.stringify(z)}'`:"";return`
            <div class="form-field">
              <label for="${n}">${e.label} ${r}</label>
              <select id="${n}" name="${e.name}" ${a} multiple data-searchable="true" data-tag-based="${e.tagBased||"false"}" data-placeholder="${e.placeholder||"Bitte wählen..."}" ${F}>
                ${I}
              </select>
            </div>
          `}return`
        ...
        `;case"toggle":const Se=t==="on"||t===!0||t==="true"||t===1?"checked":"";return`
          <div class="form-field">
            <label class="toggle-container">
              <span>${e.label} ${r}</span>
              <div class="toggle-switch">
                <input type="checkbox" id="${n}" name="${e.name}" ${Se} ${a}>
                <span class="toggle-slider"></span>
              </div>
            </label>
          </div>
        `;case"custom":if(e.customType==="addresses")return this.renderAddressesField(e,t);if(e.customType==="videos")return this.renderVideosField(e,t);if(e.customType==="file"){const F=e.accept?`accept="${e.accept}"`:"",T=e.multiple?"multiple":"";return`
            <div class="form-field">
              <label for="${n}">${e.label} ${r}</label>
              <input type="file" id="${n}" name="${e.name}" ${a} ${F} ${T}>
            </div>
          `}if(e.customType==="uploader"){const F=`uploader-${e.name}-${Math.random().toString(36).slice(2)}`;return setTimeout(()=>{const T=document.getElementById(F);T&&new Ve({multiple:!!e.multiple,accept:e.accept||"*/*"}).mount(T.querySelector(".uploader"))},0),`
...
`);alert(`Validierungsfehler:
${t}`)}showSuccessMessage(e){console.log(`✅ ${e}`)}showErrorMessage(e){console.error(`❌ ${e}`),alert(e)}};const le=new We;typeof window<"u"&&(window.formSystem=le);class Ye{constructor(){this.currentEntity=null,this.currentEntityId=null}async init(e,t){console.log("📝 NOTIZENSYSTEM: Initialisiere für",e,t),this.currentEntity=e,this.currentEntityId=t}async loadNotizen(e,t){console.log("🔄 NOTIZENSYSTEM: Lade Notizen für",e,t);try{const n=this.getNotizenTable(e);if(!n)return console.warn("⚠️ NOTIZENSYSTEM: Keine Notizen-Tabelle für",e),[];let a="*";n==="creator_notiz"&&(a+=`,
          kampagne:kampagne_id (
            id,
            kampagnenname,
            unternehmen:unternehmen_id (
  ...
  `}class et{constructor(){this.unternehmenId=null,this.unternehmen=null,this.notizen=[],this.ratings=[],this.marken=[],this.auftraege=[],this.ansprechpartner=[],this.kampagnen=[],this.kooperationen=[],this.creators=[],this.rechnungen=[],this._creatorMap={}}async init(e){console.log("🎯 UNTERNEHMENDETAIL: Initialisiere Unternehmen-Detailseite für ID:",e);try{this.unternehmenId=e,await this.loadUnternehmenData(),this.render(),this.bindEvents(),console.log("✅ UNTERNEHMENDETAIL: Initialisierung abgeschlossen")}catch(t){console.error("❌ UNTERNEHMENDETAIL: Fehler bei der Initialisierung:",t),window.ErrorHandler.handle(t,"UnternehmenDetail.init")}}async loadUnternehmenData(){console.log("🔄 UNTERNEHMENDETAIL: Lade Unternehmen-Daten...");try{const{data:e,error:t}=await window.supabase.from("unternehmen").select("*").eq("id",this.unternehmenId).single();if(t)throw t;this.unternehmen=e,console.log("✅ UNTERNEHMENDETAIL: Unternehmen-Basisdaten geladen:",this.unternehmen);try{const{data:l,error:c}=await window.supabase.from("unternehmen_branchen").select(`
    ...
    `;const n=document.querySelector(".form-page");n&&n.insertBefore(t,n.firstChild)}showErrorMessage(e){const t=document.createElement("div");t.className="alert alert-danger",t.textContent=e,t.style.cssText=`
      ...
      margin-bottom: 1rem;
      border: 1px solid #f5c6cb;
    `;const n=document.querySelector(".form-page");n&&n.insertBefore(t,n.firstChild)}destroy(){console.log("AuftragDetail: Cleaning up...")}}const ce=new tt;class nt{constructor(){this.selectedKooperation=new Set,this._boundEventListeners=new Set,this.statusOptions=[]}async init(){if(window.setHeadline("Kooperationen Übersicht"),!(window.canViewPage&&window.canViewPage("kooperation")||await window.checkUserPermission("kooperation","can_view"))){window.content.innerHTML=`
        <div class="error-message">
    ...
    `}showValidationErrors(e){console.error("❌ Validierungsfehler:",e),document.querySelectorAll(".validation-error").forEach(t=>t.remove()),Object.entries(e).forEach(([t,n])=>{const a=document.querySelector(`[name="${t}"]`);if(a){const r=document.createElement("div");r.className="validation-error",r.textContent=n,a.parentNode.appendChild(r)}})}showSuccessMessage(e){const t=document.createElement("div");t.className="alert alert-success",t.textContent=e;const n=document.getElementById("kooperation-form");n&&(n.parentNode.insertBefore(t,n),setTimeout(()=>{t.remove()},5e3))}showErrorMessage(e){const t=document.createElement("div");t.className="alert alert-danger",t.textContent=e;const n=document.getElementById("kooperation-form");n&&(n.parentNode.insertBefore(t,n),setTimeout(()=>{t.remove()},5e3))}formatNumber(e){return e?new Intl.NumberFormat("de-DE").format(e):"-"}formatCurrency(e){return e?new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(e):"-"}formatDate(e){return e?new Date(e).toLocaleDateString("de-DE"):"-"}destroy(){console.log("KooperationDetail: Cleaning up...")}}const he=new at,me={videoId:null,video:null,kooperation:null,comments:[],assets:[],async init(g){try{if(this.videoId=g==="new"?null:g,!this.videoId){window.setHeadline("Neues Video"),window.content.innerHTML='<p class="empty-state">Neuanlage für Videos ist noch nicht implementiert.</p>';return}await this.loadData(),this.render(),this.bindEvents()}catch(e){console.error("KooperationVideoDetail init error:",e),window.notificationSystem?.error?.("Video-Detail konnte nicht geladen werden.")}},async loadData(){const{data:g,error:e}=await window.supabase.from("kooperation_videos").select("id, kooperation_id, titel, content_art, asset_url, kommentar, status, position, created_at").eq("id",this.videoId).single();if(e)throw e;this.video=g;try{const{data:t}=await window.supabase.from("kooperationen").select("id, name, kampagne:kampagne_id(id, kampagnenname)").eq("id",g.kooperation_id).single();this.kooperation=t||null}catch{this.kooperation=null}try{const{data:t}=await window.supabase.from("kooperation_video_comment").select("id, video_id, runde, text, author_name, author_benutzer_id, created_at").eq("video_id",this.videoId).order("created_at",{ascending:!0});this.comments=t||[]}catch{this.comments=[]}try{const{data:t}=await window.supabase.from("kooperation_video_asset").select("id, file_url, file_path, created_at").eq("video_id",this.videoId).order("created_at",{ascending:!0});this.assets=t||[]}catch{this.assets=[]}},render(){const g=this.video||{},e=g.titel||`Video #${g.id}`,t=this.kooperation?.name||"-",n=this.kooperation?.kampagne?.kampagnenname||"-",a=h=>window.validatorSystem?.sanitizeHtml?.(h)??h,r=h=>h?new Date(h).toLocaleString("de-DE"):"-";typeof window.setHeadline=="function"&&window.setHeadline(`Video: ${a(e)}`);const i=window.currentUser?.permissions?.kooperation?.can_edit||window.currentUser?.rolle==="admin",s=g.asset_url||"",o=/\.(mp4|webm|ogg)(\?|$)/i.test(s),l=s?o?`<video src="${s}" controls style="width:100%;max-height:60vh;border-radius:8px;"></video>`:`<a href="${s}" target="_blank" rel="noopener" class="primary-btn">Asset öffnen</a>`:'<p class="empty-state">Kein Asset hinterlegt.</p>',c={r1:[],r2:[]};(this.comments||[]).forEach(h=>{const m=h.runde===2||h.runde==="2"?"r2":"r1";c[m].push(h)});const d=this.assets.length?this.assets.map(h=>`<li><a href="${h.file_url||"#"}" target="_blank" rel="noopener">${h.file_path||h.file_url}</a></li>`).join(""):"<li>Keine weiteren Assets.</li>",u=`
      <div class="page-header">
        <div class="page-header-left">
          <h1>${a(e)}</h1>
        ...
        <div class="page-header-right">
          ${g.kooperation_id?'<button id="btn-back-kooperation" class="secondary-btn">Zur Kooperation</button>':""}
        </div>
      </div>
      ...
      <div class="content-section">
        <div class="detail-grid">
          ...
          <div class="detail-card">
            <h3>Informationen</h3>
            <div class="detail-grid-2">
              <div class="detail-item"><label>Content-Art</label><span>${a(g.content_art||"-")}</span></div>
              <div class="detail-item"><label>Position</label><span>${g.position||"-"}</span></div>
              <div class="detail-item"><label>Status</label>
                <span class="status-badge status-${(g.status||"produktion").toLowerCase()}">${g.status==="abgeschlossen"?"Abgeschlossen":"Produktion"}</span>
              </div>
              <div class="detail-item"><label>Erstellt</label><span>${r(g.created_at)}</span></div>
            </div>
            ${i?`
            <div class="form-inline" style="margin-top:12px;gap:8px;display:flex;align-items:center;">
              ...
              <select id="video-status" class="form-input" style="max-width:220px;">
                <option value="produktion" ${g.status!=="abgeschlossen"?"selected":""}>Produktion</option>
                <option value="abgeschlossen" ${g.status==="abgeschlossen"?"selected":""}>Abgeschlossen</option>
              </select>
    ...
    `;window.setContentSafely(window.content,u)},renderCommentsTable(g){const e=a=>window.validatorSystem?.sanitizeHtml?.(a)??a,t=a=>a?new Date(a).toLocaleString("de-DE"):"-";return!g||g.length===0?'<p class="empty-state">Keine Kommentare vorhanden.</p>':`
      <div class="data-table-container">
        <table class="data-table">
          <thead>
            <tr>
...
${a} Briefings konnten nicht gelöscht werden.`):alert(i),r.forEach(o=>{const l=document.querySelector(`tr[data-id="${o}"]`);l&&l.remove()}),this.selectedBriefings.clear(),this.updateSelection(),this.updateSelectAllCheckbox();const s=document.getElementById("briefings-table-body");s&&s.children.length===0&&await this.loadAndRender(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:"briefing",action:"bulk-deleted",count:n}}))}else alert("Keine Briefings konnten gelöscht werden.")}updateSelection(){const e=this.selectedBriefings.size,t=document.getElementById("btn-deselect-all"),n=document.getElementById("btn-delete-selected");t&&(t.style.display=e>0?"inline-block":"none"),n&&(n.style.display=e>0?"inline-block":"none")}updateSelectAllCheckbox(){const e=document.getElementById("select-all-briefings"),t=document.querySelectorAll(".briefing-check");if(!e||t.length===0)return;const n=Array.from(t).filter(r=>r.checked).length,a=t.length;e.checked=n===a,e.indeterminate=n>0&&n<a}async updateTable(e){const t=document.getElementById("briefings-table-body");if(!t)return;if(!e||e.length===0){const{renderEmptyState:i}=await C(async()=>{const{renderEmptyState:s}=await import("./core-C7Vz5Umf.js").then(o=>o.F);return{renderEmptyState:s}},[]);i(t);return}const n=i=>i?new Date(i).toLocaleDateString("de-DE"):"-",a=i=>window.validatorSystem.sanitizeHtml(i||"—"),r=e.map(i=>`
    ...
    `}destroy(){window.setContentSafely("")}}const fe=new dt;class ct{constructor(){this.formData={}}async init(){console.log("🎯 ANSPRECHPARTNERCREATE: Initialisiere Ansprechpartner-Erstellung"),this.showCreateForm()}showCreateForm(){console.log("🎯 ANSPRECHPARTNERCREATE: Zeige Ansprechpartner-Erstellungsformular mit FormSystem"),window.setHeadline("Neuen Ansprechpartner anlegen");const e=window.formSystem.renderFormOnly("ansprechpartner"),t=document.getElementById("dashboard-content");t.innerHTML=`
      <div class="page-header">
        <div class="page-header-left">
...
${t}`)}showSuccessMessage(e){console.log(`✅ ${e}`)}showErrorMessage(e){console.error(`❌ ${e}`),alert(e)}}class mt{constructor(){this.ansprechpartner=null,this.ansprechpartnerId=null,this.formConfig=null,this.formRenderer=null,this.dataLoader=null,this.formSystem=null}async init(e){if(this.ansprechpartnerId=e,console.log("🎯 ANSPRECHPARTNERDETAIL: Initialisiere Detail-Seite für:",e),e==="new"){console.log("🎯 ANSPRECHPARTNERDETAIL: Verwende AnsprechpartnerCreate für neuen Ansprechpartner"),H.showCreateForm();return}else await this.loadAnsprechpartnerData(),this.render(),this.bindEvents()}async loadAnsprechpartnerData(){try{console.log("🔄 ANSPRECHPARTNERDETAIL: Lade Ansprechpartner-Daten...");const{data:e,error:t}=await window.supabase.from("ansprechpartner").select(`
    ...
    `,window.formSystem.bindFormEvents("rechnung",null);const t=document.getElementById("rechnung-form");t&&(t.onsubmit=async n=>{n.preventDefault(),await this.handleCreateSubmit()})}async handleCreateSubmit(){try{const e=document.getElementById("rechnung-form"),t=new FormData(e),n={};for(const[d,u]of t.entries())n[d]=u;e.querySelectorAll('select[data-searchable="true"]').forEach(d=>{const u=d.parentNode.querySelector(".searchable-select-container");if(u){const h=u.querySelector(".searchable-select-input");if(h&&h.value){const p=Array.from(d.options).find(b=>b.textContent.trim()===h.value.trim());p&&(n[d.name]=p.value,console.log(`✅ Searchable Select ${d.name} korrigiert: ${h.value} → ${p.value}`))}}});const i=["auftrag_id","kooperation_id","unternehmen_id","kampagne_id"].filter(d=>!n[d]||n[d].trim()==="");if(i.length>0){alert(`Bitte füllen Sie alle Pflichtfelder aus: ${i.join(", ")}`);return}console.log("📋 Submit-Daten vor Übertragung:",n),console.log("🔍 auftrag_id Wert:",n.auftrag_id,typeof n.auftrag_id);const s=e.querySelector('.uploader[data-name="pdf_file"]');let o=null,l=null;if(s&&s.__uploaderInstance&&s.__uploaderInstance.files.length&&window.supabase){const d=s.__uploaderInstance.files[0],u="rechnungen",h=`${n.unternehmen_id||"unknown"}/${Date.now()}_${d.name}`,{error:m}=await window.supabase.storage.from(u).upload(h,d,{cacheControl:"3600",upsert:!1,contentType:d.type});if(m)throw m;const{data:p}=window.supabase.storage.from(u).getPublicUrl(h);o=p.publicUrl,l=h}o&&(n.pdf_url=o,n.pdf_path=l);const c=await window.dataService.createEntity("rechnung",n);if(c.success){try{const d=e.querySelector('.uploader[data-name="belege_files"]');if(d&&d.__uploaderInstance&&d.__uploaderInstance.files.length&&window.supabase){const u=c.id,h=Array.from(d.__uploaderInstance.files);for(const m of h){const p="rechnung-belege",b=`${u}/${Date.now()}_${Math.random().toString(36).slice(2)}_${m.name}`,{error:f}=await window.supabase.storage.from(p).upload(b,m,{cacheControl:"3600",upsert:!1,contentType:m.type});if(f)throw f;const{data:w}=await window.supabase.storage.from(p).createSignedUrl(b,3600*24*7),v=w?.signedUrl||"";await window.supabase.from("rechnung_belege").insert({rechnung_id:u,file_name:m.name,file_path:b,file_url:v,content_type:m.type,size:m.size,uploaded_by:window.currentUser?.id||null})}}}catch(d){console.warn("⚠️ Belege-Upload teilweise fehlgeschlagen:",d)}alert("Rechnung erstellt"),window.navigateTo(`/rechnung/${c.id}`)}else throw new Error(c.error||"Unbekannter Fehler")}catch(e){alert(`Fehler: ${e.message}`)}}bindEvents(){document.addEventListener("click",e=>{e.target.id==="btn-edit-rechnung"&&this.showEditForm()})}showEditForm(){const e=window.formSystem.renderFormOnly("rechnung",this.data);window.content.innerHTML=`
      ...
      <div class="form-page">${e}</div>
    `,window.formSystem.bindFormEvents("rechnung",this.data)}}const ft=new bt;class vt{constructor(){this.rows=[]}async init(){if(window.setHeadline("Mitarbeiter"),!(window.currentUser?.rolle==="admin"||window.canViewPage?.("mitarbeiter")||window.checkUserPermission("dashboard","can_view"))){window.content.innerHTML=`
        <div class="error-message">
...
`)}`)),alert(l),this.genericDeselectAll(),this.currentListInstance&&typeof this.currentListInstance.loadAndRender=="function"?await this.currentListInstance.loadAndRender():window.location.reload(),window.dispatchEvent(new CustomEvent("entityUpdated",{detail:{entity:e,action:"bulk-deleted",count:r}}))}detectCurrentEntityType(){const e=window.location.pathname.split("/").filter(n=>n);if(e.length>0){const n=e[0];if(["creator","unternehmen","kampagne","marke","auftrag","ansprechpartner","kooperation"].includes(n))return n}const t=["creator","unternehmen","kampagne","marke","auftrag","ansprechpartner","kooperation"];for(const n of t){const a=this.getEntityConfig(n);if(document.querySelectorAll(a.checkboxSelector).length>0)return console.log(`🔧 BulkActionSystem: Entity-Type ${n} aus Checkboxen erkannt`),n}return null}getEntityConfig(e){return{creator:{checkboxSelector:".creator-check",selectAllId:"select-all-creators",displayName:"Creator"},unternehmen:{checkboxSelector:".unternehmen-check",selectAllId:"select-all-unternehmen",displayName:"Unternehmen"},kampagne:{checkboxSelector:".kampagne-check",selectAllId:"select-all-kampagnen",displayName:"Kampagnen"},marke:{checkboxSelector:".marke-check",selectAllId:"select-all-marken",displayName:"Marken"},auftrag:{checkboxSelector:".auftrag-check",selectAllId:"select-all-auftraege",displayName:"Aufträge"},ansprechpartner:{checkboxSelector:".ansprechpartner-check",selectAllId:"select-all-ansprechpartner",displayName:"Ansprechpartner"},kooperation:{checkboxSelector:".kooperation-check",selectAllId:"select-all-kooperationen",displayName:"Kooperationen"}}[e]||{checkboxSelector:`.${e}-check`,selectAllId:`select-all-${e}`,displayName:e}}hideButtons(){const e=document.getElementById("selected-count"),t=document.getElementById("btn-deselect-all"),n=document.getElementById("btn-delete-selected");e&&(e.style.display="none"),t&&(t.style.display="none"),n&&(n.style.display="none")}destroy(){this.boundEventListeners.forEach(({element:e,type:t,handler:n})=>{e.removeEventListener(t,n)}),this.boundEventListeners.clear()}}const ye=new _t;class Et{constructor(){this.notifications=[],this.unreadCount=0,this.pollIntervalMs=6e4,this._timer=null,this._initialized=!1}init(){this._initialized||(this._initialized=!0,this.bindUI(),this.refresh(!0),this.startPolling(),window.addEventListener("notificationsRefresh",()=>this.refresh()))}destroy(){this._timer&&clearInterval(this._timer),this._timer=null,this._initialized=!1}startPolling(){this._timer&&clearInterval(this._timer),this._timer=setInterval(()=>this.refresh(),this.pollIntervalMs)}bindUI(){const e=document.querySelector("#notificationBell, .notification-bell"),t=document.querySelector("#notificationDropdown, .notification-dropdown");if(!e||!t)return;const n=()=>{t.classList.toggle("show")};e.addEventListener("click",a=>{a.preventDefault(),n()}),document.addEventListener("click",a=>{!t.contains(a.target)&&!e.contains(a.target)&&t.classList.remove("show")})}async refresh(e=!1){try{if(!window.supabase||!window.currentUser?.id)return;const{data:t,error:n}=await window.supabase.from("notifications").select("id, user_id, type, entity, entity_id, title, message, created_at, read_at").eq("user_id",window.currentUser.id).order("created_at",{ascending:!1}).limit(20);if(n)throw n;this.notifications=t||[],this.unreadCount=(this.notifications||[]).filter(a=>!a.read_at).length,this.renderBadge(),this.renderDropdown(),!e&&this.unreadCount>0}catch(t){console.warn("⚠️ Notifications refresh failed",t)}}renderBadge(){const e=document.querySelector("#notificationBadge, .notification-badge");if(!e)return;const t=this.unreadCount;e.textContent=t>99?"99+":String(t||""),e.style.display=t>0?"":"none"}renderDropdown(){const e=document.querySelector("#notificationDropdown, .notification-dropdown");if(!e)return;const t=(this.notifications||[]).map(a=>{const r=this.getEntityUrl(a.entity,a.entity_id),i=new Date(a.created_at).toLocaleString("de-DE");return`
    ...
    `}getEntityLabel(e){return{creator:"Creator",kampagne:"Kampagnen",kooperation:"Kooperationen",briefing:"Briefings",rechnung:"Rechnungen",unternehmen:"Unternehmen",marke:"Marken",auftrag:"Aufträge",ansprechpartner:"Ansprechpartner"}[e]||e}bind(){const e=document.querySelectorAll(".tab-btn"),t=document.querySelectorAll(".tab-pane");e.forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.tab;e.forEach(r=>r.classList.remove("active")),t.forEach(r=>r.classList.remove("active")),n.classList.add("active"),document.getElementById(`tab-${a}`)?.classList.add("active")})}),document.getElementById("edit-profile-btn")?.addEventListener("click",()=>{this.isEditing=!this.isEditing,this.render()}),document.getElementById("save-profile-btn")?.addEventListener("click",async()=>{await this.saveProfile()}),document.getElementById("cancel-edit-btn")?.addEventListener("click",()=>{this.isEditing=!1,this.render()})}async saveProfile(){try{const t=document.getElementById("name-input")?.value?.trim();if(!t){alert("Name darf nicht leer sein");return}const{error:n}=await window.supabase.from("benutzer").update({name:t,updated_at:new Date().toISOString()}).eq("id",this.user.id);if(n)throw n;window.currentUser.name=t,this.user.name=t,window.setupHeaderUI?.(),this.isEditing=!1,await this.render(),alert("Profil erfolgreich aktualisiert"),console.log("✅ Profil gespeichert")}catch(e){console.error("❌ Fehler beim Speichern:",e),alert("Fehler beim Speichern des Profils")}}formatDate(e){return e?new Date(e).toLocaleDateString("de-DE",{year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"}):"Unbekannt"}destroy(){console.log("ProfileDetail: Cleaning up...")}}class At{constructor(){this.modules=new Map,this.currentModule=null}register(e,t){this.modules.set(e,t)}navigateTo(e){const t=e.replace(/^\//,""),[n,a]=t.split("/");this.currentModule&&this.currentModule.destroy&&(console.log("🗑️ Zerstöre aktuelles Modul:",this.currentModule.constructor.name),this.currentModule.destroy(),this.currentModule=null);let r=n,i=this.modules.get(r);if(a&&n==="creator"&&a!=="new"&&(r="creator-detail",i=this.modules.get(r),console.log(`🎯 Creator-Details erkannt, verwende Modul: ${r}`)),n==="creator-lists"&&a&&(r="creator-list-detail",i=this.modules.get(r),console.log("🎯 Creator-Listen-Detail erkannt")),a&&n==="mitarbeiter"&&a!=="new"&&(r="mitarbeiter-detail",i=this.modules.get(r),console.log(`🎯 Mitarbeiter-Details erkannt, verwende Modul: ${r}`)),a&&n==="marke"&&a!=="new"&&(r="marke-detail",i=this.modules.get(r),console.log(`🎯 Marken-Details erkannt, verwende Modul: ${r}`)),a==="neu"&&n==="unternehmen"?(r="unternehmen-create",i=this.modules.get(r),console.log(`🎯 Unternehmen-Erstellung erkannt, verwende Modul: ${r}`)):a&&n==="unternehmen"&&a!=="new"&&a!=="neu"&&(r="unternehmen-detail",i=this.modules.get(r),console.log(`🎯 Unternehmen-Details erkannt, verwende Modul: ${r}`)),a&&n==="auftrag"&&a!=="new"&&(r="auftrag-detail",i=this.modules.get(r),console.log(`🎯 Auftrags-Details erkannt, verwende Modul: ${r}`)),a&&n==="kooperation"&&a!=="new"&&(r="kooperation-detail",i=this.modules.get(r),console.log(`🎯 Kooperations-Details erkannt, verwende Modul: ${r}`)),a&&n==="video"&&a!=="new"&&(r="kooperation-video-detail",i=this.modules.get(r),console.log(`🎯 Video-Details erkannt, verwende Modul: ${r}`)),a&&n==="kampagne"&&a!=="new"&&(r="kampagne-detail",i=this.modules.get(r),console.log(`🎯 Kampagnen-Details erkannt, verwende Modul: ${r}`)),a&&n==="briefing"&&a!=="new"&&(r="briefing-detail",i=this.modules.get(r),console.log(`🎯 Briefing-Details erkannt, verwende Modul: ${r}`)),a&&n==="rechnung"&&a!=="new"&&(r="rechnung-detail",i=this.modules.get(r),console.log(`🎯 Rechnung-Details erkannt, verwende Modul: ${r}`)),a&&n==="ansprechpartner"&&(r="ansprechpartner-detail",i=this.modules.get(r),console.log(`🎯 Ansprechpartner-Details/Erstellung erkannt, verwende Modul: ${r}`)),i)return console.log(`✅ Modul gefunden: ${r}`,i),this.currentModule=i,a==="new"?n==="rechnung"?(r="rechnung-detail",i=this.modules.get(r),i.init?.("new")):(console.log(`📝 Zeige Erstellungsformular für: ${n}`),r.includes("-detail")?i.init?.("new"):i.showCreateForm?.()):a?(console.log(`👁️ Zeige Details für: ${n}/${a}`),i.init?.(a)):(console.log(`🚀 Initialisiere Modul: ${n}`),i.init());console.warn(`❌ Modul nicht gefunden: ${r}`),this.loadDashboard()}loadDashboard(){const e=this.modules.get("dashboard");e?(this.currentModule=e,e.init()):(window.setHeadline("Dashboard"),window.content.innerHTML=`
      ...
      `)}}const S=new At;window.moduleRegistry=S;S.register("creator",Z);S.register("creator-detail",G);S.register("creator-lists",Me);S.register("creator-list-detail",Ie);S.register("unternehmen",W);S.register("unternehmen-create",Y);S.register("auftrag",J);S.register("marke",Q);S.register("marke-detail",X);S.register("marke-create",U);S.register("unternehmen-detail",de);S.register("auftrag-detail",ce);S.register("kooperation",ue);S.register("kooperation-detail",he);S.register("kooperation-video-detail",me);S.register("kampagne",pe);S.register("kampagne-detail",ge);S.register("ansprechpartner",ve);S.register("ansprechpartner-detail",we);S.register("ansprechpartner-create",H);S.register("briefing",be);S.register("briefing-detail",fe);S.register("rechnung",gt);S.register("rechnung-detail",ft);S.register("mitarbeiter",wt);S.register("mitarbeiter-detail",kt);S.register("dashboard",_e);const Ee=new St;window.profileDetail=Ee;S.register("profile",Ee);window.navigateTo=g=>{S.navigateTo(g)};window.filterSystem=_;window.creatorList=Z;window.creatorDetail=G;window.unternehmenList=W;window.unternehmenCreate=Y;window.auftragList=J;window.markeList=Q;window.markeDetail=X;window.markeCreate=U;window.unternehmenDetail=de;window.auftragDetail=ce;window.kooperationList=ue;window.kooperationDetail=he;window.kooperationVideoDetail=me;window.kampagneList=pe;window.kampagneDetail=ge;window.briefingDetail=fe;window.kampagneUtils=ot;window.briefingList=be;window.AuthService=K;window.AuthUtils=q;window.authService=K;window.authUtils=q;window.navigationSystem=ee;window.permissionSystem=De;window.dataService=x;window.validatorSystem=B;window.creatorUtils=ne;window.formSystem=le;window.notizenSystem=Je;window.bewertungsSystem=Xe;window.ActionsDropdown=L;window.bulkActionSystem=ye;window.notificationSystem=ke;window.ansprechpartnerList=ve;window.ansprechpartnerDetail=we;window.ansprechpartnerCreate=H;window.dashboardModule=_e;document.addEventListener("DOMContentLoaded",async()=>{if(console.log("🎯 Initialisiere Event-basiertes Modul-System..."),window.appRoot=document.getElementById("app-root"),window.loginRoot=document.getElementById("login-root"),window.content=document.getElementById("dashboard-content"),window.nav=document.getElementById("main-nav"),window.supabase&&window.CONFIG?.SUPABASE?.URL&&window.CONFIG?.SUPABASE?.KEY)try{window.supabase=window.supabase.createClient(window.CONFIG.SUPABASE.URL,window.CONFIG.SUPABASE.KEY),console.log("✅ Supabase initialisiert")}catch(e){console.error("❌ Supabase-Initialisierung fehlgeschlagen:",e)}else console.warn("⚠️ Supabase nicht verfügbar - verwende Offline-Modus");if(await K.checkAuth()){console.log("✅ Benutzer ist authentifiziert"),ee.init(),L.init(),ye.init(),ke.init(),window.appRoot.style.display="",window.loginRoot.style.display="none";const e=location.pathname;e==="/"||e==="/dashboard"?S.loadDashboard():S.navigateTo(e),window.setupHeaderUI?.()}else console.log("🔐 Benutzer nicht authentifiziert - zeige Login"),q.showLogin()});window.handleLogout=async()=>{try{await K.signOut()}catch(g){console.warn("Logout warn:",g)}finally{window.appRoot&&(window.appRoot.style.display="none"),window.loginRoot&&(window.loginRoot.style.display="");const g=document.getElementById("mitarbeiterPanelOverlay"),e=document.getElementById("mitarbeiterPanel");g&&(g.style.display="none"),e&&(e.style.display="none"),q.showLogin?.()}};window.setupHeaderUI=()=>{try{const g=document.querySelector(".profile-img"),e=document.querySelector(".profile-initials"),t=(window.currentUser?.name||"").trim();if(t&&e){const s=t.split(/\s+/).filter(Boolean),o=(s[0]?.[0]||"").toUpperCase()+(s[1]?.[0]||"").toUpperCase();e.textContent=o||(t[0]||"?").toUpperCase()}window.currentUser?.avatar_url&&g&&(g.src=window.currentUser.avatar_url,g.style.display="",e&&(e.style.display="none"));const n=document.querySelector(".profile-btn"),a=document.querySelector(".profile-dropdown");n&&a&&!n.dataset.bound&&(n.dataset.bound="true",n.addEventListener("click",s=>{s.stopPropagation();const o=a.getAttribute("aria-hidden")==="false";a.setAttribute("aria-hidden",o?"true":"false"),n.setAttribute("aria-expanded",o?"false":"true")}),a.addEventListener("click",s=>{const o=s.target.closest("[data-action]")?.dataset.action;o&&(s.preventDefault(),a.setAttribute("aria-hidden","true"),n.setAttribute("aria-expanded","false"),o==="view-profile"&&S.navigateTo("/profile"))}),document.addEventListener("click",()=>{a.setAttribute("aria-hidden","true"),n.setAttribute("aria-expanded","false")}));const r=document.querySelector(".quick-menu-btn"),i=document.querySelector(".quick-menu-dropdown");if(r&&i&&!r.dataset.bound){const s=()=>{i.classList.remove("show"),r.setAttribute("aria-expanded","false")};r.addEventListener("click",l=>{l.preventDefault();const c=i.classList.toggle("show");r.setAttribute("aria-expanded",String(c))}),document.addEventListener("click",l=>{!i.contains(l.target)&&!r.contains(l.target)&&s()});const o={unternehmen:"unternehmen",marke:"marke",auftrag:"auftrag",ansprechpartner:"ansprechpartner",kampagne:"kampagne",briefing:"briefing",kooperation:"kooperation",creator:"creator"};i.querySelectorAll(".quick-menu-item").forEach(l=>{const c=l.getAttribute("data-entity"),d=o[c],u=d?!!window.permissionSystem?.getEntityPermissions(d)?.can_edit:!1;l.style.display=u||window.currentUser?.rolle==="admin"?"":"none"}),i.addEventListener("click",l=>{const c=l.target.closest(".quick-menu-item");if(!c)return;const d=c.getAttribute("data-entity");if(d){switch(d){case"unternehmen":window.navigateTo("/unternehmen/neu");break;case"marke":window.navigateTo("/marke/new");break;case"auftrag":window.navigateTo("/auftrag/new");break;case"ansprechpartner":window.navigateTo("/ansprechpartner/new");break;case"kampagne":window.navigateTo("/kampagne/new");break;case"briefing":window.navigateTo("/briefing/new");break;case"kooperation":window.navigateTo("/kooperation/new");break;case"creator":window.navigateTo("/creator/new");break}s()}}),r.dataset.bound="true"}}catch(g){console.warn("Header/Quick-Menu setup warn:",g)}};
