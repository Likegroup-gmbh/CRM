#!/usr/bin/env node
/**
 * Einmaliger Merge: DA direkt_KZV_0326 + DADirekt_0426_OP-Schutz Katze
 * → Base "DA Direkt TKV OP-Schutz UGC", Ziel wird "DA Direkt OP-Schutz Q2".
 *
 * - Alle kampagne_id-Referenzen der Quellen auf das Ziel umschreiben
 * - Ziel: Name, Marke DA Direkt OP Schutz, videoanzahl=36, creatoranzahl=12
 * - Herkunfts-Tags pro Kooperation (auch Ziel-Koops)
 * - Dropbox: Vertrags-PDFs, Rechnungs-PDFs und Video-Ordner
 *   nach /Vertraege/.../DA Direkt OP-Schutz Q2 verschieben, DB-Pfade + URLs updaten
 * - Validierung, danach Quellkampagnen löschen + leere Alt-Ordner aufräumen
 *
 * Usage:
 *   node scripts/merge-dadirekt-op-schutz.js --dry-run
 *   node scripts/merge-dadirekt-op-schutz.js
 *   node scripts/merge-dadirekt-op-schutz.js --db-only
 *   node scripts/merge-dadirekt-op-schutz.js --dropbox-only
 *
...
// --- Config ---
const TARGET_KAMPAGNE_ID = 'f9ee3d34-243a-4301-a69d-bd94f97975a0'; // DA Direkt TKV OP-Schutz UGC
const NEW_KAMPAGNENNAME = 'DA Direkt OP-Schutz Q2';
const NEW_EIGENER_NAME = 'DA Direkt OP-Schutz Q2';
const NEW_VIDEOANZAHL = 36; // 24 + 0 + 12
const NEW_CREATORANZAHL = 12; // 8 + 0 + 4
const NEW_UGC_PAID_VIDEO = 36;
const NEW_UGC_PAID_CREATOR = 12;
const NEW_ART_DER_KAMPAGNE = ['UGC Paid', 'UGC Organic'];
const NEW_MARKE_ID = '07b7d665-a9ff-42f2-a0c6-187d27d26bc4'; // DA Direkt OP Schutz
const AUFTRAG_ID = '0e58fefb-cc3b-4a0e-83d7-2ecaaf8bb145';

const TARGET_HERITAGE_TAG = 'TKV OP-Schutz UGC';

const SOURCES = [
  {
    id: '05dc09bc-8e6f-4263-9fed-11dc7f32ffba',
    label: 'DA direkt_KZV_0326',
    heritageTag: 'KZV_0326',
  },
  {
    id: 'e2bf8111-5fa2-4996-84a6-626b8dc9efe8',
    label: 'DADirekt_0426_OP-Schutz Katze',
    heritageTag: '0426_OP-Schutz Katze',
  },
];

const ALL_KAMPAGNE_IDS = [TARGET_KAMPAGNE_ID, ...SOURCES.map(s => s.id)];

const TARGET_DROPBOX_BASE = '/Vertraege/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DA Direkt OP-Schutz Q2';
...
const OLD_FOLDER_CANDIDATES = [
  '/Vertraege/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DADirekt - 2-2',
  '/Vertraege/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DADirekt_0426_OP-Schutz Katze',
  '/Vertraege/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DADirekt - 1-2',
  '/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DA DIrekt Tierkrankenschutzversicherung/DADirekt_0326_OP-Schutz Hunde',
  '/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DA DIrekt Tierkrankenschutzversicherung/DADirekt_0426_OP-Schutz Katze',
  '/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DA DIrekt Tierkrankenschutzversicherung/DA Direkt TKV OP-Schutz UGC',
];
...
async function renameTarget(sb) {
  console.log('\n=== Umbenennen + Zähler + Marke ===');
  console.log(`  name → "${NEW_KAMPAGNENNAME}", videoanzahl → ${NEW_VIDEOANZAHL}, creatoranzahl → ${NEW_CREATORANZAHL}`);
  console.log(`  ugc_paid → ${NEW_UGC_PAID_CREATOR}/${NEW_UGC_PAID_VIDEO}, marke → DA Direkt OP Schutz`);
  if (DRY_RUN) return;
  const { error } = await sb.from('kampagne')
    .update({
      kampagnenname: NEW_KAMPAGNENNAME,
      eigener_name: NEW_EIGENER_NAME,
      videoanzahl: NEW_VIDEOANZAHL,
      creatoranzahl: NEW_CREATORANZAHL,
      ugc_paid_video_anzahl: NEW_UGC_PAID_VIDEO,
      ugc_paid_creator_anzahl: NEW_UGC_PAID_CREATOR,
      art_der_kampagne: NEW_ART_DER_KAMPAGNE,
      marke_id: NEW_MARKE_ID,
    })
    .eq('id', TARGET_KAMPAGNE_ID);
  if (error) throw new Error(`Rename failed: ${error.message}`);
}

async function updateMarkeRefs(sb) {
  console.log('\n=== Marke auf OP Schutz umhängen ===');
  console.log(`  auftrag ${AUFTRAG_ID.slice(0, 8)}… + skripte/strategie auf Ziel`);
  if (DRY_RUN) return;

  const { error: auftragErr } = await sb.from('auftrag')
    .update({ marke_id: NEW_MARKE_ID, kampagnenanzahl: 1 })
    .eq('id', AUFTRAG_ID);
  if (auftragErr) throw new Error(`Auftrag marke update failed: ${auftragErr.message}`);

  for (const table of ['skripte', 'strategie']) {
    const { error } = await sb.from(table)
      .update({ marke_id: NEW_MARKE_ID })
      .eq('kampagne_id', TARGET_KAMPAGNE_ID);
    if (error) throw new Error(`${table} marke update failed: ${error.message}`);
  }
}
...
async function validate(sb) {
  ...
  const targetChecks = [
    ...
    { label: 'vertraege', table: 'vertraege', expected: 14 },
    { label: 'rechnung', table: 'rechnung', expected: 12 },
    { label: 'strategie', table: 'strategie', expected: 1 },
    { label: 'skripte', table: 'skripte', expected: 1 },
  ];
  const { data: videoKoops } = await sb.from('kooperationen').select('id').eq('kampagne_id', TARGET_KAMPAGNE_ID);
  const videoKoopIds = (videoKoops || []).map(k => k.id);
  let videoCount = 0;
  if (videoKoopIds.length) {
    const { count } = await sb.from('kooperation_videos').select('id', { count: 'exact', head: true }).in('kooperation_id', videoKoopIds);
    videoCount = count ?? 0;
  }
  const videosOk = videoCount === 24;
  console.log(`    ${videosOk ? '✓' : '✗'} kooperation_videos: ${videoCount} (expected 24)`);
  if (!videosOk) allOk = false;
  ...
  const { data: target } = await sb.from('kampagne')
    .select('kampagnenname, eigener_name, videoanzahl, creatoranzahl, ugc_paid_video_anzahl, ugc_paid_creator_anzahl, marke_id, art_der_kampagne')
    .eq('id', TARGET_KAMPAGNE_ID)
    .single();
  const renameOk = target?.kampagnenname === NEW_KAMPAGNENNAME
    && target?.eigener_name === NEW_EIGENER_NAME
    && target?.videoanzahl === NEW_VIDEOANZAHL
    && target?.creatoranzahl === NEW_CREATORANZAHL
    && target?.ugc_paid_video_anzahl === NEW_UGC_PAID_VIDEO
    && target?.ugc_paid_creator_anzahl === NEW_UGC_PAID_CREATOR
    && target?.marke_id === NEW_MARKE_ID;
  console.log(`    ${renameOk ? '✓' : '✗'} kampagne: name="${target?.kampagnenname}", video=${target?.videoanzahl}, creator=${target?.creatoranzahl}, ugc_paid=${target?.ugc_paid_creator_anzahl}/${target?.ugc_paid_video_anzahl}, marke=${target?.marke_id?.slice(0, 8)}…`);
  if (!renameOk) allOk = false;

  const { data: auftrag } = await sb.from('auftrag').select('marke_id, kampagnenanzahl').eq('id', AUFTRAG_ID).single();
  const auftragOk = auftrag?.marke_id === NEW_MARKE_ID && auftrag?.kampagnenanzahl === 1;
  console.log(`    ${auftragOk ? '✓' : '✗'} auftrag: marke=${auftrag?.marke_id?.slice(0, 8)}…, kampagnenanzahl=${auftrag?.kampagnenanzahl}`);
  if (!auftragOk) allOk = false;
...
async function main() {
  ...
  console.log('DA Direkt OP-Schutz Merge: KZV_0326 + 0426 Katze → TKV OP-Schutz UGC (= "DA Direkt OP-Schutz Q2")');
  ...
  if (!DB_ONLY) {
    ...
    dropboxErrors = dropboxResult?.errors?.length || 0;
    if (dropboxErrors) {
      console.warn(`\n⚠ ${dropboxErrors} Dropbox error(s) — see log above`);
    }
  }
  ...
  if (DROPBOX_ONLY) {
    ...
    process.exit(dropboxErrors ? 1 : 0);
  }
