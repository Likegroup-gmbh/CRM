#!/usr/bin/env node
/**
 * Einmaliger Merge: DA Direkt KFZ Neuwagen + Zweitwagen → Motorrad (Ziel),
 * Ziel wird umbenannt in "DA Direkt KFZ UGC".
 *
 * - Alle kampagne_id-Referenzen der Quellen auf das Ziel umschreiben
 * - Ziel: eigener_name='DA Direkt KFZ UGC', videoanzahl=21, creatoranzahl=7
 * - Herkunfts-Tags pro Kooperation (eigener_name der Ursprungskampagne, auch Ziel-Koops)
 * - Dropbox: Vertrags-PDFs, 1 Rechnungs-PDF und Video-Ordner (Share-Links)
 *   physisch nach /Vertraege/.../DA Direkt KFZ UGC verschieben, DB-Pfade + URLs updaten
 * - Validierung, danach Quellkampagnen löschen + leere Alt-Ordner aufräumen
 *
 * Usage:
 *   node scripts/merge-dadirekt-kfz.js --dry-run
 *   node scripts/merge-dadirekt-kfz.js
 *   node scripts/merge-dadirekt-kfz.js --db-only
 *   node scripts/merge-dadirekt-kfz.js --dropbox-only
 *
...
// --- Config ---
const TARGET_KAMPAGNE_ID = 'aac1f682-0e28-4846-b34c-3dd25dac9ff1'; // DA Direkt_KFZ_Motorrad
const NEW_EIGENER_NAME = 'DA Direkt KFZ UGC';
const NEW_VIDEOANZAHL = 21; // 9 + 6 + 6
const NEW_CREATORANZAHL = 7; // 3 + 2 + 2 Kooperationen

const TARGET_HERITAGE_TAG = 'DA Direkt_KFZ_Motorrad';

const SOURCES = [
  {
    id: '22b2114d-6db2-4a09-8027-34e85a11d2ea',
    label: 'DADirekt_KFZ_0326_Zweitwagen',
    heritageTag: 'DADirekt_KFZ_0326_Zweitwagen',
  },
  {
    id: '3113d0d0-8a62-4b69-a4dd-46e8a0a4866a',
    label: 'DA Direkt_KFZ_Neuwagen',
    heritageTag: 'DA Direkt_KFZ_Neuwagen',
  },
];

const ALL_KAMPAGNE_IDS = [TARGET_KAMPAGNE_ID, ...SOURCES.map(s => s.id)];

const TARGET_DROPBOX_BASE = '/Vertraege/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DA Direkt KFZ UGC';
...
const OLD_FOLDER_CANDIDATES = [
  '/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DA Direkt KFZ-Versicherung/DADirekt_KFZ_0326_Zweitwagen',
  '/Vertraege/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DADirekt_KFZ_0326_Zweitwagen',
  '/Vertraege/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DA Direkt_KFZ_Motorrad',
  '/Vertraege/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DADirekt - 1-2',
  '/Vertraege/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DA Direkt_KFZ_Neuwagen',
  '/DA Deutsche Allgemeine Versicherung Aktiengesellschaft/DA Direkt KFZ-Versicherung/DA Direkt_KFZ_Neuwagen',
];
...
async function validate(sb) {
  ...
  console.log('\n  Target counts:');
  const targetChecks = [
    { label: 'kooperationen', table: 'kooperationen', expected: 7 },
    { label: 'vertraege', table: 'vertraege', expected: 7 },
    { label: 'rechnung', table: 'rechnung', expected: 7 },
    { label: 'strategie', table: 'strategie', expected: 3 },
  ];
...
async function main() {
  ...
  console.log('DA Direkt KFZ Merge: Zweitwagen + Neuwagen → Motorrad (= "DA Direkt KFZ UGC")');
